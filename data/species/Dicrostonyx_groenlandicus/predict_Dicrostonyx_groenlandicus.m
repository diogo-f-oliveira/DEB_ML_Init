function [prdData, info] = predict_Dicrostonyx_groenlandicus(par, data, auxData)
  ...
  [t_pm, t_xm, t_bm, l_pm, l_xm, l_bm] = get_tx([g_m k l_T v_Hb v_Hx v_Hpm], f);
  tT_pm = (t_pm - t_bm)/ k_M/ TC;      % d, time since birth at puberty
  L_im = f * L_mm;                     % cm, ultimate structural length
  Ww_im = L_im^3 * (1 + f * w_m);      % g, ultimate wet weight
  L_xm = L_mm * l_xm;                  % g, structural length at weaning
  ...
  L = L_i - (L_i - L_x) * exp( - rT_B * tW_f(:,1));     % cm, structural length at time
  EWw_f = L.^3 * (1 + f * w);                           % g, wet weight
  % 
  rT_Bm = TC * k_M/ 3/ (1 + f/ g_m);                    % 1/d, von Bert growth rate
  L = L_im - (L_im - L_xm) * exp( - rT_Bm * tW_m(:,1)); % cm, structural length at time
