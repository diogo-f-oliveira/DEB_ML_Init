function [prdData, info] = predict_Epinephelus_polyphekadion(par, data, auxData)
  ...
  % time-length
  tf = [1000 f_1000; 3000 f_3000; 5000 f_5000; 7000 f_7000; 9000 f_9000]; tf(:,1) = tf(:,1)*kT_M;
  tvel = get_tj(pars_tj, tf, [], tL(:,1)*kT_M);
  ELw = L_m * tvel(:,4)/ del_M; % cm, total length
  
  % length-weight
  EWw = (LW(:,1)*del_M).^3*(1+f_5000*ome); % g, weight
