function [prdData, info] = predict_Pan_troglodytes(par, data, auxData)
  ...
  Lw_im = L_im/ del_M;                 % cm, head-body length
  ...
  % time-weight 
  [t_p, t_x, t_b, l_p, l_x, l_b, info] = get_tx(pars_tx, f_tW);
  if ~info
    prdData = []; return
  end
  L_b = L_m * l_b; L_p = L_m * l_p; L_i = L_m * f_tW; % compose parameters
  rT_B = kT_M/ 3/ (1 + f_tW/ g);    % 1/d, von Bert growth rate
  L = L_i - (L_i - L_b) * exp( - rT_B * tWw_f(:,1));      % cm, structural length at time
  EWw_f = L.^3 * (1 + f_tW * w);    % g, wet weight
  %
  tT_p = (t_p - t_b)/ kT_M; L_im = L_mm * f_tW;% compose parameters
  rT_Bm = kT_M/ 3/ (1 + f_tW/ g_m); % 1/d, von Bert growth rate
  L_bp = L_i - (L_i - L_b) * exp( - rT_B * tWw_m(tWw_m(:,1) < tT_p,1));  % cm, structural length at time
  L_pi = L_im - (L_im - L_p) * exp( - rT_Bm * (tWw_m(tWw_m(:,1) >= tT_p,1) - tT_p));  % cm, structural length at time
  EWw_m = [L_bp.^3 * (1 + f_tW * w); L_pi.^3 * (1 + f_tW * w_m)];  % g, wet weight
