function [prdData, info] = predict_Hybognathus_amarus(par, data, auxData)
...
%% compute temperature correction factors
  ...
  Lw_p = L_p/ del_M;                % cm, std length at puberty
  ...
  %
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_LN);
  rT_j = rho_j * kT_M; rT_B = rho_B * kT_M; tT_j = (t_j - t_b)/ kT_M; L_j = L_m * l_j; L_i = L_m * l_i;
  L_bj = L_b * exp(rT_j * tL1(tL1(:,1)<=tT_j,1)/ 3);
  L_ji = L_i - (L_i - L_j) * exp( - rT_B * (tL1(tL1(:,1)>tT_j,1) - tT_j)); % cm, struc length
  ELw_1 = [L_bj; L_ji]/ del_M; % cm, std length
