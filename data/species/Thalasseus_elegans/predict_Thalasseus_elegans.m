function [prdData, info] = predict_Thalasseus_elegans(par, data, auxData)
  ...
  % time-weight 
  tf = [0 15 16 35; f_00 f_00 f_H f_H]'; tf(:,1) = tf(:,1) * kT_M; % high-energy
  tvel = get_tp(pars_tp, tf, [], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % time-weight  
  tf = [0 15 16 35; f_00 f_00 f_L f_L]'; tf(:,1) = tf(:,1) * kT_M; % low energy
  tvel = get_tp(pars_tp, tf, [], tW0(:,1)*kT_M);
