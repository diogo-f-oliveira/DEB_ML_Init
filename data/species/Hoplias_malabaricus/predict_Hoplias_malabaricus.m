function [prdData, info] = predict_Hoplias_malabaricus(par, data, auxData)
...
%% compute temperature correction factors
  ...
  TC_tW = tempcorr(temp.tW, T_ref, T_A); % all rates/times at same temp
  ...
  kT_M = TC_tW * k_M; rT_j = rho_j * kT_M; rT_B = rho_B * kT_M; tT_j = (t_j - t_b)/ kT_M;
  L_bj = L_b * exp (rT_j * tW(tW(:,1) < tT_j,1)/ 3);
  L_ji = L_i - (L_i - L_j) * exp (- rT_B * (tW(tW(:,1) >= tT_j,1) - tT_j));
  EWw = [L_bj; L_ji].^3 * (1 + f_tW * ome); % cm, standard length
