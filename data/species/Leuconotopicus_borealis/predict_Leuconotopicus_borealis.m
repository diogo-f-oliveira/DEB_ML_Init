function [prdData, info] = predict_Leuconotopicus_borealis(par, data, auxData)
  ...
  % uni-variate data
  % time-weight
  tf = [0 f3_00; 5 f3_05; 10 f3_10; 15 f3_15]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW3(:,1)*kT_M);
  EWw_3 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f1_00; 5 f1_05; 10 f1_10; 15 f1_15]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW1(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f0_00; 5 f0_05; 10 f0_10; 15 f0_15]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW0(:,1)*kT_M);
  EWw_0 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
