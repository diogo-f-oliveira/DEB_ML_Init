function [prdData, info] = predict_Cynoscion_othonopterus(par, data, auxData)
  ...
  rT_B = TC * rho_B * k_M; rT_j = TC * rho_j * k_M; tT_j = (t_j - t_b)/ k_M/ TC;
  L_b = L_m * l_b;  L_j = L_m * l_j; L_i = L_m * l_i;
  L_bj = L_b * exp(tL1(tL1(:,1) < tT_j,1) * rT_j/ 3); % cm, struct length
  L_ji = L_i - (L_i - L_j) * exp( - rT_B * (tL1(tL1(:,1) >= tT_j,1) - tT_j)); % cm, struct length
