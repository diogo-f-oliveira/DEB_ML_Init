function [prdData, info] = predict_Strix_uralensis(par, data, auxData)
  ...
  L_bm = L_mm * get_lb([g_m; k; v_Hb], f); % cm, struuc length at birth
...
%% uni-variate data
  ...
  tf = [5 15 25; f_05 f_15 f_25]'; tf(:,1) = tf(:,1) * kT_M;
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  % male
  tvel = get_tp(pars_tpm, tf, [t_b, z/z_m, l_b*z/z_m], tW(:,1)*kT_M);
  EWw_m = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome_m); % g, weight
  
  % pack to output
  prdData.tW = [EWw_f EWw_m] ;
