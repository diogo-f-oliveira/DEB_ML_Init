function [prdData, info] = predict_Anas_diazi(par, data, auxData)
...
% uni-variate data
 ...
 % study 1
  [tvel, t_p, t_b, l_p, l_b, info] = get_tp(pars_tp, f, [], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  % study  2
  [tvel, t_p, t_b, l_p, l_b, info] = get_tp(pars_tp, f_2, [], tW(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight


  % pack to output
  prdData.tW = [EWw EWw_2];
end
