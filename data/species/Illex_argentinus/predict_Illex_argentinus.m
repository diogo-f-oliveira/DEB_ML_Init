function [prdData, info] = predict_Illex_argentinus(par, data, auxData)
  ...
  L_amm = L_im - (L_im - L_j_m) * exp(- rT_B * (aT_mm - t_jm/ kT_M));  
  ...
  % time-length
  % female, post metam
  rT_B = rho_B * kT_M; L_0f = L_0f * del_M; 
  L = L_i - (L_i - L_0f) * exp( - rT_B * tL_f(:,1)); 
  ELw_f = L/ del_M;   % cm, dorsal mantle length at time       
  % male
  rT_Bm = rho_Bm * kT_M; L_0m = L_0m * del_M; 
  L = L_im - (L_im - L_0m) * exp( - rT_Bm * tL_m(:,1)); 
  ELw_m = L/ del_M;   % cm, dorsal mantle length at time       
