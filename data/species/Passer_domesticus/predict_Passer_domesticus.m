function [prdData, info] = predict_Passer_domesticus(par, data, auxData)
  ...
  % uni-variate data
  % time-weight
  tf = [0 3 6 9 12; fL_00 fL_03 fL_06 fL_09 fL_12]'; tT = temp.tW(:,[1 2]);
  [t, EL] = ode45(@get_EL, [0;tW(:,1)], [E_m, L_b], [], tf, tT, T_ref, T_A, v, g, E_m, L_m, p_Am); % EL: {J/cm^3, cm}, with {[E], L}
  EL(1,:)=[]; EWw_L = EL(:,2).^3 .* (1 + EL(:,1) * w_E/ mu_E/ d_E); % g, wet weight
  %
  tf = [0 3 6 9 12; fH_00 fH_03 fH_06 fH_09 fH_12]'; tT = temp.tW(:,[1 3]);
  [t, EL] = ode45(@get_EL, [0;tW(:,1)], [E_m, L_b], [], tf, tT, T_ref, T_A, v, g, E_m, L_m, p_Am); % EL: {J/cm^3, cm}, with {[E], L}
  EL(1,:)=[]; EWw_H = EL(:,2).^3 .* (1 + EL(:,1) * w_E/ mu_E/ d_E); % g, wet weight

  % pack to output
  prdData.tW = [EWw_L EWw_H];

end

function dEL = get_EL(t, EL, tf, tT, T_ref, T_A, v, g, E_m, L_m, p_Am)
  E = EL(1); % J/cm^3, reserve density [E}
  L = EL(2); % cm, structural length
  
  TC = tempcorr(C2K(spline1(t, tT)), T_ref, T_A); % -, temp correction factor at t
