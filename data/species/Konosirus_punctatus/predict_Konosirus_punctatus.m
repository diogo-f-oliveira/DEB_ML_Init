function [prdData, info] = predict_Konosirus_punctatus(par, data, auxData)
  ...
  tT_j = (tau_j - tau_b)/ kT_M;     % d, age at metam at f and T
  ...
  % time-length
  tvel = get_tj(pars_tj, f_tL1, [], tL1(:,1)*kT_M);
  ELw_1 = L_m * tvel(:,4)/ del_M; % cm, std length
  %
  tvel = get_tj(pars_tj, f_tL2, [], tL2(:,1)*kT_M);
  ELw_2 = L_m * tvel(:,4)/ del_M; % cm, std length
