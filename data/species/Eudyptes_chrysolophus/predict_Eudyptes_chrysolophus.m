function [prdData, info] = predict_Eudyptes_chrysolophus(par, data, auxData)
  ...
  TC_tJOe = tempcorr(temp.tJOe_A, T_ref, T_A); kT_M = TC * k_M;
  ...
  % uni-variate data
  % time-weight-energy dissipation
  tf = [10 f_10; 20 f_20; 40 f_40; 60 f_60]; tf(:,1) = tf(:,1)*kT_M;
  [tvel, tau_p, ~, lp, lb, info]  = get_tp(pars_tp, tf, [tau_b, f, l_b], tWpT(:,1)*kT_M); %state vars and states at b and p at T
  L = L_m * tvel(:,4);  e = tvel(:,3); EWw = L.^3 .* (1 + e * ome); % g, weight
  %
  r = TC * v * (e ./ L - 1/ L_m) ./ (e + g); 
  pC = (TC * v ./ L - r) * E_m .* e .* L.^3; 
  pM = TC * p_M * L.^3; pG = kap * pC - pM; pD = pM + (1 - kap) * pC;
  pA = TC * p_Am * spline1(tvel(:,1), tf) .* L.^2/ kap_X;
  ...
  % authors use the conversion: 1 1 0 2 = 20.083 kJ. pp. 98
  EpT_T = - J_M(3,:)' * 20.083/ X_gas; % kJ/d  

  % time-respiration
  % embryo data
  vT = v * TC_tJOe; kT_J = k_J * TC_tJOe; pT_M = p_M * TC_tJOe;
  U_E0 = initial_scaled_reserve(fA_e, pars_UE0)/ TC_tJOe;
  a = [0;tJOe_A(:,1)]; for i=2:length(a); if a(i)<=a(i-1); a(i)=a(i-1)+1e-3;end;end % d, ages
  ...
  %
  U_E0 = initial_scaled_reserve(fB_e, pars_UE0)/ TC_tJOe;
  a = [0;tJOe_B(:,1)]; for i=2:length(a); if a(i)<=a(i-1); a(i)=a(i-1)+1e-3;end;end % d, ages
  [a LUH] = ode45(@dget_LUH, a, [1e-10 U_E0 0], [], kap, vT, kT_J, g, L_m); 
  LUH(1,:) = []; L = LUH(:,1); L3 = L .^3; M_E = LUH(:,2) * J_E_Am; ee = mu_E * M_E ./ L3/ E_m;
  r = vT * (ee ./ L - 1/ L_m) ./ (ee + g); pC = (vT ./ L - r) .* M_E * mu_E; 
  pM = pT_M * L3; pG = kap * pC - pM; pD = pM + (1 - kap) * pC;
