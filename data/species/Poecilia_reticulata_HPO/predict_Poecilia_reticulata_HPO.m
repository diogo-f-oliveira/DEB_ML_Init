function [prdData, info] = predict_Poecilia_reticulata_HPO (par, data, auxData)
   ...
   vT = v * TC; kT_J = k_J * TC; kT_M = k_M * TC; hT_a = h_a * (TC^2); pT_Am = p_Am * TC; 
   ...
   f1 = kap_X * spline1(1000, tpX.tL_f1)/ pT_Am/ (3.42 * del_M)^2;  % -, scaled functional response
   f2 = kap_X2 * spline1(1000, tpX.tL_f2)/ pT_Am/ (2.84 * del_M)^2;  % -, scaled functional response

   % NB 3.42 = mean max length at high food, 2.84 is mean max length at low food
 ...
 % NB the development delay parameter t_0 is temperature dependent
  ...
  % O2 consumption
    ...
    JT_O = - J_M(3,:)* 32000/ 24;      % mg/h, O2 consumption
   ...
   prdData.JO_74 = JT_O;   % mg/ h, oxygen consumption (respiration)
