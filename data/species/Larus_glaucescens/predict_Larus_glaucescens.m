function [prdData, info] = predict_Larus_glaucescens(par, data, auxData)
  ...
  % time-weight 
  tf = [0 10 20 30 40; f1_00 f1_10 f1_20 f1_30 f1_40]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW1(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 10 20 30 40; f2_00 f2_10 f2_20 f2_30 f2_40]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW2(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tvel = get_tp(pars_tp, tf, [], tW3(:,1)*kT_M);
