function [prdData, info] = predict_Apeltes_quadracus(par, data, auxData)
  ...
  pars_tpm = [g_m; k; l_T; v_Hb; v_Hj; v_Hp]; % compose parameter vector
  ...
  % time-length
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tj(pars_tj, f_tL, [], tL_f(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length 
  % male
  tvel = get_tj(pars_tpm, f_tL, [t_b, f_tL*z/z_m, l_b*z/z_m], tL_m(:,1)*kT_M);
