function [prdData, info] = predict_Ammodytes_americanus(par, data, auxData)
...
% zero-variate data
  ...
  [~, ~, ~, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tL);
  L_b=L_m*l_b; L_j=L_m*l_j; L_i=L_m*l_i; r_j=rho_j*k_M; r_B=rho_B*k_M;
  [~, L] = ode45(@get_dL, [0;tL(:,1)],L_b,[],T_ref,T_A,t_0,L_j,L_i,r_j,r_B);
...
function dL = get_dL(t,L, T_ref,T_A,t_0,L_j,L_i,r_j,r_B)
  TC = tempcorr(C2K(11+10*sin(2*pi*(t-t_0)/365)), T_ref, T_A); 
  if L<L_j
    dL = TC * L * r_j/3;
  else
    dL = TC * r_B * (L_i - L);
