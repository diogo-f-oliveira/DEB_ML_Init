function [prdData, info] = predict_Euphlyctis_hexadactylus(par, data, auxData)
  ...
  EWw_f = (L_i - (L_i - L_j) * exp(- rT_B * tW_fm(:,1))).^3 * (1 + f_tW * w); % g, wet weight
  %
  rT_Bm = TC * k_M/ 3/ (1 + f_tW/ g_m); L_im = L_mm * f_tW; 
  EWw_m = (L_im - (L_im - L_j) * exp(- rT_Bm * tW_fm(:,1))).^3 * (1 + f_tW * w); % g, wet weight

  % length-weight
  EWw_Lf = (LW_f(:,1) * del_M).^3 * (1 + f_tW * w); % g, weight
  EWw_Lm = (LW_m(:,1) * del_M).^3 * (1 + f_tW * w_m); % g, weight
