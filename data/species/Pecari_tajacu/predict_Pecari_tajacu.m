function [prdData, info] = predict_Pecari_tajacu(par, data, auxData)
  ...
  % ultimate
  L_i = L_m * f;
  Ww_i = L_i^3 * (1 + f * w);       % g, ultimate body weight
  ...
  tT_pm = (t_pm - t_bm)/ k_M/ TC;        % d, time sice birth at puberty
  ...
  EWw = L.^3 * (1 + w * f)/ 1e3; % kg, total weight female
