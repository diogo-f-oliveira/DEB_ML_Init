function [prdData, info] = predict_Aeorestes_cinereus(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);
 
  if f_00 > 1.5 || f_10 > 1.5
  ...
  % puberty
  tT_R = (t_R - t_b)/ kT_M;         % d, time since birth at first litter
  ...
  % time-weight 
  tf = [0 f_00; 10 f_10; 20 f_20; 30 f_30; 50 f_50]; % scaled func response in time
  [t, Le] = ode45(@get_Le, [0;tWw_f(:,1)], [L_b; 1], [], tf, v * TC, g, L_m);
  Le(1,:) = []; EWw_f = Le(:,1).^3 .* (1 + Le(:,2) * w);  % g, wet weight
  %
  [t, Le] = ode45(@get_Le, [0;tWw_m(:,1)], [L_b; z/z_m], [], tf, v * TC, g_m, L_mm);
  Le(1,:) = []; EWw_m = Le(:,1).^3 .* (1 + Le(:,2) * w);  % g, wet weight
