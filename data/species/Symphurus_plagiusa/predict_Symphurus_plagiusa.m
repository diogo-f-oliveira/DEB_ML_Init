function [prdData, info] = predict_Symphurus_plagiusa(par, data, auxData) 
...
%% compute temperature correction factors
  ...
  RT_i = 365 * reprod_rate_j(L_i, f, pars_R); % #/d, max reprod rate
  ...
  % time - length
  tvel = get_tj(pars_tj,f_tL,[], tL_fm(:,1)*k_M);
  ELw_f = L_m * tvel(:,4)/ del_M; % cm, total length
  %
  tvel = get_tj(pars_tjm,f_tL,[0 z/z_m l_b*z/z_m], tL_fm(:,1)*k_M);
