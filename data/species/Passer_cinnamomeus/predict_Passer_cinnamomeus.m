function [prdData, info] = predict_Passer_cinnamomeus(par, data, auxData)
  ...
  % time-weight
  tf = [2 4 9 15; f_02 f_04 f_09 f_15]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [2 4 9 15; f1_02 f1_04 f1_09 f1_15]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
