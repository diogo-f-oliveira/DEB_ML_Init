function [prdData, info] = predict_Conger_conger(par, data, auxData)
...
%% pack to output
  ...
  L = L_i - (L_i - L_b) * exp( - TC_tL * r_B * tL(:,1)); % cm, struct length
  ELw = L/ del_M; % cm, total length
  
  L = L_i - (L_i - L_b) * exp( - TC_tW * r_B * tW(:,1)); % cm, struct length
