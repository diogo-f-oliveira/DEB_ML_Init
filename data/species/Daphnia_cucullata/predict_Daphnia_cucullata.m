function [prdData, info] = predict_Daphnia_cucullata(par, data, auxData)
  ...
  Wd_i = L_i^3 * d_V * (1 + f * w) * 1e6; % mg, ultimate dry weight 
  ...
  % food - length at puberty
  n = size(XLp,1);  ELw_p = zeros(n,1); 
  for i = 1:n
      x = XLp(i,1)/ 12e3/ K; f = x/ (1 + x);
      [lp, ~, info] = get_lp(pars_tp, f, l_b);
      if ~info; prdData = []; return;  end
      ELw_p(i) = L_m * lp/ del_M;
  end
