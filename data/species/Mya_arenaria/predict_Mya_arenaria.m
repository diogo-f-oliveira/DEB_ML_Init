function [prdData, info] = predict_Mya_arenaria(par, data, auxData)
  ...
  TC_tWR = tempcorr(temp.tWR, T_ref, T_A);
  ...
  Wd_0 = p_Am * U_E0 * w_E/ mu_E;   % g, initial dry weight
  ...
  EWd = [L_bj; L_jm].^3 * d_V * (1 + f * w); 
  %
  L_bj = L_b * exp(tWR((tWR(:,1) <= tT_j),1) * rT_j/ 3);
  L_jm = L_i - (L_i - L_j) * exp( - rT_B * (tWR((tWR(:,1) > tT_j),1) - tT_j)); % cm, expected length at time
  EWdR = 365 * TC_tWR * Wd_0 * reprod_rate_j([L_bj; L_jm], f, pars_R); % g, dry gonadic mass, accumulated in 365 d       
