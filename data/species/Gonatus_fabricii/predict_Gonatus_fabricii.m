function [prdData, info] = predict_Gonatus_fabricii(par, data, auxData)

  %par.E_Hj = par.E_Hp - 1e-3; % overwrite!
  ...
  % length-weight
  EWw_Lf = (LWw_f(:,1) * del_M).^3 * (1 + f_LW * w);   % g, wet weight
  EWw_Lm = (LWw_m(:,1) * del_M).^3 * (1 + f_LW * w_m); % g, wet weight
