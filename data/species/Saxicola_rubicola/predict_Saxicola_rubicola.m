function [prdData, info] = predict_Saxicola_rubicola(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par);
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  f_values = [f1_00, f1_03, f1_06, f1_09, f1_12, f1_30, f2_00, f2_03, f2_06, f2_09, f2_12, f2_15, f2_30];
  if any(f_values <= 0) 
  ...
  % time-weight
  tf = [0 f1_00; 3 f1_03; 6 f1_06; 9 f1_09; 12 f1_12; 30 f1_30]; tf(:,1) = tf(:,1) * kT_M;
  [tvel, ~, ~, ~, ~, info] = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  if ~info; prdData = []; return;  end      
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f2_00; 3 f2_03; 6 f2_06; 9 f2_09; 12 f2_12; 14 f2_15; 30 f2_30]; tf(:,1) = tf(:,1) * kT_M;
  [tvel, ~, ~, ~, ~, info] = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  if ~info; prdData = []; return;  end      
