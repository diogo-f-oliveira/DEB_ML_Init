function [prdData, info] = predict_Gymnocanthus_tricuspis(par, data, auxData)
  ...
  RT_i = TC * reprod_rate_j(L_i, f_tL, pars_R);          % #/d, max reproduction rate
  ...
  EWw_f = L.^3 .* (1 + ome * tvel(:,3));
  %
  tvel = get_tj(pars_tjm, f_tL, [], tL_fm(:,1)*kT_M);
  L = L_mm * tvel(:,4); ELw_m = L/ del_M;   % cm, length
  EWw_m = L.^3 .* (1 + ome_m * tvel(:,3));
  
  % length-weight
  EWw_Lf = (LW_f(:,1)*del_M).^3 * (1 + f_tL * ome);
  EWw_Lm = (LW_m(:,1)*del_M).^3 * (1 + f_tL * ome_m);
