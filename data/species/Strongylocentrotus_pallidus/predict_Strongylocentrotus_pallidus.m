function [prdData, info] = predict_Strongylocentrotus_pallidus(par, data, auxData)
  ...
  kT_M = k_M * TC_tL; rT_B = rho_B * kT_M; L_j = L_i * l_j; L_i = L_m * l_i; 
  L = L_i - (L_i - L_j) * exp( - rT_B * tL(:,1)); % cm, structural length at time
  ELw = L/ del_M;                                 % cm, test diameter
  
  % length - weight
  EWd = (LWd(:,1)*del_M).^3 * (1 + f_tL * ome) * d_V; % g, dry weight
