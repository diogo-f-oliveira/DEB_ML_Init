function [prdData, info] = predict_Macronectes_giganteus(par, data, auxData)
  ...
  tf = [0 70 110; f_000 f_070 f_110]'; tf(:,1) = tf(:,1) * kT_M;
  % female
  ...
  tf = [0 70 110; f1_000 f1_070 f1_110]'; tf(:,1) = tf(:,1) * kT_M;
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [tau_b 1 l_b], tW_f1(:,1)*kT_M);
  EWw_f1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  % male
  tvel = get_tp(pars_tpm, tf, [t_b, z/z_m, l_b*z/z_m], tW_m1(:,1)*kT_M);
  EWw_m1 = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome_m); % g, weight

  % pack to output
  prdData.tW_f = EWw_f;
  prdData.tW_m = EWw_m;
  prdData.tW_f1 = EWw_f1;
  prdData.tW_m1 = EWw_m1;
