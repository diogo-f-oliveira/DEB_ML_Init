function [prdData, info] = predict_Chroicocephalus_ridibundus(par, data, auxData)
  ...
  % uni-variate data 
  % time-weight
  tf = [0 5 10 15 20 30; f_00 f_05 f_10 f_15 f_20 f_30]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 5 10 15 20 30; f1_00 f1_05 f1_10 f1_15 f1_20 f1_30]'; tf(:,1) = tf(:,1) * kT_M;
