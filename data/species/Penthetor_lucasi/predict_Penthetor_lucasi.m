function [prdData, info] = predict_Penthetor_lucasi(par, data, auxData)
  ...
  % birth
  L_b = L_m * l_b;                  % cm, structural length at birth at f  Lw_b = L_b/ del_M;                % cm, forearm length at birth 
  ...
  % time-weight
  EWw_e = (TC * v * tWw_e(:,1)/ 3).^3 * (1 + f * w);
  %
  rT_B = TC * k_M/ 3/ (1 + f/ g); % 1/d, von Bert growth rate 
  L = L_i - (L_i - L_b) * exp(- rT_B * (tWw_f(:,1) - aT_g));;
  EWw_f = L.^3 * (1 + f * w);  % g, wet weight
  %
  L = L_i - (L_i - L_b) * exp(- rT_B * (tWw_m(:,1) - aT_g));;
