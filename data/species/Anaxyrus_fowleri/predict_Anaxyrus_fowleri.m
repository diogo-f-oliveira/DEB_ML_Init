function [prdData, info] = predict_Anaxyrus_fowleri(par, data, auxData)
  ...
  [t, L] = ode45(@get_L, tL_f(:,1), L_j, [], T_ref, T_A, t_0, L_i, r_B); 
  ELw_f = L/ del_M; % mm, tibia length
  %
  L_im = L_mm * f_tL; r_Bm = k_M/ 3/ (1 + f_tL/ g_m);
  [t, L] = ode45(@get_L, tL_m(:,1), L_j, [], T_ref, T_A, t_0, L_im, r_Bm);
  ELw_m = L/ del_M; % mm, tibia length
...
function dL = get_L(t, L, T_ref, T_A, t_0, L_i, r_B)
  TC = tempcorr(C2K(10 + 20*sin(2*pi*(t_0 + t)/365)), T_ref, T_A);
  dL = TC * r_B * (L_i - L);
end
