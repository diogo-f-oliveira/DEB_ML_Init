function [prdData, info] = predict_Eubranchipus_grubii(par, data, auxData)
...
%   Ww_im = L_im^3 * (1 + f * w_m);      % g, ultimate wet weight
  ...
  % time-length
  tf = [5 f0_05; 10 f0_10; 20 f0_20; 40 f0_40]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tj(pars_tj, tf, [], tL(:,1)*kT_M);
  ELw_0 = L_m * tvel(:,4)/ del_M;  % cm, total length
  %
  tf = [5 f1_05; 10 f1_10; 20 f1_20; 40 f1_40]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tj(pars_tj, tf, [], tL(:,1)*kT_M);
  ELw_1 = L_m * tvel(:,4)/ del_M;  % cm, total length

  % pack to output
  prdData.tL = [ELw_0 ELw_1];
