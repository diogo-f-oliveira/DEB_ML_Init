function [prdData, info] = predict_Halichoerus_grypus(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  % customized filters to contrain a parameter 
  filterChecks = f_tLW > 1 || ...         % f contrained to not be larger than 1
                 f_tLW < 0;   % 
...
%   prdData.Wwp = Ww_p;
  ...
  L = L_i - (L_i - L_b) * exp( - rT_B * tL(:,1)); % cm, stuctural length
  ...
  ELwm = L/ del_M; % cm, standard length
  L = L_i - (L_i - L_b) * exp( - rT_B * tWm(:,1)); % cm, structural length
  EWwm = L.^3 * (1 + w * f); % g, wet weights
