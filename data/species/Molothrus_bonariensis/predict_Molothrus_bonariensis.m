function [prdData, info] = predict_Molothrus_bonariensis(par, data, auxData)
  ...
  tf = [0 3 6 9 12; f_00 f_03 f_06 f_09 f_12]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  tf = [0 2 4 6 8; fz_00 fz_02 fz_04 fz_06 fz_08]'; tf(:,1) = tf(:,1) * kT_M;
  ...
  tf = [0 3 6 9; f_fm00 f_fm03 f_fm06 f_fm09]'; tf(:,1) = tf(:,1) * kT_M;
  % female
