function [prdData, info] = predict_Columbina_talpacoti(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  if f1_09<0; info = 0; prdData = []; end
  ...
  [tau_x, ~,l_x,~,info] = get_tp(pars_tx, f); % -, scaled time
  ...
  % uni-variate data
  % time-weight
  tf = [0 f1_00; 3 f1_03; 6 f1_06; 9 f1_09]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f_00; 3 f_03; 6 f_06; 9 f_09]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

  % pack to output
  prdData.tW = [EWw_1 EWw_2 EWw_2 EWw_2 EWw_2];
end
