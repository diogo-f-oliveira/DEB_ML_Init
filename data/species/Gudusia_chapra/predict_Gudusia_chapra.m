function [prdData, info] = predict_Gudusia_chapra(par, data, auxData)
  ...
  % time - weight
  tvel = get_tj(pars_tj, f_tL ,[tau_b 1 l_b], tW_fm(:,1)*kT_M);
  EWw_f = (L_m*tvel(:,4)).^3.*(1+tvel(:,3)*ome); % g, weight
  %
  tvel = get_tj(pars_tjm, f_tL ,[tau_b z/z_m l_b*z/z_m], tW_fm(:,1)*kT_M);
  ...
  % weight-fecundity
  EN_W = TC * 365 * reprod_rate_j((WN(:,1)/(1+ome)).^(1/3), f, pars_R); % #, yearly fecundity
