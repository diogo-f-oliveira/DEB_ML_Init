function [prdData, info] = predict_Acrobates_pygmaeus(par, data, auxData)
  ...
  % time-weight
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tW, [], (tWw_f(:,1)+t_tW)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male
  tvel = get_tp(pars_tpm, f_tW, [t_b, f_tW*z/z_m, l_b*z/z_m], (tWw_m(:,1)+t_tW)*kT_M);
