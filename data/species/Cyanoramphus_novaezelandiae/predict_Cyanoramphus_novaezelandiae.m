function [prdData, info] = predict_Cyanoramphus_novaezelandiae(par, data, auxData)
  ...
  % time-weight increment: varying food
  tf = [0 10 20; f1_00 f1_10 f1_20]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tdW_f(:,1)*kT_M);
  L = (L_m * tvel(:,4)); e = tvel(:,3); r = TC * v *(e./L - 1/L_m)./(e + g); f = spline1(tdW_f(:,1), tf); 
  de = (f - e) * v * TC ./ L; dL = L .* r/3;
  EdWw_f = (1 + ome * e) * 3 .* L.^2 .* dL + L.^3 * ome .* de;
  %
  tvel = get_tp(pars_tpm, tf, [tau_b 1 l_b], tdW_m(:,1)*kT_M);
  L = (L_mm * tvel(:,4)); e = tvel(:,3); r = TC * v *(e./L - 1/L_mm)./(e + g); f = spline1(tdW_m(:,1), tf); 
  de = (f - e) * v * TC ./ L; dL = L .* r/3;
  EdWw_m = (1 + ome_m * e) * 3 .* L.^2 .* dL + L.^3 * ome_m .* de;
  
  % time-weight
  tf = [0 10 20 30 40; f_00 f_10 f_20 f_30 f_40]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

  % pack to output
  prdData.tdW_f = EdWw_f;
  prdData.tdW_m = EdWw_m;
  prdData.tW = EWw;
