function [prdData, info] = predict_Epidalea_calamita(par, data, auxData)
  ...
  [t_j, t_b, l_j, l_b, info] = get_tp(pars_tj, f);
  ...
  Lw_j = L_j/ del_M;            % cm, body length at puberty at f
  ...
  % t-L data for larva
  L_i = f_tL * L_m; % cm, ultimate struc length
  r_B = k_M/ 3/ (1 + f_tL * w); % 1/d, von Bert growth rate at T_ref
  [t, L] = ode45(@Bert, [0; tL(:,1)], L_j, [], t_0, T_ref, T_A, r_B, L_i);
  L(1) = []; ELw = L/ del_M; % cm, expected length at time
  
  % pack to output
  prdData.tL = ELw;
end

function dL = Bert(t, L, t_0, T_ref, T_A, r_B, L_i)
  TC = tempcorr(C2K(7+10*sin(2*pi*(t-t_0)/365)), T_ref, T_A);
  dL = TC * r_B * (L_i - L);
end
