function [prdData, info] = predict_Ambystoma_maculatum(par, data, auxData)
...
% zero-variate data
  ...
  Lw_j = L_j/ del_M;                % cm, physical length at metam at 
  ...
  % SVL-SVL
  Lw_i = L_m * f_tL/ del_M;
  rT_B = TC_LL * k_M/ 3/ (1 + f_tL/ g); % 1/d, von Bert growth rate
  ELw_f =  Lw_i - (Lw_i - LL_f(:,1)) .* exp(-rT_B * time.LL_f); % cm, SVL 
  %
  Lw_im = L_mm * f_tL/ del_M;
  rT_Bm = TC_LL * k_M/ 3/ (1 + f_tL/ g_m); % 1/d, von Bert growth rate
  ELw_m =  Lw_im - (Lw_im - LL_m(:,1)) .* exp(-rT_Bm * time.LL_m); % cm, SVL 
