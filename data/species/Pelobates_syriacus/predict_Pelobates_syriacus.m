function [prdData, info] = predict_Pelobates_syriacus(par, data, auxData)
  ...
  pars_tjm = [g_m k l_T v_Hj v_Hpm];   % pars for toad
  [t_pm, t_jm, l_pm, l_jm] = get_tp(pars_tjm, f);
  tT_pm = (t_pm - t_jm)/ k_M/ TC;      % d, time since metam at puberty
  L_pm = l_pm * L_mm; Lw_pm = L_pm/ del_M; % cm, struc, SVL
  L_jm = l_jm * L_mm; Lw_jm = L_jm/ del_M; % cm, struc, SVL
  ...
  % time-length
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f, [t_b, f, l_j], tL_f(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length 
  % male
  tvel = get_tpm(pars_tpm, f, [t_b, f*z/z_m, l_jm], tL_m(:,1)*kT_M);
