function [prdData, info] = predict_Rangifer_tarandus(par, data, auxData)
  ...
  L = L_i - (L_i - L_b) * exp( - rT_B * tW_t(:,1) * 365); % cm, struct length
  EWw_t = L.^3 * (1 + w * f)/ 1e3; % kg, total weight female
  % 
  L = L_i - (L_i - L_b) * exp( - rT_B * tW_g(:,1) * 365); % cm, struct length
  EWw_g = L.^3 * (1 + f * w)/ 1e3; % kg, total weight male
