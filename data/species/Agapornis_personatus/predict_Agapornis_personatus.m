function [prdData, info] = predict_Agapornis_roseicollis(par, data, auxData)
  ...
  % time - O2 cons for embryo
  f = f_e; U_E0 = initial_scaled_reserve(f, pars_UE0)/ TC_ab;
  vT = v * TC_ab; kT_J = k_J * TC_ab; pT_M = p_M * TC_ab; 
  %
  a=[1e-10;tJOe(:,1)-t_0]; n=length(a); for i=2:n; if a(i)<=a(i-1); a(i)=a(i-1)+1e-8; end; end % d, ages
  ...
  EJ_Oe = - J_M(:,3) * 24.4e3; % ml/d, O2 consumption
  
  % time-weight for embryo
  a=[1e-10;tWe(:,1)-t_0]; n=length(a); for i=2:n; if a(i)<=a(i-1); a(i)=a(i-1)+1e-8; end; end % d, ages
  [a LUH] = ode45(@dget_LUH, a, [1e-10 U_E0 0], [], kap, vT, kT_J, g, L_m); 
  LUH(1,:) = []; L = LUH(:,1); L3 = L .^3; M_E = LUH(:,2) * J_E_Am; ee = mu_E * M_E ./ L3/ E_m;
  EWw_e = L.^2 * (1 + w * f_e);
