function [prdData, info] = predict_Dendrochirus_brachypterus(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  % Check food level is greater than 0
  f_values = [f_080, f_150, f_200, f_300];
  if any(f_values < 0)
...
%   pars_tjm = [g_m k l_T v_Hb v_Hj v_Hp];
  ...
  % time-length
  tf = [80 f_080; 150 f_150; 200 f_200; 300 f_300]; tf(:,1) = tf(:,1) * kT_M;
  [tvel, ~, ~, ~, ~, ~, ~, ~, ~, ~, info] = get_tj(pars_tj, tf, [], tW(:,1)*kT_M);
  if ~info
      prdData = []; return
  end
  EWw = (L_m * tvel(:,4)).^3 .* (1 + ome*tvel(:,2)); % g, weight
