function [prdData, info] = predict_Phoeniconaias_minor(par, data, auxData)
  ...
  % time-weight 
  tel_0 = [0, f_tW, (W_0/(1+f_tW*ome))^(1/3)/L_m];
  tvel = get_tp(pars_tp, f_tW, tel_0, [0;tW(:,1)*kT_M]);
  EWw = (L_m * tvel(2:end,4)).^3 .* (1 + tvel(2:end,3) * ome);   % g, weight
