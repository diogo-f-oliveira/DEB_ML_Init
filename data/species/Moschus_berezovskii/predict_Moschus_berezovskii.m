function [prdData, info] = predict_Moschus_berezovskii(par, data, auxData)
  ...
  % life cycle
  pars_tx = [g k l_T v_Hb v_Hx v_Hp s_F];  % intermediate
...
%   tT_pm = (t_pm - t_bm)/ kT_M;         % d, time since birth at pubert
  ...
  tf = [50 f_bx; 100 f_xi]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tx(pars_tx, tf, [], tW(:,1) * kT_M); L = L_m * tvel(:,4);
  EWw = L.^3 .* (1 + tvel(:,3) * ome)/1e3; % kg, wet weight
