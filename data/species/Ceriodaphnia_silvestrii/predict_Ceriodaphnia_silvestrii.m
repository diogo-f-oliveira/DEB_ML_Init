function [prdData, info] = predict_Ceriodaphnia_silvestrii(par, data, auxData)
  ...
  [EN L] = cum_reprod(tLR(:,1), f_tL, pars_R);                % #/d, ultimate reproduction rate at T
  ELw = L/del_M; 
  
  % length-dry weight
  EWd = 1e6*(LWd(:,1)*del_M).^3*(1+f_tL*ome)*d_V; % mug, dry weight 
