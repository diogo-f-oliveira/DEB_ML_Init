function [prdData, info] = predict_Stenella_longirostris(par, data, auxData)
  ...
  % length-weight
  EWw_f = (LW_f(:,1) * del_M).^3 * (1 + f * w)/1e3; % kg, weight
  EWw_m = (LW_m(:,1) * del_M).^3 * (1 + f * w_m)/1e3; % kg, weight
