function [prdData, info] = predict_Lopholatilus_chamaeleonticeps(par, data, auxData)
...
% zero-variate data
  ...
  Lw_im = f * L_mm/ del_M;             % cm, max fork length
  ...
  % length-weight
  EWw_f = (LW_f(:,1) * del_M).^3 * (1 + f_tL * w); % g, wet weight
  EWw_m = (LW_m(:,1) * del_M).^3 * (1 + f_tL * w_m); % g, wet weight
