function [prdData, info] = predict_Crocodylus_moreletii(par, data, auxData)
...
% zero-variate data
  ...
  ELw = Lw_i - (Lw_i - Lw_b(:,1)) .* exp( - rT_B * tL(:,1) * 365); % cm, total length at time
  % time-weight
  L = L_i - (L_i - L_b(:,1)) .* exp( - rT_B * tW(:,1) * 365);     % cm, structural length at time
