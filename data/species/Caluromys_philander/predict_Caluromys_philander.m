function [prdData, info] = predict_Caluromys_philander(par, data, auxData)
  ...
  % ingestion
  pT_Xm = TC * p_Am * (300/(1 + f * w))^(2/3)/ kap_X; % J/d, caloric intake at 300 g 
  ...
  L = L_i - (L_i - L_b) * exp( - TC * r_B * tL(:,1)); % cm, structural length
  ELw = L/ del_M; % g, length for females
 
  % time-weight
  L = L_i - (L_i - L_b) * exp( - TC * r_B * tW(:,1)); % cm, structural length
  EWw = L.^3 * (1 + f * w); % kg, weight for females
