function [prdData, info] = predict_Centrocercus_urophasianus(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  if E_Hx < 0 || f_010 > 1.2 || f_010 > 1.2 || f_030 > 1.2 || f_030 > 1.2
  ...
  % time-weight
  tf = [10 f_010; 20 f_020; 30 f_030; 60 f_060; 100 f_100; 140 f_140; 180 f_180]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tvel = get_tp(pars_tpm, tf, [], tW_m(:,1)*kT_M);
