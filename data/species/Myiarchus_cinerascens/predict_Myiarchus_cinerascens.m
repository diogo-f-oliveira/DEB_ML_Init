function [prdData, info] = predict_Myiarchus_cinerascens(par, data, auxData)
  ...
  % time-weight
  tf = [5 f_05; 10 f_10; 15 f_15; 20 f_20]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tWp(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

  % time-FMR
  e = tvel(:,3); L = tvel(:,4) * L_m;
  r = TC * v * (e ./ L - 1/ L_m) ./ (e + g); % 1/d, specific growth rate
  p_C = L.^3 * E_m .* e .* (TC * v ./ L - r); % J/d, mobilization power
  p_J = TC * k_J * tvel(:,1) * (1 - kap) * g * E_m * L_m^3; p_R = (1 - kap) * p_C - p_J; % maturation/reproduction
  p_D = TC * p_M * L.^3 + p_J + p_R * (1 - kap_R); % J/d, dissipation
  p_A = TC * p_Am * spline1(kT_M*tWp(:,1),tf) .* L.^2; % J/d, assimilation
  FMR = (p_D + p_A * ((1-kap_P)/kap_X - 1))/ 1e3; % kJ/d, field metabolic rate
