function [prdData, info] = predict_Sardinella_brasiliensis(par, data, auxData)
  ...
  % time - rel change in length post metam
  ...
  L = L_i - (L_i - L_j) * exp( - rT_B * (tdL(:,1)*365 - tT_j)); % cm, struct length
  EdLL = RMI * rT_B * 365 * (L_i./ L - 1);
