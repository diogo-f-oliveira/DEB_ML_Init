function [prdData, info] = predict_Liopholis_inornata(par, data, auxData)

% unpack par, data, auxData
cPar = parscomp_st(par); vars_pull(par);
vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

% this line is outcommented by Bas Kooijman
%k_J = k_M; % no data to estimate k_J so assume it is k_M
...
o2Ww=12.236; % mean wet weight of experimental animals
...
L = (o2Ww / (1 + f * w)).^(1/3); % cm, structural length, using 239 g wet mass
