function [prdData, info] = predict_Ailuropoda_melanoleuca(par, data, auxData)
  ...
  l = (l_i - (l_i - l_b) * exp( - rT_B * 365));     % -, scaled structural length at time
  Ww_1 = (L_m * l)^3 * (1 + f * w);                 % g, weight at 1 yr
  ...
  % time-weight 
  l = l_i - (l_i - l_b) * exp( - rT_B * tW(:,1));  % -, scaled structural length at time
  EWw = (L_m * l).^3 * (1 + f * w);                % g, wet weight
