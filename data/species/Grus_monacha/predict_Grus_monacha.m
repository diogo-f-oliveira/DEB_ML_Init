function [prdData, info] = predict_Grus_monacha(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);
  
  if f_03 < 0.1; info = 0; prdData = []; return; end
  ...
  % uni-variate data
  % time-weight  
  tf = [3 5 7 10 20 30; f_03 f_05 f_07 f_10 f_20 f_30]'; tf(:,1) = tf(:,1) * kT_M;
