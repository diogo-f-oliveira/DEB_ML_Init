function [prdData, info] = predict_Alca_torda(par, data, auxData)
  ...
  % time-weight: varying food 81
  tf = [11  f_81; 15 f]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW81(:,1)*kT_M);
  EWw_81 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

  % time-weight: varying food 82
  tf = [6 f; 11  f_82; 15 f]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW82(:,1)*kT_M);
  EWw_82 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
