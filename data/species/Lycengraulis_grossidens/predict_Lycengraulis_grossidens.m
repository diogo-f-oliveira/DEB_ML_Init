function [prdData, info] = predict_Lycengraulis_grossidens(par, data, auxData)
...
%% compute temperature correction factors
  ...
  Ww_p = L_p^3 * (f * ome);         % g, weight at puberty
  ...
  ELw_f = L_m*tvel(:,4)/ del_M; % cm, total lengths
  %
  tvel = get_tj(pars_tjm, f_tL, [0 z/z_m l_b*z/z_m], tL_fm(:,1)*kT_M);
  ELw_m = L_mm*tvel(:,4)/ del_M; % catenate lengths
