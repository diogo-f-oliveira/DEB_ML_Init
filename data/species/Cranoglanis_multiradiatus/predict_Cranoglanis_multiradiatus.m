function [prdData, info] = predict_Cranoglanis_multiradiatus(par, data, auxData)
  ...
  % time-length
  tf = [800 f_0800; 1600 f_1600]; tf(:,1)=tf(:,1)*kT_M;
  %
  ...
  % weight-fecundity
  EN_W = TC * 365 * reprod_rate_j((WN(:,1)/(1+ome)).^(1/3), f, pars_R); % #, fecundity
