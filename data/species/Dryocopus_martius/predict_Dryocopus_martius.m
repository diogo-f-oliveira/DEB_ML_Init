function [prdData, info] = predict_Dryocopus_martius(par, data, auxData)
  ...
  % uni-variate data
  % time-weight 
  tf = [0 5 10 15 20 25 30 35 40; f1_00 f1_05 f1_10 f1_15 f1_20 f1_25 f1_30 f1_35 f1_40]'; tf(:,1) = tf(:,1) * kT_M;
  l_0 = (W_01/(1 + f1_00 * ome))^(1/3)/ L_m; 
  tvel = get_tp(pars_tp, tf, [0 f1_00 l_0], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 5 10 15 20 25 30 35 40; f2_00 f2_05 f2_10 f2_15 f2_20 f2_25 f2_30 f2_35 f2_40]'; tf(:,1) = tf(:,1) * kT_M;
  l_0 = (W_02/(1 + f2_00 * ome))^(1/3)/ L_m;
  tvel = get_tp(pars_tp, tf, [0 f2_00 l_0], tW(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % pack to output
  prdData.tW = [EWw EWw_1];
