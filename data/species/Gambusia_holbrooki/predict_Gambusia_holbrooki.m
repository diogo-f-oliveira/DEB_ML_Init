function [prdData, info] = predict_Gambusia_holbrooki(par, data, auxData)
  ...
  % time-length
  % female
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, f_tL, [t_b, f_tL, L_0*del_M/L_m], tL_f(:,1)*k_M*TC_tL);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length
  % male
  tvel = get_tj(pars_tjm, f_tL, [t_b, f_tL, L_0m*del_M/L_mm], tL_m(:,1)*k_M*TC_tL);
