function [prdData, info] = predict_Arctocephalus_tropicalis(par, data, auxData)
  ...
  [t_pm, t_xm, t_bm, l_pm, l_xm, l_bm, info] = get_tx_old(pars_txm, f);
  if info == 0
    prdData = []; return;
  end
  ...
  % males have the same parameters as females, before puberty
  L = L_i - (L_i - L_b) * exp( - TC * rT_B * tW_m(:,1)); % cm, structural length
  EWw_m = L.^3 * (1 + f * w)/ 1e3; % kg, weight for females
