function [prdData, info] = predict_Trachinus_radiatus(par, data, auxData)
  ...
  % time-length 
  [tvel,~,~,~,~,~,l_b] = get_tj(pars_tj, f_tL, [], tL_f(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M; % cm, total length
  %
  tvel = get_tj(pars_tjm, f_tL, [0 1 l_b*z/z_m], tL_m(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M; % cm, total length
 
  % time-weight 
  [tvel,~,~,~,~,~,l_b] = get_tj(pars_tj, f_tL, [], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3.*(1 + ome * tvel(:,3)); % g, weight
  %
  tvel = get_tj(pars_tjm, f_tL, [0 1 l_b*z/z_m], tW_m(:,1)*kT_M);
