function [prdData, info] = predict_Spermophilus_citellus(par, data, auxData)
  ...
  % weight at 30 d since birth
  rT_B = TC * k_M/ 3/ (1 + f/g);
  L_30 = L_i - (L_i - L_b) * exp(-rT_B * 30);
  Ww_30 = L_30^3 * (1 + f * w);     % g, weight at 30 d since birth
  ...
  % time-weight
  [t, EL] = ode45(@get_EL, tWw_f(:,1), [E_m, L_b], [], f_m, f_a, v * TC, g, E_m, L_m, p_Am * TC); % EL: {J/cm^3, cm}, with {[E], L}
  EWw_f = EL(:,2).^3 .* (1 + EL(:,1) * w_E/ mu_E/ d_E); % g, wet weight
  %
  [t, EL] = ode45(@get_EL, tWw_m(:,1), [E_m, L_b], [], f_m, f_a, v * TC, g_m, E_m_m, L_mm, p_Am_m * TC); % EL: {J/cm^3, cm}, with {[E], L}
...
function dEL = get_EL(t, EL, f_m, f_a, v, g, E_m, L_m, p_Am)
  E = EL(1); % J/cm^3, reserve density [E}
  L = EL(2); % cm, structural length
  
  f = min(1,f_m+f_a*cos(2*pi*(t-200)/365)); % -, scaled functional response at t
