function [prdData, info] = predict_Cynoscion_nothus(par, data, auxData)
...
%   L_pm = L_mm * l_p; Lw_pm = L_pm/ del_M; % cm, total length at puberty
  ...
  ELw = L_m * tvel(:,4)/ del_Mb;   % cm, length
  %
  tf = [1000 f_1000; 2000 f_2000; 3000 f_3000; 4000 f_4000]; tf(:,1) = tf(:,1)*kT_M;
  tvel = get_tj(pars_tj, tf, [], tL1(:,1)*kT_M);
