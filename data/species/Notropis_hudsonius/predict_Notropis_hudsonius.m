function [prdData, info] = predict_Notropis_hudsonius(par, data, auxData)
...
%% compute temperature correction factors
  ...
  L = L_i - (L_i - L_j) * exp( - rT_B * (tL_f(:,1) - tT_j)); % cm, struc length
  ELw_f = L/ del_M;  % for females
  ...
  L = L_im - (L_im - L_jm) * exp( - rT_Bm * (tL_m(:,1) - tT_jm)); % cm, struc length
  ELw_m = L/ del_M;  % for males
