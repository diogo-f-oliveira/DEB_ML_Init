function [prdData, info] = predict_Melospiza_melodia(par, data, auxData)
  ...
  tf = [0 13 25 26 60; f_00 f_13 f_25 f_26 f_60]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % time-weight: varying food and temperature
  tf = [0 2 4 6 8 10; f1_00 f1_02 f1_04 f1_06 f1_08 f1_10]'; 
  [t, EL] = ode45(@get_ELT, tW1(:,1), [E_m, L_b], [], tf, temp.tW1, T_A, T_ref, v, g, E_m, L_m, p_Am); % EL: {J/cm^3, cm}, with {[E], L}
...
function dEL = get_ELT(t, EL, tf, tT, T_A, T_ref, v, g, E_m, L_m, p_Am)
