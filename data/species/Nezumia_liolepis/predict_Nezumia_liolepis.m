function [prdData, info] = predict_Nezumia_liolepis(par, data, auxData)
  ...
  Ww_b = L_b^3 * (1 + f * w);       % g, wet weight length at birth at f
  ...
  ELw = L_m * tvel(:,4)/ del_Ma; % cm, total length
  
  % length-length
  ELw_L = LL(:,1)*del_Ma/del_M; % cm, total length

  % length-weight
  EWw = (LW(:,1)*del_Ma).^3*(1+f*ome); % g, weight
