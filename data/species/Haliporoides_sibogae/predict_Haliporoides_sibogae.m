function [prdData, info] = predict_Haliporoides_sibogae(par, data, auxData)
  ...
  L = L_i - (L_i - Lw_f0 * del_M) * exp( - rT_B * tL_f(:,1)); % cm, expected length at time
  tLw_f = L/ del_M;
  % males
  [t_jm, t_pm, t_bm, l_jm, l_pm, l_bm, l_im, rho_jm, rho_Bm] = get_tj(pars_tjm, f);
  rT_Bm = TC * rho_Bm * k_M; L_im = L_mm * l_im;
  L = L_i - (L_i - Lw_m0 * del_M) * exp( - rT_Bm * tL_m(:,1)); % cm, expected length at time
  tLw_m = L/ del_M;
