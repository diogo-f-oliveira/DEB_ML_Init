function [prdData, info] = predict_Vultur_gryphus(par, data, auxData)
  ...
  Ww_0 = w_E * p_Am * U_E0/ mu_E/ d_E; % g, egg wet weight
  ...
  % time-weight
  tf = [0 30 99; f_00 f_30 f_99]'; tf(:,1) = tf(:,1) * kT_M;
  % female
  [tvel_f, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [tau_b 1 l_b], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel_f(:,4)).^3 .* (1 + tvel_f(:,3) * ome);   % g, weight
  % male
  tvel_m = get_tp(pars_tpm, tf, [t_b, z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
  EWw_m = (L_mm * tvel_m(:,4)).^3 .* (1 + tvel_m(:,3) * ome_m); % g, weight
  
  % time-food intake
  %f = spline1(tW_f(:,1),tf); % -, scaled functional response
  EJX_f = (L_m * tvel_f(:,4)).^2 * TC .* f * p_Am/ kap_X/ mu_X * w_X/ d_X;   % g/d, food intake females
  EJX_m = (L_mm * tvel_m(:,4)).^2 * TC .* f * p_Am_m/ kap_X/ mu_X * w_X/ d_X; % g/d, food intake males
