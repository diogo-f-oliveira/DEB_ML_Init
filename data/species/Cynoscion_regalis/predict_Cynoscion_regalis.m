function [prdData, info] = predict_Cynoscion_regalis(par, data, auxData)
  ...
  Ww_i = L_i^3 * (1 + f * w);       % g, ulrimate wet weigh
  ...
  [t_j, t_p, t_b, l_j, l_p, l_b, ~, rho_j, rho_B] = get_tj(pars_tjm, f);
  ...
  % time-length
  % tf = [1700 f_1700; 2200 f_2200]; tf(:,1)=tf(:,1)*kT_M;
  % female
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, f_tL, [], tL(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length
  % male
  tvel = get_tj(pars_tjm, f_tL, [t_b, z/z_m, l_b*z/z_m], tL(:,1)*kT_M);
