function [prdData, info] = predict_Selar_crumenophthalmus(par, data, auxData)
...
%% compute temperature correction factors
  ...
  TC_tLj = tempcorr(temp.tLj, T_ref, T_A);
...
%   Ww_im = L_im^3 * (1 + f * ome_m);      % g, ultimate wet weight
  ...
  %
  tvel = get_tj(pars_tj, f, [], tLj(:,1)*k_M*TC_tLj);
  ELw_j = L_m*tvel(:,4)/ del_M; % cm, total length
  
  % length-weight
  EWw_j = (LWj(:,1)*del_M).^3*(1+ome); % g, weight
