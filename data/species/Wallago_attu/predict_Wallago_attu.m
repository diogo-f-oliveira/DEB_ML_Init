function [prdData, info] = predict_Wallago_attu(par, data, auxData)
...
%% compute temperature correction factors
  ...
  % time-length
  tvel = get_tj(pars_tj, f_tL, [], tL(:,1)*kT_M); % -, scaled length at birth at f
  ELw = L_m * tvel(:,4)/ del_M; % cm, total length
  
  % length-weight
  EWw = (del_M * LW(:,1)).^3 * (1 + ome * f); % g, wet weight
