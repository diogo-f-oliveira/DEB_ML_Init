function [prdData, info] = predict_Melursus_ursinus(par, data, auxData)
  ...
  L_i = f_tW * L_m; L_b = get_lb([g; k; v_Hb], f_tW) * L_m; % cm, length at birth, ultimate
  L = L_i - (L_i - L_b) * exp( - rT_B * tW_1(:,1));       % cm, structural length at time
  EWw_1 = L.^3 * (1 + f_tW * w);                          % g, wet weight
  %
  L = L_i - (L_i - L_b) * exp( - rT_B * tW_2(:,1));      % cm, structural length at time
  EWw_2 = L.^3 * (1 + f_tW * w);                         % g, wet weight
