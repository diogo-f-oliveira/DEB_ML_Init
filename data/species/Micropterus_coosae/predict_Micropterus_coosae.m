function [prdData, info] = predict_Micropterus_coosae(par, data, auxData)
...
% zero-variate data
  ...
  RT_i = TC_tL1 * reprod_rate_j((98/(1 + f_tL1 * ome)).^(1/3), f_tL1, pars_R); % #/d, reprod rate at 98 g
  ...
  kT_M = k_M * TC_tL1; rT_j = rho_j * kT_M; rT_B = rho_B * kT_M;        
  L_b = L_m * l_b;  L_j = L_m * l_j; L_i = L_m * l_i; tT_j = (t_j - t_b)/ kT_M;
  L = L_b * exp(tL1(:,1) * rT_j/ 3) .* (tL1(:,1) < tT_j);
  L = L + (tL1(:,1) >= tT_j) .* (L_i - (L_i - L_j) * exp( - rT_B * (tL1(:,1) - tT_j))); % cm, expected length at time
  ELw_1 = L/ del_M;                              % cm, total length

  % length - weight
  EWw = (LWw(:,1) * del_M).^3 * (1 + f_tL1 * ome); % g, wet weight
