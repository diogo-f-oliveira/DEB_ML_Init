function [prdData, info] = predict_Ciconia_abdimii(par, data, auxData)
  ...
  % time-weight at varing food
  tf = [0 15 30 45 60 75 90; f_00 f_15 f_30 f_45 f_60 f_75 f_90]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % time-energy intake
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tpX(:,1)*kT_M);
  EpX = (L_m * tvel(:,4)).^2 * TC .* spline1(tpX(:,1),tf) * p_Am/ kap_X/ 1e3; % kJ/d, energy intake
