function [prdData, info] = predict_Synchiropus_phaeton(par, data, auxData)
  ...
  EWw_f = L.^3.*(1+tvel(:,3)*ome); % g, weight
  %
  tvel = get_tj(pars_tjm, f_tL ,[0 1 l_b*z/z_m], tL_fm(:,1)*kT_M);
  L=L_mm*tvel(:,4); ELw_m = L/ del_M; % cm, total length 
  EWw_m = L.^3.*(1+tvel(:,3)*ome_m); % g, weight
  
  % length-weight
  EWw_fL = (LW_f(:,1)*del_M).^3*(1+f_tL*ome); % g, weight
  EWw_mL = (LW_m(:,1)*del_M).^3*(1+f_tL*ome_m); % g, weight
