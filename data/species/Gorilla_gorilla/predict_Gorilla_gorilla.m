function [prdData, info] = predict_Gorilla_gorilla(par, data, auxData)
  ...
  L_im = (f - l_T) * L_mm; Ww_im = L_im^3 * (1 + f * ome_m);
  L_bm = L_mm * get_lb([g_m k v_Hb], f); % cm, struct length at birth
  ...
  % univariate data
  % time-weight
  f = f_tW; L_im = f * L_mm; L_bm = L_mm * get_lb([g k v_Hb], f); % cm, struc length
  rT_Bm = TC * k_M/ 3/ (1 + f/ g_m); % 1/d, von Bert growth rate
  L = L_im - (L_im - L_bm) * exp(-rT_Bm * tW(:,1)); % cm, structural length
  EWw = L.^3 * (1 + f * ome_m); % g, weit weight
