function [prdData, info] = predict_Sepia_apama(par, data, auxData)
  ...
  % time-weight
  tvel = get_tj(pars_tj, f_tWw, [], tWw(:,1) * kT_M);
  EWw = (L_m * tvel(:,4)).^3 * (1 + f_tWw * ome);   % g, wet weight
  
  % length-weight
  EWw_L = (LWw(:,1) * del_M).^3 * (1 + f_tWw * w); % g, wet weight 
