function [prdData, info] = predict_Osteogeneiosus_militaris(par, data, auxData)
  ...
  %
  tvel = get_tj(pars_tjm,f_tL,[tau_b z/z_m l_b*z/z_m], tL_fm(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M; % cm, total length

  % length-weight
  EWw_L = (LW(:,1)*del_M).^3*(1+f_tL*ome); % g, weight
  
  % length-fecundity
  EN_L = TC * 365 * reprod_rate_j(LN(:,1)*del_M, f_tL, pars_R); % #, yearly fecundity

  % weight-fecundity
  L = (WN(:,1)/ (1 + f_tL * ome)).^(1/3);
  EN_W = TC * 365 * reprod_rate_j(L, f_tL, pars_R); % #, yearly fecundity
