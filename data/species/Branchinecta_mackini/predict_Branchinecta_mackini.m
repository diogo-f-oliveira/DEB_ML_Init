function [prdData, info] = predict_Branchinecta_mackini(par, data, auxData)
  ...
  aT_b = tau_b/ k_M;                 % d, age at birth at f and T_ref
  ...
  % time-length
  tTC = [0 3; 10 5; 30 20; 60 25];  tTC(:,2) = tempcorr(C2K(tTC(:,2)), T_ref, T_A);
  options = odeset('Events',@eoa);
  tf = [0 f70_00; 20 f70_20; 40 f70_40; 60 f70_60]; 
  [t, HeL] = ode45(@get_HeL, [0;tL_70(:,1)], [E_Hb, 1, L_b], options, tf, tTC, v, g, E_m, L_b, L_m, kap, k_J, E_Hj);
  HeL(1,:)=[]; ELw_70f = HeL(:,3)/ del_M;  % cm, total length
  %
  [t, HeL] = ode45(@get_HeL, [0;tL_70(:,1)], [E_Hb, 1, L_b], options, tf, tTC, v, g_m, E_m_m, L_b, L_mm, kap, k_J, E_Hj);
  HeL(1,:)=[]; ELw_70m = HeL(:,3)/ del_M;  % cm, total length

  tTC = [0 3; 10 5; 20 20; 60 25];  tTC(:,2) = tempcorr(C2K(tTC(:,2)), T_ref, T_A);
  tf = [0 f71_00; 20 f71_20; 40 f71_40; 60 f71_60];
  [t, HeL] = ode45(@get_HeL, [0;tL_71(:,1)], [E_Hb, 1, L_b], options, tf, tTC, v, g, E_m, L_b, L_m, kap, k_J, E_Hj);
  HeL(1,:)=[]; ELw_71f = HeL(:,3)/ del_M;  % cm, total length
  %
  [t, HeL] = ode45(@get_HeL, [0;tL_71(:,1)], [E_Hb, 1, L_b], options, tf, tTC, v, g_m, E_m_m, L_b, L_mm, kap, k_J, E_Hj);
  HeL(1,:)=[]; ELw_71m = HeL(:,3)/ del_M;  % cm, total length

  tTC = [0 3; 10 5; 30 20; 60 25];  tTC(:,2) = tempcorr(C2K(tTC(:,2)), T_ref, T_A);
  tf = [0 f72_00; 20 f72_20; 40 f72_40; 60 f72_60];
  [t, HeL] = ode45(@get_HeL, [0;tL_72(:,1)], [E_Hb, 1, L_b], options, tf, tTC, v, g, E_m, L_b, L_m, kap, k_J, E_Hj);
  HeL(1,:)=[]; ELw_72f = HeL(:,3)/ del_M;  % cm, total length
  %
  [t, HeL] = ode45(@get_HeL, [0;tL_72(:,1)], [E_Hb, 1, L_b], options, tf, tTC, v, g_m, E_m_m, L_b, L_mm, kap, k_J, E_Hj);
  HeL(1,:)=[]; ELw_72m = HeL(:,3)/ del_M;  % cm, total length

  % pack to output
  prdData.tL_70 = [ELw_70f ELw_70m];
  prdData.tL_71 = [ELw_71f ELw_71m];  
  prdData.tL_72 = [ELw_72f ELw_72m];
end
