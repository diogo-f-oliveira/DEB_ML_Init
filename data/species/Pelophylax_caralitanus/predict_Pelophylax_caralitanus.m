function [prdData, info] = predict_Pelophylax_caralitanus(par, data, auxData)
  ...
  [t_pm, t_jm, l_pm, l_jm, info] = get_tp(pars_tpm, f);
  ...
  % univariate data
  % time-length
  L_b = L_m * get_lb([g; k; v_Hb], f_tL); L_i = L_m * f_tL; % cm, struc length
  rT_B = TC * k_M/ 3/ (1 + f_tL/ g); % 1/d, von Bert growth rate
  ELw_f = (L_i - (L_i - L_b) * exp(- rT_B * tL_fm(:,1)))/ del_M; % cm, SVL
  %
  L_bm = L_m * get_lb([g_m; k; v_Hb], f_tL); L_im = L_mm * f_tL; % cm, struc length
