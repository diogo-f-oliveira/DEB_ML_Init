function [prdData, info] = predict_Zhangixalus_omeimontis(par, data, auxData)
  ...
  % univariate data
  % time-length
  L_b = L_m * get_lb([g; k; v_Hb], f_H); L_i = L_m * f_H;
  rT_B = TC_H * k_M/ 3/ (1 + f_H/ g); % 1/d, von Bert growth rate
  L = L_i - (L_i - L_b) * exp(- rT_B * tL_Hfm(:,1)); % cm, struc length
  ELw_Hf = L/ del_M; % cm, SVL
  %
  L_bm = L_mm * get_lb([g_m; k; v_Hb], f_H); L_im = L_mm * f_H;
  rT_Bm = TC_H * k_M/ 3/ (1 + f_H/ g_m); % 1/d, von Bert growth rate
  L = L_im - (L_im - L_bm) * exp(- rT_B * tL_Hfm(:,1)); % cm, struc length
  ELw_Hm = L/ del_M; % cm, SVL
  %
  L_b = L_m * get_lb([g; k; v_Hb], f_M); L_i = L_m * f_M;
  rT_B = TC_M * k_M/ 3/ (1 + f_M/ g); % 1/d, von Bert growth rate
  L = L_i - (L_i - L_b) * exp(- rT_B * tL_Mfm(:,1)); % cm, struc length
  ELw_Mf = L/ del_M; % cm, SVL
  %
  L_bm = L_mm * get_lb([g_m; k; v_Hb], f_M); L_im = L_mm * f_M;
  rT_Bm = TC_M * k_M/ 3/ (1 + f_M/ g_m); % 1/d, von Bert growth rate
  L = L_im - (L_im - L_bm) * exp(- rT_B * tL_Mfm(:,1)); % cm, struc length
  ELw_Mm = L/ del_M; % cm, SVL
  %
  L_b = L_m * get_lb([g; k; v_Hb], f_L); L_i = L_m * f_L;
  rT_B = TC_L * k_M/ 3/ (1 + f_L/ g); % 1/d, von Bert growth rate
  L = L_i - (L_i - L_b) * exp(- rT_B * tL_Lfm(:,1)); % cm, struc length
  ELw_Lf = L/ del_M; % cm, SVL
  %
  L_bm = L_mm * get_lb([g_m; k; v_Hb], f_L); L_im = L_mm * f_L;
  rT_Bm = TC_L * k_M/ 3/ (1 + f_L/ g_m); % 1/d, von Bert growth rate
  L = L_im - (L_im - L_bm) * exp(- rT_B * tL_Lfm(:,1)); % cm, struc length
