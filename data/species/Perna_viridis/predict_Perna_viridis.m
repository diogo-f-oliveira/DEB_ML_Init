function [prdData, info] = predict_Perna_viridis(par, data, auxData)
  ...
  % time-length
  t = tL(:,1) - t_0; % d, time since birth
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tL);
  kT_M = k_M * TC_tL; rT_B = rho_B * kT_M; L_j = L_m * l_j; L_i = L_m * l_i; tT_j = (t_j - t_b)/ kT_M; 
  L = L_i - (L_i - L_j) * exp( - rT_B * (t - tT_j)); % cm, expected length at time
  ELw = L/del_M;                                     % cm, shell length
%
  % time-length (Cheng2016)
  t = tL2(:,1) - t_0; % d, time since birth
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tL2);
  kT_M = k_M * TC_tL2; rT_B = rho_B * kT_M; L_j = L_m * l_j; L_i = L_m * l_i; tT_j = (t_j - t_b)/ kT_M; 
  L = L_i - (L_i - L_j) * exp( - rT_B * (t - tT_j)); % cm, expected length at time
  ELw_2 = L/del_M;                                     % cm, shell length
%   
  % time-weight 
  t = tW(:,1) - t_0; % d, time since birth
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tW);
  kT_M = k_M * TC_tW; rT_B = rho_B * kT_M; L_j = L_m * l_j; L_i = L_m * l_i; tT_j = (t_j - t_b)/ kT_M; 
  L = L_i - (L_i - L_j) * exp( - rT_B * (t - tT_j)); % cm, expected length at time
  EWd = L.^3 * d_V * (1 + f * w);  % g, dry weight                             % g, wet weight
  
  % length-weight 
%   EWd = (LW(:,1) * del_M).^3 * d_V * (1 + f_tW * w); % g, dry weight   
  
  % length-weight (Cheng2016)
  EWw = (LW2(:,1) * del_M).^3 * (1 + f_tL2 * w); % g, wet weight from Cheng2016
