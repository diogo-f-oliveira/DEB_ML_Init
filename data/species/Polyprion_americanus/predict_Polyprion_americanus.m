function [prdData, info] = predict_Polyprion_americanus(par, data, auxData)
...
% zero-variate data
  ...
  tLw_f = [L_bj; L_ji]/ del_M;
  % males
  L_bj = L_b * exp(tL_m(tL_m(:,1) < tT_j,1) * rT_j/ 3); % cm, struc length
  L_ji = L_i - (L_i - L_j) * exp( - rT_B * (tL_m(tL_m(:,1) >= tT_j,1) - tT_j)); % cm, struc length
  tLw_m = [L_bj; L_ji]/ del_M;
