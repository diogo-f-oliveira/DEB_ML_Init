function [prdData, info] = predict_Paralichthys_orbignyanus(par, data, auxData)
  ...
  ELw_f = L_m*tvel(:,4)/ del_M; % cm, total length at time
  %
  tvel = get_tj(pars_tjm, f_tL, [], tL_fm(:,1)*kT_M);
  ELw_m = L_mm*tvel(:,4)/ del_M; % cm, total length at time
