function [prdData, info] = predict_Pagothenia_borchgrevinki(par, data, auxData)
...
%   pars_tjm = [g_m k l_T v_Hb v_Hj v_Hp];
  ...
  % time-length
  % female
  tvel = get_tj(pars_tj, f_tL, [0 1 L_0*del_M/L_m], tL(:,1)*kT_M);
  ELw = L_m * tvel(:,4)/ del_M;   % cm, length

  %time-weight
  tvel = get_tj(pars_tj, f_tL, [0, 1, (W_0/(1+ome))^(1/3)/L_m], tW(:,1)*kT_M);
