function [prdData, info] = predict_Emberiza_citrinella(par, data, auxData)
  ...
  % uni-variate data
  % time-weight
  tf = [3 f_03; 6 f_06; 9 f_09]; 
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW_B(:,1)*kT_M);
  EWw_B = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW_N(:,1)*kT_M);
  EWw_N = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
