function [prdData, info] = predict_Cebus_albifrons(par, data, auxData)
  ...
  % univariate data
  % time-change in weight
  L_b = L_m * get_lb([g k v_Hb], f_tdW); L_i = L_m * f_tdW;
  rT_B = kT_M/ 3/ (1 + f_tdW/ g); % 1/d, von Bert growth rate
  L = L_i - (L_i - L_b) * exp(-rT_B * tdWw(:,1)); % cm, struc length
  dL = rT_B * (L_i - L);
  EdWw = 3 * L.^2 .* dL * (1 + f_tdW * ome); % g/d, change inweight
