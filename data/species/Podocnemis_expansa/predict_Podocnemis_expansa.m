function [prdData, info] = predict_Podocnemis_expansa(par, data, auxData)
  ...
  TC_tLe = tempcorr(temp.tLe_CL, T_ref, T_A);
  ...
  % time-embryo length
  vT = v * TC_tLe; kT_J = TC_tLe * k_J;
  [a LUH] = ode45(@dget_LUH, [tLe_CRL(:,1); tLe_CL(:,1)], [1e-10 U_E0 0], [], kap, vT, kT_J, g, L_m); 
  ELw_ecr = LUH(1:size(tLe_CRL,1),1)/ del_MCRL; ELw_ec = LUH(1+size(tLe_CRL,1):end,1)/ del_MCL;
  
  % time-length/weight
  rT_B = TC * k_M/ 3/ (1 + f_tL/ g); L_b = L_m * get_lb([g k v_Hb], f_tL); L_i = L_m * f_tL;
  ELw = (L_i - (L_i - L_b) * exp(-rT_B * tL(:,1)))/ del_M; % cm, carapace length
  
  % pack to output
  prdData.tLe_CRL = ELw_ecr;
  prdData.tLe_CL = ELw_ec;
  prdData.tL = ELw;
