function [prdData, info] = predict_Catharus_ustulatus(par, data, auxData)
  ...
  % uni-variate data  
  % time-weight
  tf = [0 f_00; 2 f_02; 8 f_08; 10 f_10]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f1_00; 2 f1_02; 8 f1_08; 10 f1_10]; tf(:,1) = tf(:,1) * kT_M;
