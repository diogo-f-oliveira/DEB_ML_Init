function [prdData, info] = predict_Upeneus_pori(par, data, auxData)
...
% zero-variate data
  ...
  pars_tjm = [g_m k l_T v_Hb v_Hp];
  [t_p, t_b, l_p, l_b] = get_tp(pars_tjm, f);
  ...
  %
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tpm, f_tL, [0 z/z_m l_b*z/z_m], tL_fm(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M;   % cm, length 

  % length-weight
  EWw_f = (LW_fm(:,1)*del_M).^3*(1+f_tL*ome);
  EWw_m = (LW_fm(:,1)*del_M).^3*(1+f_tL*ome_m);
 
%% pack to output
  prdData.tL_fm = [ELw_f ELw_m];
  prdData.LW_fm = [EWw_f EWw_m];
