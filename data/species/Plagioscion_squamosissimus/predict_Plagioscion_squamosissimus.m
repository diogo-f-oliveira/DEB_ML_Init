function [prdData, info] = predict_Plagioscion_squamosissimus(par, data, auxData)
  ...
  L_pm = l_pm * L_mm; Lw_pm = L_pm/ del_M; % cn, struc, total length at puberty
  ...
  tvel = get_tj(pars_tj, tf ,[tau_b 1 l_b], tL_f(:,1)*kT_M);
  ELw_f = L_m*tvel(:,4)/ del_M; % cm, total length 
  %
  tvel = get_tj(pars_tjm, tf ,[tau_b z/z_m l_b*z/z_m], tL_m(:,1)*kT_M);
  ELw_m = L_mm*tvel(:,4)/ del_M; % cm, total length 

  % length-weight
  EWw_f = (LW_f(:,1)*del_M).^3*(1+f_1500*ome); % g, weight
  EWw_m = (LW_m(:,1)*del_M).^3*(1+f_1500*ome_m); % g, weight
