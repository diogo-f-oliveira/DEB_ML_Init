function [prdData, info] = predict_Boulengerula_taitana(par, data, auxData)
  ...
  % length-weight
  LW_f = (LW_f(:,1) * del_M).^3 * (1 + f_tL * w);  % g, wet weight 
  LW_m = (LW_m(:,1) * del_M).^3 * (1 + f_tL * w_m);  % g, wet weight 
