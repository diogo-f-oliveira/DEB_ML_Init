function [prdData, info] = predict_Alosa_pseudoharengus(par, data, auxData)
...
%% compute temperature correction factors
  ...
  L_pm = L_mm * l_pm; Lw_pm = L_pm/ del_M; % cm, fork length at puberty
  ...
  ELw_m = L_mm * tvel(:,4)/ del_M;   % cm, length  ELw_m = [L_bj; L_ji]/ del_M; % cm, fork length
  
  % length - weight
  EWw_f = (LWw_f(:,1) * del_M) .^ 3 * (1 + f_tL * w);   % g, wet weight females
  EWw_m = (LWw_m(:,1) * del_M) .^ 3 * (1 + f_tL * w_m); % g, wet weight males
