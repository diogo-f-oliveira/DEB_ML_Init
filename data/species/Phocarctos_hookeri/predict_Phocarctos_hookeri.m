function [prdData, info] = predict_Phocarctos_hookeri(par, data, auxData)
  ...
  L = L_i - (L_i - L_b) * exp( - TC * rT_B * tWw_f(:,1)); % cm, structural length
  EWw = L.^3 * (1 + f * w); % g, weight 
  %
  L = L_i - (L_i - L_b) * exp( - TC * rT_B * tL_f(:,1)); % cm, structural length
  ELw = L./ del_M; % kg, weight 
