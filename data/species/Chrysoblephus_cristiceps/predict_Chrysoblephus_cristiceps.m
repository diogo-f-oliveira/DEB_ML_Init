function [prdData, info] = predict_Chrysoblephus_cristiceps(par, data, auxData)
...
%   pars_tjm = [g_m k l_T v_Hb v_Hj v_Hp];
  ...
  % time-length 
  tvel = get_tj(pars_tj, f_T, [], tL_TS(:,1)*kT_M);
  ELw_T = L_m * tvel(:,4)/ del_M; 
  %
  tvel = get_tj(pars_tj, f_S, [], tL_TS(:,1)*kT_M);
  ELw_S = L_m * tvel(:,4)/ del_M; 

  % pack to output
  prdData.tL_TS = [ELw_T ELw_S];
