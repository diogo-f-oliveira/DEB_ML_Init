function [prdData, info] = predict_Sufflogobius_bibarbatus(par, data, auxData)
...
% zero-variate data
  ...
  RT_i = TC * reprod_rate(L_i, f, pars_R);    % #, fecundity at T
  ...
  % time-length
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tL, [], tL_sm(:,1)*kT_M);
  ELw_sm = L_m * tvel(:,4)/ del_M;   % cm, length 
  % male
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tL, [0 z/z_m l_b*z/z_m], tL_tm(:,1)*kT_M);
  ELw_tm = L_mm * tvel(:,4)/ del_M;   % cm, length 
