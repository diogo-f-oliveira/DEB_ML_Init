function [prdData, info] = predict_Bufo_gargarizans(par, data, auxData)
...
% zero-variate data
  ...
  % univariate data
  % time - length
  rT_B = TC_fm * k_M/ 3/ (1 + f_tL/ g); L_i = f_tL * L_m; L_b = L_m * get_lb([g; k; v_Hb], f_tL); 
  L = L_i - (L_i - L_b) * exp(- rT_B * tL_fm(:,1)); ELw_f = L/ del_M; EWw_f = L.^3 * (1 + f_tL * w);
  %
  rT_Bm = TC_fm * k_M/ 3/ (1 + f_tL/ g_m); L_im = f_tL * L_mm; L_bm = L_mm * get_lb([g_m; k; v_Hb], f_tL); 
  L = L_im - (L_im - L_bm) * exp(- rT_Bm * tL_fm(:,1)); ELw_m = L/ del_M; EWw_m = L.^3 * (1 + f_tL * w_m);
