function [prdData, info] = predict_Engraulis_anchoita(par, data, auxData)
...
%% compute temperature correction factors
  ...
  kT_M = TC_ab * k_M; rT_B = rho_B * kT_M; rT_j = rho_j * kT_M;              % 1/d, von Bert, exponential growth rate
  tT_j = (t_j - t_b)/ kT_M; L_b = l_b * L_m; L_j = l_j * L_m; L_i = l_i * L_m;
  ...
  %
  kT_M = TC * k_M; rT_B = rho_B * kT_M; rT_j = rho_j * kT_M;              % 1/d, von Bert, exponential growth rate
  L_bj = L_b * exp(tL1(tL1(:,1) < tT_j, 1) * rT_j/3); % cm, struc length
  L_ji = L_i - (L_i - L_j) * exp( - rT_B * (tL1(tL1(:,1) >= tT_j,1) - tT_j)); % cm, expected length at time
  ELw_1 = [L_bj; L_ji]/ del_M; % catenate lengths
