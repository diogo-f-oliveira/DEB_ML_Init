function [prdData, info] = predict_Atheresthes_evermanni(par, data, auxData)
  ...
  % time-length
  % female
  [tvel, ~, ~, ~, ~, ~, ~, ~, ~, ~, info] = get_tj(pars_tj, f_tL, [], tL_f(:,1)*kT_M);
  if ~info
      prdData = []; return
  end
  ...
  % male
  [tvel, ~, ~, ~, ~, ~, ~, ~, ~, ~, info] = get_tj(pars_tjm, f_tL, [tau_b, f_tL*z/z_m, l_b*z/z_m], tL_m(:,1)*kT_M);
  if ~info
      prdData = []; return
  end
