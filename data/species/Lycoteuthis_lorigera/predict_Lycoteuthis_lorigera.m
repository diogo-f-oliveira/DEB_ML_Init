function [prdData, info] = predict_Lycoteuthis_lorigera(par, data, auxData)
  ...
  % length=weight
  EWw_Lf = (LWw_f(:,1)*del_Mform).^3 * (1 + f * w); % g, wet weight
  EWw_Lm = (LWw_m(:,1)*del_Mform).^3 * (1 + f * w_m); % g, wet weight
