function [prdData, info] = predict_Micropteropus_pusillus(par, data, auxData)
  ...
  pars_tpm = [g_m k l_T v_Hb v_Hpm];% use g, not g_m, since till puberty males are identical to females
  [t_pm, t_bm, l_pm, l_bm] = get_tp(pars_tpm, f, [t_b, f*z/z_m, l_b*z/z_m]);
  L_bm = l_bm * L_mm;                  % cm, struc length at birth
  L_im = f * L_mm; Ww_im = L_im^3 * (1 + f * w_m); % g, ultimate weight
