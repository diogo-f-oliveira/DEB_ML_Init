function [prdData, info] = predict_Donacobius_atricapilla(par, data, auxData)
  ...
  % uni-variate data
  % time-weight
  tf = [0 f2_00; 3 f2_03; 12 f2_12; 15 f2_15]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW2(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f3_00; 3 f3_03; 12 f3_12; 15 f3_15]; tf(:,1) = tf(:,1) * kT_M;
