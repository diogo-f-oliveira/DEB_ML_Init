function [prdData, info] = predict_Psammomys_obesus(par, data, auxData)
  ...
  % time-weight 
  f = f_tW; L_0 = (Ww_0/ (1 + f * w))^(1/3);           % cm, intial struc length
  ...
  %
  f = f_tW1; L_0 = (Ww_0/ (1 + f * w))^(1/3);          % cm, intial struc length
  rT_B = TC * k_M/ 3 / (1 + f/ g);                     % 1/d, von Bert growth rate
  L_i = f * L_m - L_T;                                 % cm, ultimate struc length 
