function [prdData, info] = predict_Sphyraena_sphyraena(par, data, auxData)
...
% zero-variate data
  ...
  RT_i = TC *  reprod_rate_j(40*del_M, f, pars_R); % 1/d,  reprod rate at 40 cm
...
%   Ww_pm = L_pm.^3 * (1 + f * w_m);     % g, wet weight at puberty
  ...
  % time-length-weight
  tf = [1000 f_1000; 2500 f_2500]; tf(:,1) = tf(:,1)*kT_M;
