function [prdData, info] = predict_Cisticola_aridulus(par, data, auxData)
  ...
  % uni-variate data
  % time-weight 
  tf = [1 3 9 12; f1_01 f1_03 f1_09 f1_12]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [1 3 9 12; f2_01 f2_03 f2_09 f2_12]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % pack to output
  prdData.tW = [EWw_1 .1+EWw_2 EWw_2];
