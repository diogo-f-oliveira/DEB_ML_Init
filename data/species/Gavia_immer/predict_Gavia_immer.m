function [prdData, info] = predict_Gavia_immer(par, data, auxData)
  ...
  % time-weight 
  tf = [0 f1_00; 15 f1_15; 70 f1_70; 90 f1_90]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f_00; 15 f_15; 70 f_70; 90 f_90]; tf(:,1) = tf(:,1) * kT_M;
  % female
