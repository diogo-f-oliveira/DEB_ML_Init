function [prdData, info] = predict_Furcifer_pardalis(par, data, auxData)
  ...
  rT_B = TC * k_M/ 3/ (1 + f_tL/ g); % 1/d, 1von Bert growth rate
  Et_f = max(0.01,-1+ceil(log((L_i - L_b)./max(.05,L_i - Lt_f(:,1)*del_M))/ rT_B/ 365)); % #, LAG
  %
  L_bm = L_mm * get_lb([g_m; k; v_Hb], f_tL); L_im = L_mm * f_tL;
  rT_Bm = TC * k_M/ 3/ (1 + f_tL/ g_m); % 1/d, 1von Bert growth rate
  Et_m = max(0.01,-1+ceil(log((L_im - L_bm)./max(.05,L_im - Lt_m(:,1)*del_M))/ rT_Bm/ 365)); % #, LAG
