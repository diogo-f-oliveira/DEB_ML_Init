function [prdData, info] = predict_Macaca_fascicularis(par, data, auxData)
  ...
  L_bm = l_bm * L_mm;                  % cm, structural length at birth
  ...
  % time-weight
  % female for Thailand
  [tvel, t_p, t_b, l_p, l_b, info] = get_tp(pars_tp, f_tW, [], tW_f(:,1)*kT_M);
  if ~info; prdData = []; return; end
  ...
  % female for Signapore
  [tvel, t_p, t_b, l_p, l_b, info] = get_tp(pars_tp, f_tW1, [], tW1_f(:,1)*kT_M);
  if ~info; prdData = []; return; end
  EWw_f1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male for Signapore
  tvel = get_tp(pars_tpm, f_tW1, [t_b, f_tW1*z/z_m, l_b*z/z_m], tW1_m(:,1)*kT_M);
  EWw_m1 = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * w_m); % g, weight

  % pack to output
  prdData.tW1_f = EWw_f1;
  prdData.tW1_m = EWw_m1;
  prdData.tW_f = EWw_f;
  prdData.tW_m = EWw_m;
