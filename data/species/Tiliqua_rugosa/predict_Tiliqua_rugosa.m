function [prdData, info] = predict_Tiliqua_rugosa(par, data, auxData)
  ...
  TC    = tempcorr(temp.ap, T_ref, pars_T); 
  ...
  L = (138.3/ d_V/ (1 + f * w)).^(1/3); % cm, structural length, using dry mass for 461 g wet mass
  ...
  EO = (- 1 * (JM(:,3) * X_gas) .* tempcorr(C2K(TO(:,1)), T_ref, pars_T))/461/24*1000;
