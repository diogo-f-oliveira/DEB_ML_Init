function [prdData, info] = predict_Gekko_gecko(par, data, auxData)
...
%   Ww_im = L_im^3 * (1 + f * w_m);       % g, ultimate wet weight
  ...
  ELw_f = (L_i - (L_i - L_b) * exp( - rT_B * tL_f(:,1)))/ del_M; % cm, SVL
  ELw_m = (L_i - (L_i - L_b) * exp( - rT_B * tL_m(:,1)))/ del_M; % cm, SVL
  % time-weight
  EWw_f = (L_i - (L_i - L_b) * exp( - rT_B * tW_f(:,1))).^3 * (1 + f_tL * ome); % g, weight
  EWw_m = (L_i - (L_i - L_b) * exp( - rT_B * tW_m(:,1))).^3 * (1 + f_tL * ome); % g, weight
