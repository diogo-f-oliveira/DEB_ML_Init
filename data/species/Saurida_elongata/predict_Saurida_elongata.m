function [prdData, info] = predict_Saurida_elongata(par, data, auxData) 
...
%% univariate data
  ...
  %
  tvel = get_tj(pars_tj, f_tLfm, [], tL_fm(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length
  %
  tvel = get_tj(pars_tjm, f_tLfm, [0 f_tL*z/z_m l_b*z/z_m], tL_fm(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M;   % cm, length

  % length-weight
  EWw = (LW(:,1)*del_M).^3 * (1+f_tLfm * ome); % g, weight 
