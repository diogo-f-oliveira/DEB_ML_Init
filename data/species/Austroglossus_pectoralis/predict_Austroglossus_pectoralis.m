function [prdData, info] = predict_Austroglossus_pectoralis(par, data, auxData) 
...
%% uni-variate data
  ...
  % time-weight
  % female
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, f_tL, [], tW_fm(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + ome*tvel(:,3)); % g, weight 
  % male
  tvel = get_tj(pars_tjm, f_tL, [t_b, f_tL*z/z_m, l_b*z/z_m], tW_fm(:,1)*kT_M);
