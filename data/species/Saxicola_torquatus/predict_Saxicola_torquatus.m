function [prdData, info] = predict_Saxicola_torquatus(par, data, auxData)
  ...
  % time-weight
  tf = [1 5 9 13 29; f_01 f_05 f_09 f_13 f_29]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW_a(:,1)*kT_M);
  EWw_a = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
