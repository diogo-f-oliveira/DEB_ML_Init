function [prdData, info] = predict_Psammobates_oculiferus(par, data, auxData)
  ...
  %
  L = L_i - (L_i - L_b) * exp (- rT_B * tL_m(:,1)); % cm, structural length
  ...
  EWw_f = L.^3 * (1 + f_tL * w);  
  %
  L= L_i - (L_i - L_b) * exp (- rT_B * tWw_m(:,1)); % cm, structural length
  EWw_m = L.^3 * (1 + f_tL * w);  
