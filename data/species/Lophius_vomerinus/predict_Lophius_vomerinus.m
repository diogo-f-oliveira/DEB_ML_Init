function [prdData, info] = predict_Lophius_vomerinus(par, data, auxData)
...
%% compute temperature correction factors
  ...
  tj = (t_j - t_b)/ kT_M; % time since birth at metam
  t_bj = tL(tL(:,1) < tj,1);  % select times between birth & metamorphosis
  EL_bj = Lw_b * exp(t_bj * rT_j/3); % exponential growth as V1-morph
  t_ji = tL(tL(:,1) >= tj,1); % select times after metamorphosis
  EL_ji = Lw_i - (Lw_i - Lw_j) * exp( - rT_B * (t_ji - tj)); % cm, expected length at time
