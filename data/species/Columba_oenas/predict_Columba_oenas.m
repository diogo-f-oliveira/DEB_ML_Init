function [prdData, info] = predict_Columba_oenas(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  if p_M > 9e3; info = 0; prdData = []; return; end
  ...
  % time-weight
  tf = [8 10 12 14 16 18; f_08 f_10 f_12 f_14 f_16 f_18]'; tf(:,1) = tf(:,1) * kT_M;
