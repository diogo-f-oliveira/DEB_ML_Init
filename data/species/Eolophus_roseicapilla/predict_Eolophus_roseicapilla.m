function [prdData, info] = predict_Eolophus_roseicapilla(par, data, auxData)
  ...
  tf = [5 10 30 40 50 90; f_05 f_10 f_30 f_40 f_50 f_90]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [5 10 30 40; f1_05 f1_10 f1_30 f1_40]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
