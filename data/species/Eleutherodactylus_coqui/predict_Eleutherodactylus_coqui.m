function [prdData, info] = predict_Eleutherodactylus_coqui(par, data, auxData)
  ...
  rT_B = TC * k_M/ 3/ (1 + f/ g);
  EdLw_f = 1e4 * rT_B * (Lw_i - LdL_f(:,1)); % mum/d, carapace length
  %
  rT_Bm = TC * k_M/ 3/ (1 + f/ g_m); 
  EdLw_m = 1e4 * rT_Bm * (Lw_im - LdL_m(:,1)); % mum/d, carapace length
