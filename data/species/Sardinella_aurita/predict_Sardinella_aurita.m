function [prdData, info] = predict_Sardinella_aurita(par, data, auxData)
  ...
  tj = (t_j - t_b)/ kT_M; % time since birth at metamorphosis
  ...
  t_ji = tL(tL(:,1) >= tj,1); % selects times after metamorphosis
  ...
  % weight=fecundity
  EN = TC * 365 * reprod_rate_j((WN(:,1)/ (1 + f * w)).^(1/3), f, pars_R); % #, eggs
