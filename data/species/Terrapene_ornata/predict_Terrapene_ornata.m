function [prdData, info] = predict_Terrapene_ornata(par, data, auxData)
  ...
  Ww_i = L_i^3 * (1 + f * w);       % g, ulitmate wet weight
  ...
  L_pm = L_mm * l_pm; Lw_pm = L_pm/ del_M; % cm, carapace length at puberty
  L_bm = L_mm * l_bm;    % cm, carapace length at birth
  ...
  % time-length
  %tf = [0 1500 4000 12000; f_0 f_0 f_i f_i]';
  tf = [0 2000 4000 12000; f_0 f_2 f_i f_i]';
  [t, EL] = ode45(@get_EL, tL_f(:,1), [E_m * f_0, L_b], [], tf, v * TC, g, E_m, L_m, p_Am * TC); % EL: {J/cm^3, cm}, with {[E], L}
  ELw_f = EL(:,2)/ del_M; % cm, carapace length
  %
  [t, EL] = ode45(@get_EL, tL_m(:,1), [E_m_m * f_0, L_bm], [], tf, v * TC, g_m, E_m_m, L_mm, p_Am_m * TC); % EL: {J/cm^3, cm}, with {[E], L}
  ELw_m = EL(:,2)/ del_M; % cm, carapace length
