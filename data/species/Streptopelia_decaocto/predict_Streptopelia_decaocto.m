function [prdData, info] = predict_Streptopelia_decaocto(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  if p_M > 1e4; info = 0; prdData = []; return; end
  ...
  % uni-variate data
  % time-weight
  tf = [0 5 10 20; f_00 f_05 f_10 f_20]'; tf(:,1) = tf(:,1) * kT_M;
