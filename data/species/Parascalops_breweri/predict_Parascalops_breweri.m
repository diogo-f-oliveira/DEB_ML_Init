function [prdData, info] = predict_Parascalops_breweri(par, data, auxData)
  ...
  % time-weight
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tWw, [t_b, f_tWw, (Ww_0f/(1+f_tWw*w))^(1/3)/ L_m], tWw_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male
  tvel = get_tp(pars_tpm, f_tWw, [t_b, f_tWw*z/z_m, (Ww_0m/(1+f_tWw*w_m))^(1/3)/L_mm], tWw_m(:,1)*kT_M);
