function [prdData, info] = predict_Branta_canadensis(par, data, auxData) 
 ...
 %% zero-variate data
  ...
  % time-weight
  % female
  [tvel, tau_p, tau_b, l_p, l_b] = get_tp(pars_tp, f_tW, [], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  % male
  tvel = get_tp(pars_tpm, f_tW, [tau_b, f_tW*z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
