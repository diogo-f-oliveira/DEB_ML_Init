...
function [prdData, info] = predict_Anas_acuta(par, data, auxData)

% unpack par, data, auxData
cPar = parscomp_st(par); vars_pull(par);
vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

%if p_M > 7e3; info = 0; prdData = []; return; end
...
% uni-variate data
  ...
  % male
  tvel = get_tp(pars_tpm, f_tW, [t_b, f*z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
