function [prdData, info] = predict_Dasyprocta_leporina(par, data, auxData)
  ...
  % foetal development
  E_m     = p_Am/ v_e;           % J/cm^3, [E_m], reserve capacity 
  g       = E_G/ kap/ E_m ;      % -, energy investment ratio
  v_Hb    = V_Hb * g^2 * k_M^3/ v_e^2; % -, scaled maturity density 
  aT_g = t_0 + get_tb_foetus([g, k, v_Hb])/ k_M/ TC; % time at birth
  %
  EWw_e = (1 + f * w) * (TC * v_e * max(0, tWwe(:,1) - t_0)/ 3) .^ 3;

  % pack to output
  prdData.tg = aT_g; % overwrite due to deviating v for foetus
  prdData.tWw = EWw;
  prdData.tWwe = EWw_e;
