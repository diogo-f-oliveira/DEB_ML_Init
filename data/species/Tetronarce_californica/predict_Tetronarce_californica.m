function [prdData, info] = predict_Tetronarce_californica(par, data, auxData)  
  ...
  % reproduction
  pars_R = [kap; kap_R; g; k_J; k_M; L_T; v; U_Hb; U_Hpm]; % compose parameter vector at T
  RT_i = TC * reprod_rate((100 * del_M), f, pars_R);   % #/d, reproduction rate at TL 100 cm
  ...
  pars_tpm = [g_m k l_T v_Hb v_Hpm]; % pack parameters
  ...
  L_im = L_mm * f; Lw_im = L_im/ del_M; % cm, ultimate total length
  Ww_im = L_im^3 * (1 + f * w_m);      % g, wet weight at puberty
