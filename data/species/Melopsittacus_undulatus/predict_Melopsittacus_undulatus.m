function [prdData, info] = predict_Melopsittacus_undulatus(par, data, auxData)
  ...
  tf = [10 20 30; f_10 f_20 f_30]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % time - O2 cons for embryo
  UT_E0 = pars_UE0/ TC_ab;
  vT = v * TC_ab; kT_J = k_J * TC_ab; pT_M = p_M * TC_ab; 
  %
  a = max(1e-3,tJOe(:,1)); n=length(a); for i=2:n; if a(i)<=a(i-1); a(i)=a(i-1)+1e-3; end;end 
  [a LUH] = ode45(@dget_LUH, [0;a], [1e-10 U_E0 0], [], kap, vT, kT_J, g, L_m); 
  LUH(1,:) = [];
