function [prdData, info] = predict_Alectoris_graeca(par, data, auxData)
  ...
  % time-weight
  tf = [10 f_10; 20 f_20; 60 f_60]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tWJX(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

  % time-feeding rate
  EJX = (L_m * tvel(:,4)).^2 .* spline1(tWJX(:,1),tf) * TC * p_Am/ kap_X/ mu_X * w_X; % g/d, dry food intake
  
  % pack to output
  prdData.tWJX = [EWw EJX];
end
