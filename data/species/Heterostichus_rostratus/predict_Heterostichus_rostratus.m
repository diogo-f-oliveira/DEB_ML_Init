function [prdData, info] = predict_Heterostichus_rostratus(par, data, auxData)
  ...
  %
  tvel = get_tj(pars_tj, f_tLj, [], tLj(:,1)*kT_M);
  ELw_j = L_m * tvel(:,4)/ del_M; % cm, total length
  %
  tvel = get_tj(pars_tj, f_tLfm, [], tL_f(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M; % cm, total length
  %
  tvel = get_tj(pars_tjm, f_tLfm, [0 z/z_m l_b*z/z_m], tL_m(:,1)*kT_M);
