function [prdData, info] = predict_Eopsetta_jordani(par, data, auxData)
  ...
  f = f_tL; [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tL);
  ...
  tf = [0 8000; f_tL f_tL1]';
  [t, EL] = ode45(@get_EL, [0; tL_f(:,1) - tT_j - t_0], [E_m, L_j], [], tf, s_M * v * TC, g, E_m, s_M * L_m, s_M * p_Am * TC); % EL: {J/cm^3, cm}, with {[E], L}
  EL(1,:) = []; ELw_f= EL(:,2)/ del_M; % cm, total length
  % males
  [t, EL] = ode45(@get_EL, [0; tL_m(:,1) - tT_jm - t_0], [E_mm, L_jm], [], tf, s_M * v * TC, g_m, E_mm, s_M * L_mm, s_M * p_Am_m * TC); % EL: {J/cm^3, cm}, with {[E], L}
  EL(1,:) = []; ELw_m= EL(:,2)/ del_M; % cm, total length
