function [prdData, info] = predict_Anguilla_anguilla(par, data, auxData)
...
%% compute temperature correction factors
  ...
  L = L_i - (L_i - L_b) * exp( - TC_tL * r_B * (tL(:,1) + t_sj)); % cm, expected physical length at time
  ELw = L/ del_M; % cm, total length
  
  L = L_i - (L_i - L_b) * exp( - TC_tW * r_B * (tW(:,1) + t_sj)); % cm, expected physical length at time
