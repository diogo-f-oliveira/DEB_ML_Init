function [prdData, info] = predict_Prionailurus_bengalensis(par, data, auxData)
  ...
  % Amur subspecies
  p_Am_m = z_A * p_M/ kap;             % J/d.cm^2, {p_Am} spec assimilation flux
  ...
  Ww_iA = L_im^3 * (1 + f * w_m);      % g, ultimate weight
  ...
  rT_B = TC * k_M/ 3/ (1 + f_tW/g);     % d, 1/von Bert growth rate
  L = L_i - (L_i - L_b) * exp( - rT_B * tW(:,1));      % cm, structural length at time
  EWw = L.^3 * (1 + f_tW * ome);                       % g, weight
  %
  L_bA = L_mm * get_lb([g_m k v_Hb], f_tW); L_iA = L_mm * f_tW;
  rT_BA = TC * k_M/ 3/ (1 + f_tW/g_m);     % d, 1/von Bert growth rate
  L = L_iA - (L_iA - L_bA) * exp( - rT_BA * tW_Amur(:,1));      % cm, structural length at time
  EWw_Amur = L.^3 * (1 + f_tW * w_m);                       % g, weight
 
  % pack to output
  prdData.tW = EWw;
  prdData.tW_Amur = EWw_Amur;
