function [prdData, info] = predict_Aphanius_danfordii(par, data, auxData)
  ...
  tTC = tT; tTC(:,2) = tempcorr(C2K(tT(:,2)), T_ref, T_A);
...
% zero-variate data
  ...
  % length-weight
  EWw_Lf = (LWw_f(:,1) * del_M).^3 * (1 + f * ome);   % g, wet weight 
  EWw_Lm = (LWw_m(:,1) * del_M).^3 * (1 + f * ome_m); % g, wet weight 
...
function dL = get_dL(t, L, tTC, r_B, L_i)
  dL = spline1(mod(t,365), tTC) * r_B * (L_i - L);
end