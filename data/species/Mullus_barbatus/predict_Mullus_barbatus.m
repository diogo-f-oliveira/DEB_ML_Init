function [prdData, info] = predict_Mullus_barbatus(par, data, auxData)
  ...
  Ww_im = L_im^3 * (1 + f * ome_m);      % g, ult wet weight
  ...
  % time-length
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tL, [], tL_fm(:,1)*kT_M);
  ...
  % length-fecundity
  EN = TC_LN * 365 * reprod_rate(LN(:,1)*del_M, f, pars_R); % #, fecundity
