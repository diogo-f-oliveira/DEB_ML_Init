function [prdData, info] = predict_Pipilo_chlorurus(par, data, auxData)
  ...
  % uni-variate data 
  % time-weight
  tf = [1 f1_01; 2 f1_02; 4 f1_04; 6 f1_06]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [1 f2_01; 2 f2_02; 4 f2_04; 6 f2_06]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [1 f3_01; 2 f3_02; 4 f3_04; 6 f3_06]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
