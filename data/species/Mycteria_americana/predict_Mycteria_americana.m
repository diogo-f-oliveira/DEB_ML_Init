function [prdData, info] = predict_Mycteria_americana(par, data, auxData)
  ...
  tf = [0 10 20 30 40 50 60; f_00 f_10 f_20 f_30 f_40 f_50 f_60]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tWJX(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % time-food intake: varying food
  EJ_X =  TC * p_Am * (L_m * tvel(:,4)).^2  .* spline1(tWJX(:,1)*kT_M,tf)/ mu_X * w_X/ d_X/ kap_X; %g/d, (wet) food intake
  
  % pack to output
  prdData.tWJX = [EWw EJ_X];
