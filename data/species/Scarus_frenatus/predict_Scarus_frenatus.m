function [prdData, info] = predict_Scarus_frenatus(par, data, auxData)
...
%   Ww_pm = L_pm.^3 * (1 + f * w_m);     % g, wet weight at puberty
  ...
  % time-length
  tvel = get_tj(pars_tj, f_mid, [], tL_mid(:,1)*kT_M);
  ELw_mid = L_m * tvel(:,4)/ del_M; % cm,  length
  %
  tvel = get_tj(pars_tj, f_out, [], tL_out(:,1)*kT_M);
  ELw_out = L_m * tvel(:,4)/ del_M; % cm,  length

    
  %% pack to output
  prdData.tL_mid = ELw_mid;
  prdData.tL_out = ELw_out; 
