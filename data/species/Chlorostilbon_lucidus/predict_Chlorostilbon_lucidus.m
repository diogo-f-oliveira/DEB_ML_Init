function [prdData, info] = predict_Chlorostilbon_lucidus(par, data, auxData)
  ...
  % compute temperature correction factors - depends on temperature
  ...
  % time-weight
  tf = [2 f_02; 5 f_05; 10 f_10; 15 f_15]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tpm, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome_m);   % g, weight
  %
  tvel = get_tp(pars_tpm, tf, [tau_b 1 l_b], tW1(:,1)*kT_M);
  EWw_1 = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome_m);   % g, weight
