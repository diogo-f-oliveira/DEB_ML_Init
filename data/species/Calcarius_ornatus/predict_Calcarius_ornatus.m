function [prdData, info] = predict_Calcarius_ornatus(par, data, auxData)
  ...
  % uni-variate data
  % time-weight
  tf = [0 f_02; 4 f_04; 6 f_06; 8 f_08]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f1_02; 4 f1_04; 6 f1_06; 8 f1_08]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
