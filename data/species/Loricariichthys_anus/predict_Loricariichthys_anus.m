function [prdData, info] = predict_Loricariichthys_anus(par, data, auxData)
  ...
  % time - length
  tvel = get_tj(pars_tj, f_tL ,[tau_b 1 l_b], tL_f(:,1)*kT_M);
  ELw_f = L_m*tvel(:,4)/ del_M; % cm, total length 
  %
  tvel = get_tj(pars_tjm, f_tL ,[tau_b z/z_m l_b*z/z_m], tL_m(:,1)*kT_M);
  ELw_m = L_mm*tvel(:,4)/ del_M; % cm, total length 

  % time - weight
  tvel = get_tj(pars_tj, f_tL ,[tau_b 1 l_b], tW_f(:,1)*kT_M);
  EWw_f = (L_m*tvel(:,4)).^3.*(1+tvel(:,3)*ome); % g, weight 
  %
  tvel = get_tj(pars_tjm, f_tL ,[tau_b z/z_m l_b*z/z_m], tW_m(:,1)*kT_M);
