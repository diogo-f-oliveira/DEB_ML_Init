function [prdData, info] = predict_Carcharhinus_brevipinna(par, data, auxData)
  ...
  rT_B = kT_M/ 3/ (1 + f_tL1/ g); Lw_b = L_m * get_lb_foetus([g k v_Hb])/ del_M; Lw_i = L_m * f_tL1/ del_M;
  ELw_f1 = Lw_i - (Lw_i - Lw_b) * exp(- rT_B * tL_f1(:,1)); % cm, total length

  % length - weight
  EWw = (LWw_f(:,1) * del_M).^3 * (1 + f_tL1 * ome); % g, wet weight
