function [prdData, info] = predict_Hyla_arborea(par, data, auxData)
  ...
  Lw_jb = L_j/ del_Mb;              % cm, total length at metam for tadpole
  ...
  % univariate data
  % time-length
  rT_B = TC_tL * k_M/ 3/ (1 + f/ g);
  ELw_b = (L_i - (L_i - L_b) * exp(- rT_B * tLb(:,1)))/ del_Mb; % cm, total length
  %
  ELw = (L_i - (L_i - L_j) * exp(- rT_B * tL(:,1)))/ del_M; % cm, SVL
  %
  EWw = (L_i - (L_i - L_j) * exp(- rT_B * tW(:,1))).^3 * (1 + f * w); % g, wet weight
