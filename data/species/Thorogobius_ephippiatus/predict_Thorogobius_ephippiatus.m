function [prdData, info] = predict_Thorogobius_ephippiatus(par, data, auxData)
...
% zero-variate data
  ...
  ELw_f = L_m * tvel(:,4)/ del_M;                                 % cm, total length females
  %
  tvel = get_tp(pars_tpm, f_tL, [], tL_fm(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M;                                 % cm, total length females
