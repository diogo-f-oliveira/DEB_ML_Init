function [prdData, info] = predict_Sceloporus_undulatus(par, data, auxData)
...
% zero-variate data
  ...
  [t, L] = ode45(@get_L, tL_fm(:,1), L_b, [], T_ref, pars_T, L_i, r_B); % EL: {J/cm^3, cm}, with {[E], L}
  ELw_f = L/ del_M; % cm, SVL
  %
  L_bm = L_mm * get_lb([g_m k v_Hb], f_tL); L_im = f_tL * L_mm;
  r_Bm = k_M/ 3/ (1 + f_tL/ g_m); % 1/d, von Bert growth rate
  [t, L] = ode45(@get_L, tL_fm(:,1), L_bm, [], T_ref, pars_T, L_im, r_Bm); % EL: {J/cm^3, cm}, with {[E], L}
...
function dL = get_L(t, L, T_ref, pars_T, L_i, r_B)
  TC = tempcorr(C2K(15+25*sin(2*pi*(t+100)/365)), T_ref, pars_T); % -, TC at t
