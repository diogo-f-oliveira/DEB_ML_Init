function [prdData, info] = predict_Theropithecus_gelada(par, data, auxData)
  ...
  L_im = (f - l_T) * L_mm;             % cm, ultimate struct length
  Lw_im = L_im/ del_M;                 % cm, ultimate sholder-rump length
  Ww_im = L_im^3 * (1 + f * w_m);      % g, ultimate weight
  pars_tpm = [g_m k l_T v_Hb v_Hpm]; 
  [tau_pm, tau_bm, l_pm, l_bm, info] = get_tp(pars_tpm, f, [t_b f*z/z_m l_b*z/z_m]);
  if info == 0
    prdData = []; return
  end
  tT_pm = (tau_pm - tau_bm)/ kT_M;     % d, time since birth at puberty
  Lw_pm = L_m * l_pm/ del_M;           % cm, sholder-rump length at puberty
