function [prdData, info] = predict_Falco_biarmicus(par, data, auxData)
  ...
  % time-weight 
  tf = [20 f_20; 30 f_30; 35 f_35; 40 f_40; 50 f_50; 60 f_60]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  
  % time-weight-feeding rate 
  tvel = get_tp(pars_tp, f_tW, [tau_b 1 l_b], tWJX(:,1)*kT_M);
  EWw_J = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  EJX = (L_m * tvel(:,4)).^2 * f_tW * TC * p_Am/ kap_X/ mu_X * w_X/ d_X; % g/d, (wet) feeding rate

  % pack to output
  prdData.tW = EWw;
  prdData.tWJX = [EWw_J EJX];  
end
