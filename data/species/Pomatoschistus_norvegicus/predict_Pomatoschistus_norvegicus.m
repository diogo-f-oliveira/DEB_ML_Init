function [prdData, info] = predict_Pomatoschistus_norvegicus(par, data, auxData)
...
%   pars_tpm = [g_m k l_T v_Hb v_Hp];
  ...
  % male
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tL, [], tL_m(:,1)*kT_M);
  ELw_m = L_m * tvel(:,4)/ del_M;   % cm, length 
  
  % length-fecundity
  pars_R = [kap; kap_R; g; k_J; k_M; L_T; v; U_Hb; U_Hp]; % compose parameter vector at T
  EN = TC * 365 * reprod_rate(LN(:,1) * del_M, f, pars_R);    % #, fecundity at T
