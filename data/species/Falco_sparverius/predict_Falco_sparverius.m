function [prdData, info] = predict_Falco_sparverius(par, data, auxData)
  ...
  tf = [0 f_00; 5 f_05; 10 f_10; 20 f_20; 30 f_30; 35 f_35]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW_m(:,1)*kT_M);
  EWw_m = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
