function [prdData, info] = predict_Globicephala_melas(par, data, auxData)
  ...
  EWw_f = (L_m * tvel(:,4)).^3 * (1 + f_tL * w)/ 1e3;   % kg, weight 
  % male
  tvel = get_tp(pars_tpm, f_tL, [t_b, f_tL*z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
  EWw_m = (L_mm * tvel(:,4)).^3 * (1 + f_tL * w_m)/ 1e3;   % kg, weight 
