function [prdData, info] = predict_Elliptio_complanata(par, data, auxData)
  ...
  TC_LdL3 = tempcorr(temp.LdL3, T_ref, T_A); 
  ...
  RT_i = TC * reprod_rate_j(5.8*del_M, f, pars_R);  % #/d, reprod rate for SL 5.8 cm
  ...
  EdLw_1 = rT_B * (Lw_i - LdL1(:,1));
  %
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_LdL2);
  kT_M = k_M * TC_LdL2; rT_B = rho_B * kT_M; Lw_i = L_m * l_i/ del_M;
  EdLw_2 = rT_B * (Lw_i - LdL2(:,1));
  %
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_LdL3);
  kT_M = k_M * TC_LdL3; rT_B = rho_B * kT_M; Lw_i = L_m * l_i/ del_M;
  EdLw_3 = rT_B * (Lw_i - LdL3(:,1));
