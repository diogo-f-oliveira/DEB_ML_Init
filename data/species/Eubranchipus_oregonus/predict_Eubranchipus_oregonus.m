function [prdData, info] = predict_Eubranchipus_oregonus(par, data, auxData)
  ...
  tTC = temp.tL_fm; tTC(:,2)= tempcorr(C2K(tTC(:,2)), T_ref, T_A);
  ...
  % time-length
  tf = [20 f_020; 60 f_060; 120 f_120; 160 f_160];
  [tau_j, tau_p, tau_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_060);
  [t, HeL] = ode45(@get_HeL, [0;tL_fm(:,1)], [E_Hb 1 L_m*l_b], [], tf, tTC, v, g, E_m, L_b, L_m, kap, k_J, E_Hj); 
  HeL(1,:)=[]; ELw_f = HeL(:,3)/del_M;
  %
  [tau_jm, tau_pm, tau_bm, l_jm, l_pm, l_bm, l_im, rho_jm, rho_Bm] = get_tj(pars_tjm, f_060);
  [t, HeL] = ode45(@get_HeL, [0;tL_fm(:,1)], [E_Hb 1 L_mm*l_bm], [], tf, tTC, v, g_m, E_m_m, L_mm*l_bm, L_mm, kap, k_J, E_Hj); 
  HeL(1,:)=[]; ELw_m = HeL(:,3)/del_M;
