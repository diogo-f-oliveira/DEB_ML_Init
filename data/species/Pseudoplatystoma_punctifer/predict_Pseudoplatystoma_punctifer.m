function [prdData, info] = predict_Pseudoplatystoma_punctifer(par, data, auxData)
  ...
  TC_P = tempcorr(temp.tL_Pfm, T_ref, T_A);
  ...
  % time - length
  tvel = get_tj(pars_tj, f_A ,[], tL_Afm(:,1)*k_M*TC_A);
  ELw_Af = L_m*tvel(:,4)/ del_M; % cm, total length 
  %
  tvel = get_tj(pars_tjm, f_A ,[0 f_A*z/z_m l_b*z/z_m], tL_Afm(:,1)*k_M*TC_A);
  ELw_Am = L_mm*tvel(:,4)/ del_M; % cm, total length 
  %
  tvel = get_tj(pars_tj, f_P ,[], tL_Pfm(:,1)*k_M*TC_P);
  ELw_Pf = L_m*tvel(:,4)/ del_M; % cm, total length 
  %
  tvel = get_tj(pars_tjm, f_P ,[0 f_P*z/z_m l_b*z/z_m], tL_Pfm(:,1)*k_M*TC_P);
  ELw_Pm = L_mm*tvel(:,4)/ del_M; % cm, total length 


  % pack to output
  prdData.tL_Afm = [ELw_Af ELw_Am];
  prdData.tL_Pfm = [ELw_Pf ELw_Pm];
