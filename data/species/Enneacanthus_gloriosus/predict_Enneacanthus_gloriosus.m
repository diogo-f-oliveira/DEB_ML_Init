function [prdData, info] = predict_Enneacanthus_gloriosus(par, data, auxData)
...
L_bj = L_b * exp(rT_j * tL_M(tL_M(:,1) <= tT_j,1,1)/ 3);
L_ji = L_i - (L_i - L_j) * exp( - rT_B * (tL_M(tL_M(:,1) > tT_j,1) - tT_j)); % cm, struc length
ELw_M = [L_bj; L_ji]/ del_M; % cm, total length
%
[t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tLF);
kT_M  = k_M * TC; rT_j = rho_j * kT_M; rT_B = rho_B * kT_M; tT_j = (t_j - t_b)/ kT_M;
L_b = l_b * L_m; L_j = l_j * L_m; L_i = l_i * L_m;     % cm, length at birth, metamorphosis, ultimate
L_bj = L_b * exp(rT_j * tL_F(tL_F(:,1) <= tT_j,1,1)/ 3);
