function [prdData, info] = predict_Poecile_atricapillus(par, data, auxData)
  ...
  tf = [0 7 14 21; f_00 f_07 f_14 f_21]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

  % time-feeding rate
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tJX(:,1)*kT_M);
  EJX_f = TC * w_X * spline1(kT_M*tJX(:,1),tf) * p_Am .* (L_m * tvel(:,4)).^2/ mu_X/ kap_X;  % g/d, feeding rate at
  %
  tvel = get_tp(pars_tpm, tf, [tau_b 1 l_b], tJX(:,1)*kT_M);
  EJX_m = TC * w_X * spline1(kT_M*tJX(:,1),tf) * p_Am_m .* (L_mm * tvel(:,4)).^2/ mu_X/ kap_X;  % g/d, feeding rate at

  % pack to output
  prdData.tW = EWw;  
  prdData.tJX = [EJX_f EJX_m];  
end
