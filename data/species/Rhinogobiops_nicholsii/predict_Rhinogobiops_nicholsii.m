function [prdData, info] = predict_Rhinogobiops_nicholsii(par, data, auxData)
  ...
  [l_pm, l_bm, l_im] = get_lp(pars_tpm, f);
  ...
  ELw_f = L_m * tvel(:,4)/ del_M; % cm,  TL
  %
  tvel = get_tp(pars_tpm, f_tL, [0 z/z_m l_b*z/z_m], tL_fm(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M; % cm,  TL
