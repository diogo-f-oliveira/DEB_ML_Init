function [prdData, info] = predict_Rhinecanthus_aculeatus(par, data, auxData)
...
% zero-variate data
  ...
  %
  tvel = get_tp(pars_tp, f_tL, [0 z/z_m l_b*z/z_m], tL_m(:,1)*kT_M);
  ELw_M = L_mm * tvel(:,4)/ del_M; % cm, fork length
  %
  tvel = get_tp(pars_tp, f_tL, [], tL_fm(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M; % cm, fork length
  %
  tvel = get_tp(pars_tp, f_tL, [0 z/z_m l_b*z/z_m], tL_fm(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M; % cm, fork length
