function [prdData, info] = predict_Agkistrodon_piscivorus(par, data, auxData)
...
% zero-variate data
  ...
  EdLw_f = rT_B * (Lw_i - LdL_f(:,1)); % cm/d, chamge in SVL
  %
  rT_Bm = TC * k_M/ 3/ (1 + f_tL/ g_m); Lw_im = f_tL * L_mm/ del_M;
  EdLw_m = rT_Bm * (Lw_im - LdL_m(:,1)); % cm/d, chamge in SVL
