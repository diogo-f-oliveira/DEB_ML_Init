function [prdData, info] = predict_Anguilla_japonica(par, data, auxData)
...
%% compute temperature correction factors
  ...
  % time-length
  % leptocephali
  L_b =  get_lb([g k v_Hb], f_tLj); L_i = L_m * f_tLj; 
  kT_M = TC * k_M; rT_B = kT_M/ 3/ (1 + f_tLj/ g); % 1/d, von Bert growth rate
  L = L_i - (L_i - L_b) * exp( - rT_B * tLj(:,1)); % cm, struct length 
  ELw_j = L/ del_Ml; % cm, total length  
  % post metam
  ...
  % time-weight
  L = L_i - (L_i - L_b) * exp( - rT_B * (tW(:,1) - t_sj)); % cm, struct length
