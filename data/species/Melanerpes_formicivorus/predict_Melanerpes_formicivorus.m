function [prdData, info] = predict_Melanerpes_formicivorus(par, data, auxData)
  ...
  % uni-variate data
  % time-weight 
  tf = [05 10 15 17 20 25; f_05 f_10 f_15 f_17 f_20 f_25]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [05 10 15 17 20 25; f_05 f_10 f_15 f1_17 f1_20 f1_25]'; tf(:,1) = tf(:,1) * kT_M;
