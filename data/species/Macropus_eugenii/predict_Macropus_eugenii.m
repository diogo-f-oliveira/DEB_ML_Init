function [prdData, info] = predict_Macropus_eugenii(par, data, auxData)
  ...
  tT_g = t_0 + t_b/ k_M/ TC_tg;     % d, age at birth at f and T
  ...
  % Food intake
  L = (4300/ (1 + f * w))^(1/3);        % cm, sturcutral length of a 4.3 kg animal
  pT_X = L^2 * p_Xm * TC_pL;            % J/d, for an animal of length L
  
  % Digestible energy intake
  pT_A = L^2 * p_Am * TC_pL;            % J/d, for an animal of length L
  
  % Feaces production
  pT_P = kap_P * pT_X;                  % J/d, for an animal of length L
  ...
  ELw = L/ del_M;                            % g, wet weight

  pT_ref = TC_pL * p_Am * L_m^2;         % J/d, max assimilation power at max size  pars_power = [kap; kap_R; g; k_J; k_M; L_T; v; U_Hb; U_Hp];
  L = (WJO(:,1)/ (1 + w * f)).^(1/3);    % cm, structural length
  pars_power = [kap; kap_R; g; k_J; k_M; L_T; v; E_Hb; E_Hp];
  %pACSJGRD = scaled_power(L, f, pars_power, l_b, l_p); 
  %pT_ADG = pT_ref * pACSJGRD(:,[1 7 5])'; % J/d, temp corrected assim, dissip, growth
  l = L/ L_m;  % -, scaled structural length
  H = maturity_old (L, f, pars_power);  % d.cm^2, scaled maturities E_H/ {p_Am}
  ...
  EJO = - 1e3 * J_M(3,:)'/ (24 * 60 * X_gas);  % ml/min, dioxygen use 
