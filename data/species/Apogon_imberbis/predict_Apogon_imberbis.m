function [prdData, info] = predict_Apogon_imberbis(par, data, auxData)
...
% zero-variate data
  ...
  tT_p = (tau_p - tau_b)/ kT_M;     % d, timse since birt at puberty
  ...
  % time-length
  tf = [200 f_0200; 1200 f_1200]; tf(:,1)=tf(:,1)*kT_M;
  [tvel,~,~,~,~,~,l_b] = get_tj(pars_tj, tf,[], tL_f(:,1)*kT_M);
  ELw_f = L_m*tvel(:,4)/ del_M; % cm, total length
  %
  tvel = get_tj(pars_tjm, tf,[0 1 l_b*z/z_m], tL_m(:,1)*kT_M);
  ...
  EN = TC * 365 * reprod_rate_j(LN(:,1) * del_M, f, pars_R);    % #, fecundity at T
