function [prdData, info] = predict_Duttaphrynus_melanostictus(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);
  
  if E_Hj < E_Hb || E_Hj > E_Hp || kap < 0.45
  ...
  rT_B = TC * k_M/ 3/ (1 + f_tL/ g); % d 1/von Bert growth rate
  ELw = (L_i - (L_i - L_b) * exp( - rT_B * tL(:,1)))/del_M; % cm, SVL
  %
  L = L_i - (L_i - L_b) * exp( - rT_B * tW(:,1)); % cm, struct length
  EWw = L.^3*(1 + f_tL * w); % , weight
  
  % length-weight
  EWw_L = (LW(:,1) * del_M).^3 * (1 + f_tL * w);
