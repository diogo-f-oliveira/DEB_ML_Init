function [prdData, info] = predict_Cervus_elaphus(par, data, auxData)
  ...
  % time-weight
  % female
  L_0 = (1e3 * (W_f0/ del_W - Ww_0)/ (1 + f * w))^(1/3);
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tW, [t_b f_tW L_0/L_m], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male
  L_0m = (1e3 * (W_m0/ del_W - Ww_0)/ (1 + f * w))^(1/3);
  tvel = get_tp(pars_tpm, f_tW, [t_b, f_tW*z/z_m, L_0m/L_mm], tW_m(:,1)*kT_M);
