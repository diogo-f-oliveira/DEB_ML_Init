function [prdData, info] = predict_Oncorhynchus_nerka(par, data, auxData)
...
%% compute temperature correction factors
  ...
  kT_M = TC_tWw * k_M; rT_j = rho_j * kT_M; rT_B = rho_B * kT_M;  tT_j = (tau_j - tau_b)/ kT_M;
  L_b = L_m * l_b; L_j = L_m * l_j; L_i = L_m * l_i; t = tWw(:,1);
  L_0 = (Ww_0/(1 + f_tWw * ome))^(1/3); t_0 = log(L_0/L_b)*3/rT_j; tT_j = tT_j - t_0;
  L_bj = L_0 * exp(t(t<tT_j) * rT_j/3);
  L_ji = L_i - (L_i - L_j) * exp(-rT_B * (t(t>tT_j) - tT_j)); % cm, struc length
  EWw = [L_bj; L_ji].^3 * (1 + f_tWw * ome);
