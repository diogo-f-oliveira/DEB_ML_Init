function [prdData, info] = predict_Hadropareia_middendorffii(par, data, auxData)
  ...
  % length-weight
  LWw_f = (LW_f(:,1) * del_M) .^ 3 * (1 + f_tL * w); % g, wet weight females
  LWw_m = (LW_m(:,1) * del_M) .^ 3 * (1 + f_tL * w_m); % g, wet weight males
  
  % length-fecundity
  EN = 365 * TC * reprod_rate_j(LN(:,1) * del_M, f_tL, pars_R);        % #/d, ultimate reproduction rate
