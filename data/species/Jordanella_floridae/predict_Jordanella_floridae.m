function [prdData, info] = predict_Jordanella_floridae(par, data, auxData)
  ...
  L_ji = L_i - (L_i - L_j) * exp(- rT_B * (tL(tL(:,1) >= tT_j,1) - tT_j)); % cm, atruc length 
  ELw = [L_bj; L_ji]/ del_M; % cm, total length

  % time - weight
  EWw = (L_i - (L_i - L_j) * exp( - rT_B * (tWw(:,1) - tT_j))).^3 * (1 + f_tL * ome); % g, wet weight
