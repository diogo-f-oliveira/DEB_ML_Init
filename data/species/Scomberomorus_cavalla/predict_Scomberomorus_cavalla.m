function [prdData, info] = predict_Scomberomorus_cavalla(par, data, auxData)
  ...
  % time-length
  % female
  tf = [1000 f_1000; 5000 f_5000]; tf(:,1) = tf(:,1) * kT_M;
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, tf, [], tL_f(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length
  % male
  tvel = get_tj(pars_tjm, tf, [t_b, z/z_m, l_b*z/z_m], tL_m(:,1)*kT_M);
