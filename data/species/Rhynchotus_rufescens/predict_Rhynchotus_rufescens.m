function [prdData, info] = predict_Rhynchotus_rufescens(par, data, auxData)
  ...
  % time-weight
  tf = [5 10 70 80;f_05 f_10 f_70 f_80]'; tf(:,1) = tf(:,1) * kT_M;
  [ii, ia, ic] = unique(tW(:,1));
  tvel = get_tp(pars_tp, tf, [], tW(ia,1) * kT_M);
  EWw = (L_m * tvel(ic,4)).^3 .* (1 + tvel(ic,3) * ome);   % g, weight
