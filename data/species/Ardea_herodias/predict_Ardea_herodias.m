function [prdData, info] = predict_Ardea_herodias(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);
  
  if f1_30 < 0; info = 0; prdData = []; return; end
  ...
  % time-weight
  % female
  tf = [5 15 40; f_05 f_15 f_40]'; tf(:,1) = tf(:,1) * kT_M;
  % female
  ...
  % time - energy intake
  tf = [0 5 15 30 45; f1_00 f1_05 f1_15 f1_30 f1_45]'; tf(:,1) = tf(:,1) * kT_M;
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [tau_b 1 l_b], tp_A(:,1)*kT_M);
  EpT_A = TC * p_Am * (L_m * tvel(:,4)).^2/ 1e3/ kap_X; % kJ/d, energy intake
