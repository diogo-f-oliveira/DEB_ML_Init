function [prdData, info] = predict_Dermophis_mexicanus(par, data, auxData)
  ...
  % time-length 
  L_i = L_m * f_tL; L_j = L_m * get_lp(pars_tj, f_tL); rT_B = TC * k_M/ 3/ (1 + f_tL/ g);
  L = L_i - (L_i - L_j) * exp( - rT_B * tL(:,1)); % cm, expected length at time
