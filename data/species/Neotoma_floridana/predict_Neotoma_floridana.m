function [prdData, info] = predict_Neotoma_floridana(par, data, auxData)
  ...
  % time-weight 
  tf = [15 25 35 90; f_15 f_25 f_35 f_90]';
  [t, EL] = ode45(@get_EL, [0;tW_f(:,1)], [E_m, L_b], [], tf, v * TC, g, E_m, L_m, p_Am * TC); % EL: {J/cm^3, cm}, with {[E], L}
  EL(1,:)=[]; EWw_f = EL(:,2).^3 .* (1 + EL(:,1) * w_E/ mu_E/ d_E); % g, wet weight
  %
  [t, EL] = ode45(@get_EL, [0;tW_m(:,1)], [E_m, L_b], [], tf, v * TC, g_m, E_m_m, L_mm, p_Am_m * TC); % EL: {J/cm^3, cm}, with {[E], L}
