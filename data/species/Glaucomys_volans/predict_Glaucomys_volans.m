function [prdData, info] = predict_Glaucomys_volans(par, data, auxData)
  ...
  % time-weight 
  f = f_tW; L_b = L_m * get_lb([g k v_Hb], f);         % cm, intial struc length
  rT_B = TC * k_M/ 3 / (1 + f/ g);                     % 1/d, von Bert growth rate
  L_i = f * L_m - L_T;                                 % cm, ultimate struc length 
  L = L_i - (L_i - L_b) * exp( - rT_B * tW(:,1));      % cm, structural length at time
  EWw = L.^3 * (1 + f * w);                            % g, wet weight
  %
  f = f_tW1; L_b = L_m * get_lb([g k v_Hb], f);        % cm, intial struc length
  rT_B = TC * k_M/ 3 / (1 + f/ g);                     % 1/d, von Bert growth rate
  L_i = f * L_m - L_T;                                 % cm, ultimate struc length 
