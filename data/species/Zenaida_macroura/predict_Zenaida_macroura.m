function [prdData, info] = predict_Zenaida_macroura(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  if E_Hx > E_Hp ||  p_M > 9500
  ...
  % puberty
  tT_p = (tau_p - tau_b)/ kT_M;         % d, time since birth at pubert at f and T
  ...
  % time-weight
  tf = [20 f_20; 30 f]; tf(:,1) = tf(:,1) * kT_M;
