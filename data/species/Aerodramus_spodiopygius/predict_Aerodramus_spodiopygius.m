function [prdData, info] = predict_Aerodramus_spodiopygius(par, data, auxData)
  ...
  % time-weight
  tf = [1 3 15 20;f f_tW f_tW f_end]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [v_Hb 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [1 3 15 20;f f_tW2 f_tW2 f_end]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [v_Hb 1 l_b], tW2(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [1 3 20 25;f f_tW3 f_tW3 f_end]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [v_Hb 1 l_b], tW3(:,1)*kT_M);
