function [prdData, info] = predict_Donax_vittatus(par, data, auxData)
  ...
  kT_M = TC_tL * k_M; rT_B = rho_B * kT_M; L_j = l_j * L_m; L_i = l_i * L_m;       
  ...
  kT_M = TC_tL1 * k_M; rT_B = rho_B * kT_M; L_j = l_j * L_m; L_i = l_i * L_m;       
  L = L_i - (L_i - L_j) * exp( - rT_B * (tL1(:,1) - tT_j)); % cm, struc length
  ELw_1 = L/ del_M;                     % cm, shell length
  %
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tL2);
  kT_M = TC_tL2 * k_M; rT_B = rho_B * kT_M; L_j = l_j * L_m; L_i = l_i * L_m;       
