function [prdData, info] = predict_Ailurus_fulgens(par, data, auxData)
  ...
  EWw_2 = L.^3 * (1 + f * w);                          % g, weight
  %
  L = L_i - (L_i - L_b) * exp( - rT_B * tW3(:,1));     % cm, structural length at time
  EWw_3 = L.^3 * (1 + f * w);                          % g, weight
  %
  L = L_i - (L_i - L_b) * exp( - rT_B * tW4(:,1));     % cm, structural length at time
  EWw_4 = L.^3 * (1 + f * w);                          % g, weight
