function [prdData, info] = predict_Sphyrna_lewini(par, data, auxData)  
  ...
  [t_pm t_b l_pm l_b] = get_tp_foetus(pars_tpm, f); % -, scaled length at birth at f
...
%   Ww_im = L_mm^3 * (1 + f * wm);   % g, wet weight at puberty 
...
%   Prd_data. = Ww_im;
  ...
  % length-weight
  EWw = (LW(:,1) * del_M).^3 * (1 + f_tL * w); % g, weight weight at f = 1
