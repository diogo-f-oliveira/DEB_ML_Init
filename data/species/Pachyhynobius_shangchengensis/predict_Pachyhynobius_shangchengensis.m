function [prdData, info] = predict_Pachyhynobius_shangchengensis(par, data, auxData)
  ...
  TC_fm = tempcorr(temp.tL_fm, T_ref, T_A);
...
% zero-variate data
  ...
  %
  rT_B = TC_fm * k_M/ 3/ (1 + f_tLfm/ g); L_i = f_tLfm * L_m; L_b = L_m * get_lb([g; k; v_Hb], f_tLfm); 
  L = L_i - (L_i - L_b) * exp(- rT_B * tL_fm(:,1)); ELw_f = L/ del_M; EWw_f = L.^3 * (1 + f_tLfm * w);
  %
  rT_Bm = TC_fm * k_M/ 3/ (1 + f_tLfm/ g_m); L_im = f_tLfm * L_mm; L_bm = L_mm * get_lb([g_m; k; v_Hb], f_tLfm); 
  L = L_im - (L_im - L_bm) * exp(- rT_Bm * tL_fm(:,1)); ELw_m = L/ del_M; EWw_m = L.^3 * (1 + f_tLfm * w_m);
