function [prdData, info] = predict_Lepus_europaeus(par, data, auxData)
  ...
  % foetal development
  vT = TC * v;
  EWw_e = (1 + f_tW_e * w) * (vT * max(0, tW_e(:,1) - t_0)/ 3) .^ 3;
  EWw_e2 = (1 + f_tW_e2 * w) * (vT * max(0, tW_e2(:,1) - t_0)/ 3) .^ 3;
  ELw_e2 = vT * max(0, tL_e2(:,1) - t_0)/ 3/ del_Me;
