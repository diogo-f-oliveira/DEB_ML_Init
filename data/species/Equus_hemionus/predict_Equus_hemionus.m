function [prdData, info] = predict_Equus_hemionus(par, data, auxData)
  ...
  [t_pm, t_xm, t_bm, l_pm, l_xm, l_bm, info] = get_tx(pars_tx, f);
  if info == 0
    prdData = []; return
  end
  ...
  L = L_i - (L_i - 0*L_b) * exp( - rT_B * tL(:,1));    % cm, structural length at time
  ELw = L/ del_M;                                         % cm, ditance to crown
