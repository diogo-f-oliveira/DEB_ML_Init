function [prdData, info] = predict_Natrix_natrix(par, data, auxData)
...
% zero-variate data
  ...
  %
  pars_tpm = [g_m k l_T v_Hbm v_Hpm];
  ...
  % time-length
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f, [], tL_f(:,1)*k_M*TC_tL);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length 
  % male
  tvel = get_tpm(pars_tpm, f, [t_b, f*z/z_m, l_b*z/z_m], tL_m(:,1)*k_M*TC_tL);
  ELw_m = L_mm * tvel(:,4)/ del_M;   % cm, length 

  % fecundity
  pars_R = [kap; kap_R; g; k_J; k_M; L_T; v; U_Hb; U_Hp]; % compose parameter vector at T
  EN = TC_LN * 365 * reprod_rate(LN(:,1) * del_M, f, pars_R);  % #, fecindity at T
