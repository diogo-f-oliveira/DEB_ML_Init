function [prdData, info] = predict_Streptopelia_roseogrisea(par, data, auxData)
  ...
  % time-weight 
  tf = [0 2 20 30; f1_00 f1_02 f1_20 f1_30]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW1(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 2 20 30; f2_00 f2_02 f2_20 f2_30]'; tf(:,1) = tf(:,1) * kT_M;
