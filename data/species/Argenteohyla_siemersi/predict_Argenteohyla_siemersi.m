function [prdData, info] = predict_Argenteohyla_siemersi(par, data, auxData)
  ...
  % metam
  L_j = L_m * l_j;                  % cm, SVL at metam at f
  ...
  % univariate data
  % time-length
  L_i = L_m * f_tL; rT_B = TC * k_M/ 3/ (1 + f_tL/ g);
  L = L_i - (L_i - L_j) * exp(- rT_B * tL_fm(:,1)); % cm, total length
  ELw_f = L/ del_M; EWw_f = L.^3*(1+f_tL * w);
  %
  L_im = L_mm * f_tL; rT_Bm = TC * k_M/ 3/ (1 + f_tL/ g_m);
  L = L_im - (L_im - L_j) * exp(- rT_Bm * tL_fm(:,1)); % cm, total length
  ELw_m = L/ del_M; EWw_m = L.^3*(1+f_tL * w_m);
