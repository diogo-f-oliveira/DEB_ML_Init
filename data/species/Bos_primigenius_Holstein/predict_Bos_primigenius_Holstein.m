function [prdData, info] = predict_Bos_primigenius_Holstein(par, data, auxData)
  ...
  pars_tx = [g k l_T v_Hb v_Hx v_Hp 1];  % slow foetal development
  ...
  [t_pm, t_bm, l_pm, l_bm] = get_tp(pars_tpm, f_tW, [t_b f_tW*z/z_m l_b*z/z_m]);
  tT_pm = (t_pm - t_bm)/ kT_M;         % d, time since birth at puberty
  L_pm = L_mm * l_pm; L_im = (f - l_T) * L_mm;
  Ww_pm = L_pm^3 * (1 + f * w_m); Ww_im = L_im^3 * (1 + f * w_m);
