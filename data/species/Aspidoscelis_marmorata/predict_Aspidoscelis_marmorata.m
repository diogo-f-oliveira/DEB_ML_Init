function [prdData, info] = predict_Aspidoscelis_marmorata(par, data, auxData)
...
% zero-variate data
  ...
  Ww_i = (7.68 * del_M)^3 * (1 + f * w); % g, ultimate wet weight at f
  ...
  Ww_im = (8.097 * del_M)^3 * (1 + f * w_m);          % g, ltimate wet weight
  ...
  EdLw_f = rT_B * (Lw_i - LdL_f(:,1)) * 1e4; % mum/d, change in SVL 
  %
  Lw_im = L_mm * f_tL/ del_M;
  rT_Bm = TC * k_M/ 3/ (1 + f_tL/ g_m); % 1/d, von Bert growth rate
  EdLw_m = rT_Bm * (Lw_im - LdL_m(:,1)) * 1e4; % mum/d, change in SVL 
