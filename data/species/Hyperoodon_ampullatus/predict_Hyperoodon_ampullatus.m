function [prdData, info] = predict_Hyperoodon_ampullatus(par, data, auxData)
  ...
  L_pm = L_mm * l_pm; Lw_pm = L_pm/ del_M; % cm, length at buperty
  L_im = f * L_mm; L_bm = l_bm * L_mm; % cm, ultimate structural length
  Lw_im = L_im/ del_M;                 % cm, ulitmate length 
...
%   L = L_i - (L_i - L_b) * exp( - rT_B * tW_f(:,1) * 365); % cm, struct length
%   EWw_f = L.^3 * (1 + w * f)/ 1e3; % kg, total weight female
%   % 
%   ir_B = 3/ k_M + 3 * f * L_mm/ v; rT_Bm = TC/ ir_B;  % d, 1/von Bert growth rate
%   L = L_im - (L_im - L_bm) * exp( - rT_Bm * tW_m(:,1) * 365); % cm, struct length
