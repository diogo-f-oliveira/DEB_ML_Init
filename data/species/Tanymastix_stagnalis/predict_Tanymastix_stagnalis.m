function [prdData, info] = predict_Tanymastix_stagnalis(par, data, auxData)
  ...
  tTC_87 = temp.tL_1987; tTC_87(:,2) = tempcorr(C2K(tTC_87(:,2)), T_ref, T_A);
  tTC_88 = temp.tL_1988; tTC_88(:,2) = tempcorr(C2K(tTC_88(:,2)), T_ref, T_A);
...
%   pars_tjm = [g_m k l_T v_Hb v_Hj v_Hp];
  ...
  % length-fecundity
  EN = 7 * TC * reprod_rate_j(LN(:,1)*del_M, f, pars_R); % #/d, max reprod rate
  
  % time-length
  options = odeset('Events',@eoa); options = [];
  tf = [5 f87_05; 10 f87_10; 30 f87_30; 60 f87_60];  
  [t, HeL] = ode45(@get_HeL, tL_1987(:,1), [E_Hb, 1, L_b], options, tf, tTC_87, v, g, E_m, L_b, L_m, kap, k_J, E_Hj);
  ELw_87 = HeL(:,3)/ del_M;  % cm, total length
  %
  tf = [5 f88_05; 10 f88_10; 30 f88_30; 60 f88_60];  
  [t, HeL] = ode45(@get_HeL, tL_1988(:,1), [E_Hb, 1, L_b], options, tf, tTC_88, v, g, E_m, L_b, L_m, kap, k_J, E_Hj);
  ELw_88= HeL(:,3)/ del_M;  % cm, total length

  % pack to output
  prdData.tL_1987 = ELw_87; 
  prdData.tL_1988 = ELw_88;
  prdData.LN = EN;
end
