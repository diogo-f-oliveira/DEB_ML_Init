function [prdData, info] = predict_Sceloporus_spinosus(par, data, auxData)
...
%% uni-variate data
  ...
  % univariate data
  % length-chnange in length
  Lw_i = L_m * f_tL/ del_M;
  rT_B = TC * k_M/ 3/ (1 + f_tL/ g); % 1/d, von Bert growth rate
  EdLw_f = rT_B * (Lw_i - LdL_f(:,1)); % cm/d, chnage in SVL
  EdLw_m = rT_B * (Lw_i - LdL_m(:,1)); % cm/d, chnage in SVL
