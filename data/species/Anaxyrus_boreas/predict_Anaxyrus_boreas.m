function [prdData, info] = predict_Anaxyrus_boreas(par, data, auxData)
  ...
  % time-weight
  L_j = L_m * get_lp(pars_tj, f_tW); L_i = L_m * f_tW; r_B = k_M/ 3/ (1 + f_tW/ g);
  L = L_i - (L_i - L_j) * exp(-TC_tW(1) * r_B * tW(:,1)); 
  EWw = L.^3 * (1 + f_tW * w); % g, weight
  %
  L = L_i - (L_i - L_j) * exp(-TC_tW(2) * r_B * tW(:,1)); 
  EWw = [EWw, L.^3 * (1 + f_tW * w)]; % g, weight
  %
  L = L_i - (L_i - L_j) * exp(-TC_tW(3) * r_B * tW(:,1)); 
  EWw = [EWw, L.^3 * (1 + f_tW * w)]; % g, weight
  %
  L = L_i - (L_i - L_j) * exp(-TC_tW(4) * r_B * tW(:,1)); 
  EWw = [EWw, L.^3 * (1 + f_tW * w)]; % g, weight
