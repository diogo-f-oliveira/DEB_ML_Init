function [prdData, info] = predict_Carassius_gibelio(par, data, auxData)
  ...
  % time-length post metam & length-weight
  % Gruza, 
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_G);
  ...
  ELw = L/ del_M;
  %
  EWw = (LW(:,1) * del_M).^3 * (1 + f_G * w);  % g, wet weight
  % Meduvrsje
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_M);
  kT_M = k_M * TC; rT_B = rho_B * kT_M; tT_j = (t_j - t_b)/ kT_M; L_j = L_m * l_j; L_i = L_m * l_i;
  L = L_i - (L_i - L_j) * exp( - rT_B * (tL1(:,1) - tT_j)); % cm, structural length
  ELw_1 = L/ del_M;
  %
  EWw_1 = (LW1(:,1) * del_M).^3 * (1 + f_M * w);  % g, wet weight
