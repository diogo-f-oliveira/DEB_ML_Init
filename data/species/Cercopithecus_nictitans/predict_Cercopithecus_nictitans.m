function [prdData, info] = predict_Cercopithecus_nictitans(par, data, auxData)
  ...
  % time-weight
  % female
  [tvel, tau_p, tau_b, l_p, l_b] = get_tp(pars_tp, f_tW, [], tWw_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  %
  [t, vel] = ode45(@dget_vel, tWw_m(:,1)*kT_M, [v_Hb f_tW l_b], [], f_tW, g, g_m, k, l_T, v_Hp, z, z_m); % EL: {J/cm^3, cm}, with {[E], L}
  EWw_m = (L_m * vel(:,3)).^3 .* (1 + vel(:,2) * w);   % g, weight
...
function dvel = dget_vel(tau, vel, f, g, g_m, k, l_T, v_Hp, z, z_m)
  % unpack vel
  ...
  if v_H < v_Hp % juvenile
    ...
    rho = g * (e/ l - (1 + l_T/ l))/ (e + g_m); % scaled specific growth rate
    dv_H = 0;
    de = (f*z_m/z - e) * g_m/ l;  % d/dtau e
  end
  dl = rho * l/ 3; % d/dtau l
