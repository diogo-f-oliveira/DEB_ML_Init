function [prdData, info] = predict_Knipowitschia_panizzae(par, data, auxData)
...
% zero-variate data
  ...
  % time-length
  [t, L] = ode45(@get_L, [0;tL(:,1)], L_m*get_lb([g k v_Hb], f_tL), [], T_ref, T_A, L_m*f_tL, k_M/3/(1+f_tL/g)); 
...
function dL = get_L(t,L, T_ref, T_A, L_i, r_B)
  TC = tempcorr(C2K(15+11.7*sin(2*pi*(t-330)/365)), T_ref, T_A);
  dL = TC * r_B * (L_i - L);
end
