function [prdData, info] = predict_Rattus_argentiventer(par, data, auxData)
  ...
  L_mm = v/ k_M/ g_m; L_im = L_mm * f; % cm, max struct length
  Ww_bm = (l_b*L_mm)^3 * (1 + f * w_m);% g, wet weight at birth
  Ww_im = L_im^3 * (1 + f * w_m);      % g, ultimate wet weight
  pars_tpm = [g_m k l_T v_Hb v_Hpm]; 
  [t_pm, t_bm] = get_tp(pars_tpm, f, [t_b f*z/z_m l_b*z/z_m]);
  tT_pm = (t_pm - t_bm)/kT_M;          % cm, time since birth at maturity
