function [prdData, info] = predict_Daphnia_schoedleri(par, data, auxData)
  ...
  % time-length
  tf = [5 f_05; 10 f_10; 50 f_50; 90 f_90]; tf(:,1) = tf(:,1)* kT_M;
  tvel = get_tp(pars_tp, tf,[],tL(:,1)*kT_M); 
  ELw = L_m*tvel(:,4)/del_M; 
    
  % length-fecundity
  EN = 2.5 * TC * reprod_rate(LN(:,1)*del_M, f_50, pars_R); % #, fecundity
