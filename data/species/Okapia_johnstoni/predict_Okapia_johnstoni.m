function [prdData, info] = predict_Okapia_johnstoni(par, data, auxData)
  ...
  % time-weight 
  f = f_tW; rT_B = TC * k_M/ 3/ (1 + f/ g); l_b = get_lb([g k v_Hb], f);        % 1/d, von Bert growth rate
  l = f - (f - l_b) * exp( - rT_B * tW1(:,1));                                  % cm, structural length at time
  EWw_1 = (L_m * l).^3 * (1 + f * w)/ 1e3;  % kg, weight                                         % g, wet weight
  %
  l = f - (f - l_b) * exp( - rT_B * tW2(:,1));                                  % cm, structural length at time
  EWw_2 = (L_m * l).^3 * (1 + f * w)/ 1e3;  % kg, weight                                              % g, wet weight
  %
  l = f - (f - l_b) * exp( - rT_B * tW3(:,1));                                  % cm, structural length at time
  EWw_3 = (L_m * l).^3 * (1 + f * w)/ 1e3;  % kg, weight                                      % g, wet weight
