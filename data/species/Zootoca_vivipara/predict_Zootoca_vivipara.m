function [prdData, info] = predict_Zootoca_vivipara(par, data, auxData)
...
Ww_O2 = 2; % g, mean wet weight of experimental animals
...
L = (Ww_O2 / (1 + f * w)).^(1/3); % cm, structural length, using 2 g wet mass
...
EO = - JM(:,3) * X_gas .* tempcorr(C2K(TO(:,1)), T_ref, pars_T)/ 24/ 60 * 1000; % L/day to ml/min
...
EWt = (EL2 * del_M).^3 * (1 + f * w); % g, wet weight
