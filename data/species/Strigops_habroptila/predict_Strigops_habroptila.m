function [prdData, info] = predict_Strigops_habroptila(par, data, auxData)
  ...
  pars_tpm = [g_m; k; l_T; v_Hb; v_Hp];  % compose parameter vector for males
  [t_pm, t_bm, l_pm, l_bm, info] = get_tp(pars_tpm, f); % -, scaled times & lengths at f
  if ~info; prdData = []; return; end
  L_im = f * L_mm;                     % cm, struct length, birth & ultimate
  L_xm = l_pm * L_mm;                  % cm, struct length at fledging
  Ww_xm = L_xm^3 * (1 + f * ome_m);      % g, wet weight at fledging
...
%   prdData.Wwx = Ww_x;
  ...
  % uni-variate data 
  % time-weight
  tf = [10 f_10; 30 f_30; 50 f_50; 70 f_70; 90 f_90]; tf(:,1) = tf(:,1) * kT_M;
  % female
