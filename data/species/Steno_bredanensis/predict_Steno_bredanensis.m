function [prdData, info] = predict_Steno_bredanensis(par, data, auxData)
  ...
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tLfm, [], tL_f(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length 
  % male
  tvel = get_tp(pars_tpm, f_tLfm, [t_b, f_tLfm*z/z_m, l_b*z/z_m], tL_m(:,1)*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M;   % cm, length 

  
  % length-weight
  EWw = (LW(:,1) * del_M).^3 * (1 + f * w)/1e3; % g, weight
