function [prdData, info] = predict_Carcharias_taurus(par, data, auxData)
  ...
  Ww_pm = L_pm^3 * (1 + f * w_m);      % g, wet weight at birth
  ...
  % time-length for embryo/juvenile
  U_E0 = initial_scaled_reserve(f, [V_Hb/TC; g; kT_J; kT_M; vT]);
  [a LUH] = ode45(@dget_LUH, [0; tL_e(tL_e(:,1) < aT_b,1)], [1e-10 U_E0 0], [], kap, vT, kT_J, g, L_m); 
  LUH(1,:) = []; L = LUH(:,1); % cm, structural length of embryo
  rT_B = kT_M/ 3/ (1 + f/ g); % 1/d, von Bert growth rate
  ELw_e = [L; L_i - (L_i - L_b) * exp(- rT_B * (tL_e(tL_e(:,1) >= aT_b) - aT_b))]/ del_M; % cm, total length
