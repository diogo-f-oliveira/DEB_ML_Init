function [prdData, info] = predict_Potamopyrgus_antipodarum(par, data, auxData)
  ...
  % uni-variate data
  % time-length unknown age t-L2
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tL2);
  kT_M = k_M * TC_tL2; 
  rT_B = rho_B * kT_M; 
  Lw_i = l_i * L_m/ del_M;
  ELw2 = Lw_i - (Lw_i - L0) * exp( - rT_B * tL2(:,1)); % cm, expected length at time
 
   % time-length unknown age t-L3 (adults)
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tL3);
  kT_M = k_M * TC_tL3; 
  rT_B = rho_B * kT_M; 
  Lw_i = l_i * L_m/ del_M;
  ELw3 = Lw_i - (Lw_i - L03) * exp( - rT_B * tL3(:,1)); % cm, expected length at time
  
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f);
  rT_B = TC_tL2 * rho_B * k_M; rT_j = TC_tL2 * rho_j * k_M; % 1/d, von Bert, exponential growth rate
  L_j = l_j * L_m; L_i = l_i * L_m;
  
  
  % gaiac unpublished tL-data
  % f_tL used for these data
  ...
  %Length-wet weight L-Ww 
  L = LWw(:,1) * del_M; %structural length at time t
  EWw2 = L.^3 * (1 + f * w); % g, wet weight 
