...
function [prdData, info] = predict_Mareca_strepera(par, data, auxData)
...
% uni-variate data
  ...
  % time-weight
  tf = [30 f_30; 40 f_40];  tf(:,1) = tf(:,1) * kT_M; 
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  % male
  tvel = get_tp(pars_tp, tf, [t_b, tf(end,2)*z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
  EWw_m = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome_m); % g, weight

% time-embryo weight
a = [0; tW_e(:,1) - t_0]; n = length(a); for i=2:n; if a(i)<=a(i-1); a(i)=a(i-1)+1e-8;end;end
[a, LUH] = ode45(@dget_LUH, a, [1e-10 U_E0/TC 0], [], kap, v * TC_ab, k_J * TC_ab, g, L_m);
