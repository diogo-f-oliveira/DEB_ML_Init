function [prdData, info] = predict_Genetta_genetta(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);
  
  if f_tW > 0.9 | f_tW2 > 0.9
  ...
  EWw_1 = L.^3 * (1 + f_tW * ome); % g, weight
  %
  rT_B = TC * k_M/ 3/ (1 + f_tW2/ g); % 1/d, von Bert growth rate
  L_b = L_m * get_lb([g k v_Hb], f_tW2); L_i = L_m * f_tW2;
  L = L_i - (L_i - L_b) * exp(-rT_B * tW2(:,1)); % cm struct length
  EWw_2 = L.^3 * (1 + f_tW2 * ome); % g, weight
