function [prdData, info] = predict_Turdus_merula(par, data, auxData)
  ...
  % time-weight
  % time-weight
  tf = [04 08 12; f_04 f_08 f_12]'; tf(:,1) = tf(:,1) * kT_M;
  ...
  % embryo-data
  vT = v * TC_ab; kT_J = k_J * TC_ab; kT_M = k_M * TC_ab; JT_E_Am = TC_ab * J_E_Am;
  pars_UE0 = [V_Hb/ TC; g; kT_J; kT_M; vT];
  U_E0 = initial_scaled_reserve(f, pars_UE0);
  % aW-data
  a = [0; tWe(:,1) ]; for i=2:length(a); if a(i)<=a(i-1); a(i)=a(i-1)+1e-8;end;end
  [a LUH] = ode45(@dget_LUH, a, [1e-10 U_E0 0], [], kap, vT, kT_J, g, L_m); 
  LUH(1,:) = []; L = LUH(:,1); % cm, structural length
  EWw_e = L .^ 3 * (1 + f * ome); % g, wet weight embryo minus yolk
  % aJO-data
  a = [0; tJOe(:,1) ]; for i=2:length(a); if a(i)<=a(i-1); a(i)=a(i-1)+1e-8;end;end
  [a LUH] = ode45(@dget_LUH, a, [1e-10 U_E0 0], [], kap, vT, kT_J, g, L_m); 
  LUH(1,:) = [];
  ...
  EJO_e = - J_M(:,3) * 1e3/ X_gas/ 24; % ml/h, O2 flux
