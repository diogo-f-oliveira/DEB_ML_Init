function [prdData, info] = predict_Cepaea_nemoralis(par, data, auxData)
  ...
  Wd_i = (2.2 * del_M)^3 * (1 + f * ome) * d_V;     % g, dry weight at 2.2 m shell diameter 
...
function dL = get_L(t, L, L_i, r_B, T_A, T_ref) 
  TC = tempcorr(C2K(6 + 18 * cos(2*pi*(t-80)/365)), T_ref, T_A);
  dL = TC * r_B * (L_i - L);
end