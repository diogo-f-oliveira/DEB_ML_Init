function [prdData, info] = predict_Lolliguncula_brevis(par, data, auxData)
  ...
  L_amm = L_im - (L_im - L_jm) * exp(- rT_Bm * (aT_mm - t_j/ kT_M));  
  ...
  % male
  tvel = get_tj(pars_tjm, f, [t_b, f*z/z_m, l_b*z/z_m], tWw_m(:,1)*kT_M);
  EWw_m = (L_m * tvel(:,4)).^3 * (1 + f * w_m);   % g, weight
