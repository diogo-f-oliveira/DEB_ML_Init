function [prdData, info] = predict_Phascogale_calura(par, data, auxData)
  ...
  % males
  Ww_im = L_m^3 * (1 + f * w);         % g, ultimate wet weight
  ...
  EWw_f = L.^3 * (1 + f * w); % g, weight
  %
  L = L_i - (L_i - L_b) * exp(-rT_B * tW_m(:,1)); % cm, struct length
  EWw_m = L.^3 * (1 + f * w); % g, weight
