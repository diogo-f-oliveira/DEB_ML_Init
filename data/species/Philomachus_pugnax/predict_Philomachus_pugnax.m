function [prdData, info] = predict_Philomachus_pugnax(par, data, auxData)
...
% life cycle
  ...
  % time-weight
  % female
  tf = [5 f_05; 20 f_20; 30 f]; tf(:,1) = tf(:,1) * kT_M;
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  % male
  tf = [5 f_05; 10 f_10; 20 f_20; 30 f]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tpm, tf, [0, z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
