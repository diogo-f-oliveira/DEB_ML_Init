function [prdData, info] = predict_Archaeopteryx_lithographica(par, data, auxData)
  ...
  % time-weight
  ir_B = 3/ k_M + 3 * f * L_m/ v; rT_B = TC/ ir_B; % yr, 1/von Bert growth rate
  L = L_i - (L_i - L_b) * exp( - rT_B * tW(:,1));        % cm, structural length at time
  EWw = L .^ 3 * (1 + f * w);                        % kg, wet weight
