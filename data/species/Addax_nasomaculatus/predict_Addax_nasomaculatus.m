function [prdData, info] = predict_Addax_nasomaculatus(par, data, auxData)
...
%   end
  ...
  % life cycle
  pars_tx = [g k l_T v_Hb v_Hx v_Hp]; % a bit faster than slow development
  ...
  % time-weight 
  % females
  rT_B = TC * k_M/ 3/ (1 + f_tW/ g);                     % 1/d, von growth rate
  L_b = L_m * get_lb_foetus([g k v_Hb]); L_i = L_m * f_tW; % cm, struc length
  L = L_i - (L_i - L_b) * exp( - rT_B * tW_f(:,1));   % cm, structural length at time
  EWw_f = L.^3 * (1 + f * w)/1e3;                     % kg, wet weight
