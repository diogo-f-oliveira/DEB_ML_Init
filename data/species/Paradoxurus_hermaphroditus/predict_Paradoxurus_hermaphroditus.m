function [prdData, info] = predict_Paradoxurus_hermaphroditus(par, data, auxData)
  ...
  L = L_i - (L_i - L_b) * exp(- rT_B * tW_f(:,1));
  EWw_f = L.^3 * (1 + f * w);
  %
  L = L_i - (L_i - L_b) * exp(- rT_B * tW_m1(:,1));
  EWw_m1 = L.^3 * (1 + f * w);
  %
  L = L_i - (L_i - L_b) * exp(- rT_B * tW_m2(:,1));
  EWw_m2 = L.^3 * (1 + f * w);
