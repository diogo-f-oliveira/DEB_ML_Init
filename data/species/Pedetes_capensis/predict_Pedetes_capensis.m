function [prdData, info] = predict_Pedetes_capensis(par, data, auxData)
  ...
  % weaning
  [t_x, t_b, l_x, l_b] = get_tp([g k l_T v_Hb v_Hx], f_bx); % map weaning to puberty
  L_x = L_m * l_x;                  % cm, structural length at weaning at f
  Ww_x = L_x^3 * (1 + f_bx * w);    % g, wet weight at weaning at f 
  tT_x = (t_x - t_b)/ k_M/ TC;      % d, time since birth at weaning at f and T
  
  % puberty 
  [t_p, t_bb, l_p] = get_tp([g k l_T v_Hx v_Hp], f); % map weaning to "birth"
  L_p = L_m * l_p;                  % cm, structural length at puberty at f
  Ww_p = L_p^3 * (1 + f * w);       % g, wet weight at puberty at f 
  tT_p = (t_p - t_bb)/ k_M/ TC;     % d, time since birth at puberty at f and T
