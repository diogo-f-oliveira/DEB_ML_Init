function [prdData, info] = predict_Glyptosternon_maculatum(par, data, auxData)
  ...
  %
  tvel = get_tj(pars_tjm, tf ,[tau_b 1 l_b], tL_fm(:,1)*kT_M);
  ELw_m = L_mm*tvel(:,4)/ del_M; % cm, std length 

  % length-weight
  EWw_f = (LW_f(:,1)*del_M).^3*(1+ome); % g, weight
  EWw_m = (LW_m(:,1)*del_M).^3*(1+ome_m); % g, weight
