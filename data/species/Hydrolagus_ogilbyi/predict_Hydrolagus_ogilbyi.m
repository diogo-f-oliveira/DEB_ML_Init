function [prdData, info] = predict_Hydrolagus_ogilbyi(par, data, auxData)  
  ...
  RT_i = TC * reprod_rate(70*del_M, f, pars_R);                % #/d, ultimate reproduction rate at T 
  ...
  % time-length
  % female
  rT_B = kT_M/3/(1+f_tL/g); Lw_i = f_tL * L_m/ del_M; Lw_b = L_m * get_lb([g; k; v_Hb], f_tL)/ del_M;
  ELw_f = Lw_i - (Lw_i-Lw_b)*exp(-rT_B*tL_f(:,1));
  % male
  rT_Bm = kT_M/3/(1+f_tL/g_m); Lw_im = f_tL * L_mm/ del_M; Lw_bm = L_mm * get_lb([g_m; k; v_Hb], f_tL)/ del_M;
  ELw_m = Lw_im - (Lw_im-Lw_bm)*exp(-rT_Bm*tL_m(:,1));
