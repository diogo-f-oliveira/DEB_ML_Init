function [prdData, info] = predict_Porcellio_scaber(par, data, auxData)
  ...
  [t_p, t_b, l_p, l_b] = get_tp(pars_tpm, f);%, [t_b, f*z/z_m, l_b*z/z_m]);
  ...
  % time-weight
  % female
  l_0 = (W_0/ (1 + f_tW * w))^(1/3)/ L_mm;
  tvel = get_tpm(pars_tp, f_tW, [t_b, f_tW, l_0], tW_f(:,1)*k_M*TC_tW);
  EWw_f = (L_m * tvel(:,4)).^3 * (1 + f_tW * w);   % g, weight
  % male
  tvel = get_tpm(pars_tpm, f_tW, [t_b, f_tW*z/z_m, l_0*z/z_m], tW_m(:,1)*k_M*TC_tW);
