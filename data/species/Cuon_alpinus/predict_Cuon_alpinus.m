function [prdData, info] = predict_Cuon_alpinus(par, data, auxData)
  ...
  % feeding rate
  JT_Xi = TC * p_Am * L_m^2/ kap_X * w_X/ mu_X/ d_X; % g/d, ultimate feeding rate 
  ...
  % time - weight  
  rT_B = kT_M/ 3/ (1 + f_1/ g);  % 1/d, von Bert growth rate
  L_b = L_m * get_lb([g k v_Hb], f_1); L_i = L_m * f_1;
  L = L_i - (L_i - L_b) * exp( - rT_B * tWw(:,1)); % cm, struct length
  EWw_1 = L.^3 * (1 + w * f_1); % kg, total weight female
  %
  rT_Bm = kT_M/ 3/ (1 + f_2/ g_m);  % 1/d, von Bert growth rate
  L_bm = L_mm * get_lb([g_m k v_Hb], f_2); L_im = L_m * f_2;
  L = L_im - (L_im - L_bm) * exp( - rT_Bm * tWw(:,1)); % cm, struct length
  EWw_2 = L.^3 * (1 + w_m * f_2); % kg, total weight male
  %
  rT_Bm = kT_M/ 3/ (1 + f_3/ g_m);  % 1/d, von Bert growth rate
  L_bm = L_mm * get_lb([g_m k v_Hb], f_3); L_im = L_m * f_3;
  L = L_im - (L_im - L_bm) * exp( - rT_Bm * tWw(:,1)); % cm, struct length
  EWw_3 = L.^3 * (1 + w_m * f_3); % kg, total weight male
  
  % pack to output
  prdData.tWw = [EWw_1, EWw_2, EWw_3];
