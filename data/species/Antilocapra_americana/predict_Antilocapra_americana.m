function [prdData, info] = predict_Antilocapra_americana(par, data, auxData)
  ...
  % life cycle
  pars_tx = [g k l_T v_Hb v_Hx v_Hp sF]; 
  ...
  % t-Ww data
  f = f_tW; L_b = L_m * get_lb([g k v_Hb], f); L_i = L_m * f;
  ...
  %
  f = f_tW1; L_b = L_m * get_lb([g k v_Hb], f); L_i = L_m * f;
  ir_B = 3/ k_M + 3 * f * L_m/ v; rT_B = TC/ ir_B; % d, 1/von Bert growth rate
  L = L_i - (L_i - L_b) * exp( - rT_B * tW1(:,1)); % cm, struct length
  EWw_1 = L.^3 * (1 + f * w)/ 1e3; % kg, weight
  
  % t-Ww_e data for slow development
  f = 1; L_b = L_m * get_lb([g k v_Hb], f); L_i = L_m * f;
  ir_B = 3/ k_M + 3 * f * L_m/ v; rT_B = TC/ ir_B; % d, 1/von Bert growth rate
  L = L_i * (1 - exp( - rT_B * max(0,tWe(:,1) - t_0))); % cm, struct length
  EWw_e = L.^3 * (1 + f * w)/ 1e3; % kg, weight
