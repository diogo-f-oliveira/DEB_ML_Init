function [prdData, info] = predict_Streptocephalus_proboscideus(par, data, auxData)
...
%   Ww_im = L_im^3 * (1 + f * w_m);      % g, ultimate wet weight
  ...
  % time-length
  tf = [5 f2_05; 10 f2_10; 15 f2_15; 20 f2_20]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tj(pars_tj, tf,[],tL(:,1)*kT_M); 
  ELw_2 = L_m*tvel(:,4)/del_M;
  %
  tf = [10 f1_10; 20 f1_20; 30 f1_30; 35 f1_35]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tj(pars_tj, tf,[],tL(:,1)*kT_M); 
  ELw_1 = L_m*tvel(:,4)/del_M;
  %
  tf = [10 f0_10; 15 f0_15; 20 f0_20; 25 f0_25; 30 f0_30; 35 f0_35]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tj(pars_tj, tf,[],tL(:,1)*kT_M); 
  ELw_0 = L_m*tvel(:,4)/del_M;

  % pack to output
  prdData.tL = [ELw_2 ELw_1 ELw_0];
