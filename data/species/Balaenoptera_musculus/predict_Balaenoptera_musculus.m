function [prdData, info] = predict_Baleanoptera_musculus(par, data, auxData)
...
%% compute temperature correction factors
  ...
  % weaning (after puberty to compute shape coefficient)
  L_x = L_m * l_x;                  % cm, structural length at weaning
  w_pi = min(1,(L_x - L_b)/ (L_p - L_b)); w_bp = 1 - w_pi; % -, weight coefficients
  del  = del_Mb * w_bp + del_M * w_pi; % -, shape coefficient
  Lw_x = L_x/ del;                  % cm, total length at weaning
...
%% pack to output
  ...
  l = tvel(:,4); w_pi = min(1,(l - l_b)/ (l_p - l_b)); w_bp = 1 - w_pi;
  ELw_f = L_m * l ./ (del_Mb * w_bp + del_M * w_pi); % cm, length
  % male
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tpm, f_tL, [t_b, f_tL*z/z_m, l_b*z/z_m], tL_m(:,1)*kT_M);
  l = tvel(:,4); w_pi = min(1,(l - l_b)/ (l_p - l_b)); w_bp = 1 - w_pi;
  ELw_m = L_mm * l ./ (del_Mb * w_bp + del_M * w_pi); % cm, length
