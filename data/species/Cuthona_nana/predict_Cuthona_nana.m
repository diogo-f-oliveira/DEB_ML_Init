function [prdData, info] = predict_Cuthona_nana(par, data, auxData)
  ...
  a_b = t_b/ k_M; aT_b = t_0 + a_b/ TC_ab;  % d, age at birth at f and T
  ...
  % length-feeding rate
  EJT_X = 1e3 * TC_LJX * p_Am * (LJX(:,1) * del_M).^2 * w_X/ kap_X/ mu_X/ d_X; % mg/d, feeding rate

  % temp-age at birth
  EaT_b = (t_0 + a_b) ./ tempcorr(C2K(Tab(:,1)), T_ref, T_A); % d, age at birth at T
