function [prdData, info] = predict_Rana_chensinensis(par, data, auxData)
  ...
  ELw_Mf = (L_i - (L_i - L_j) * exp(-rT_B * tL_Mfm(:,1)))/ del_M;  % cm, SVL
  %
  L_im = L_mm * f_M; 
  rT_Bm = TC_M * k_M/ 3/ (1 + f_M/ g_m); % 1/d, von Bert growth rate
  ELw_Mm = (L_im - (L_im - L_j) * exp(-rT_Bm * tL_Mfm(:,1)))/ del_M;  % cm, SVL
%
  L_i = L_m * f_H; 
