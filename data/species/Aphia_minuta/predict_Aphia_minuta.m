function [prdData, info] = predict_Aphia_minuta(par, data, auxData)
...
% zero-variate data
  ...
  % time-length 
  [tvel,~,~,~,l_b] = get_tp(pars_tp, f_tL, [], tL_f(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length
  %
  tvel = get_tp(pars_tpm, f_tL, [0 1 l_b*z/z_m], tL_m(:,1)*kT_M);
