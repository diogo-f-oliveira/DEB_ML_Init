function [prdData, info] = predict_Lithobates_virgatipes(par, data, auxData)
...
%   Ww_pm = L_pm^3 * (1 + f * w_m);      % g, wet weight at puberty
  ...
  L = (Wr(:,1)/(1+f_Wr*w)).^(1/3); % % cm, struc length
  Er = TC * v * (f_Wr./ L - 1/L_m)/ (f_Wr + g);  % 1/d, spec growth rate
