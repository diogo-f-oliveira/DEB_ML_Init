function [prdData, info] = predict_Mormyrus_kannume(par, data, auxData)
  ...
  EWw_m = (LW_m(:,1)*del_M).^3*(1+f_LW*ome); % g, weight
  
  % length-fecundity
  EN_L = TC * 365 * reprod_rate_j(LN(:,1)*del_M, f_tL, pars_R); % #, yearly fecundity

  % weight-fecundity
  EN_W = TC * 365 * reprod_rate_j((WN(:,1)/(1+f_tL*ome)).^(1/3), f_tL, pars_R); % #, yearly fecundity
