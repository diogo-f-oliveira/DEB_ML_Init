function [prdData, info] = predict_Surnia_ulula(par, data, auxData)
  ...
  % time-weight 
  L_0 = (WW(:,1)/ (1 + f_tW * ome)).^(1/3); L_i = L_m * f_tW; % cm, struc length
  rT_B = TC * k_M/ 3/ (1 + f_tW/ g);                   % 1/d, von Bert growth rate
  L = L_i - (L_i - L_0) * exp( - rT_B * 10);           % cm, structural length at time
