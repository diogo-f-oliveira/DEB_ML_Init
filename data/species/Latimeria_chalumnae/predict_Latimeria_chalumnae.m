function [prdData, info] = predict_Latimeria_chalumnae(par, data, auxData)
  ...
  Lw_i = L_i/ del_M;                % cm, ultimate total length for females
  Ww_i = L_i^3 * (1 + f * w);       % g, ultimate wet weight 
  Lw_im = Lw_i * z_m/ z;            % cm, ultimate total length for males
  ...
  % length-weight
  EWw = (LW(:,1) * del_M).^3 * (1 + f * w);  % g, expected wet weight
