function [prdData, info] = predict_Sciurus_carolinensis(par, data, auxData)
  ...
  [t_pm, t_bm, l_pm, l_bm, info] = get_tp(pars_tpm, f, [t_b f*z/z_m l_b*z/z_m]);
  if ~info
    prdData = []; return
  end
  ...
  % time-weight
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tW, [], tWw_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male
  tvel = get_tp(pars_tpm, f_tW, [t_b, f_tW*z/z_m, l_b*z/z_m], tWw_m(:,1)*kT_M);
  EWw_m = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * w_m); % g, weight
  % female 1
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tW, [], tWw_1f(:,1)*kT_M);
  EWw_1f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male 1
  tvel = get_tp(pars_tpm, f_tW, [t_b, f_tW*z/z_m, l_b*z/z_m], tWw_1m(:,1)*kT_M);
  EWw_1m = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * w_m); % g, weight
