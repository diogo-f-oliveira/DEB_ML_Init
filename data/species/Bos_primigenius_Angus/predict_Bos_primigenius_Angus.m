function [prdData, info] = predict_Bos_primigenius_Angus(par, data, auxData)
  ...
  L_xm = L_mm * l_xm; L_pm = L_mm * l_pm; L_im = (f - l_T) * L_mm;
  Ww_xm = L_xm^3 * (1 + f * w_m); Ww_pm = L_pm^3 * (1 + f * w_m); Ww_im = L_im^3 * (1 + f * w_m);
  tT_gm = t_0 + t_bm/ k_M/ TC;    % d, gestation time for male calves
  tT_pm = (t_pm - t_bm)/ k_M/ TC; % d, age at puberty for males

  % milk production at ultimate size (r = 0)
  pars_L = [L_b; L_x; L_i; tT_x; kap_L];
  JXT_L = RT_i * p_Am * milk(f, pars_L)/ 2000; % g/d, milk production
