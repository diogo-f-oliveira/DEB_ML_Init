function [prdData, info] = predict_Podocnemis_lewyana(par, data, auxData)
  ...
  rT_B = TC * k_M/ 3/ (1 + f/ g);
  EdLw_f = 365 * rT_B * (Lw_i - LdL_f(:,1)); % cm/yr, carapace length
  %
  rT_Bm = TC * k_M/ 3/ (1 + f/ g_m); 
  EdLw_m = 365 * rT_Bm * (Lw_im - LdL_m(:,1)); % cm/yr, carapace length
  
  % temperature-incubcation time
  EaT_b = a_b ./ TC_ab;
