function [prdData, info] = predict_Circus_aeruginosus(par, data, auxData)
  ...
  % uni-variate data
  % time-weight
  tf = [0 f_00; 10 f_10; 20 f_20; 30 f_30; 40 f_40]; tf(:,1) = tf(:,1) * kT_M;
  % female
  ...
  %time-food intake
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [], tpX_f(:,1)*kT_M);
  EpX_f = p_Am * spline1(tpX_f(:,1)*kT_M, tf) .* (L_m * tvel(:,4)).^2/ kap_X/ 1e3; % kJ/d, food intake females
  %
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tpm, tf, [], tpX_m(:,1)*kT_M);
  EpX_m = p_Am_m * spline1(tpX_m(:,1)*kT_M, tf) .* (L_mm * tvel(:,4)).^2/ kap_X/ 1e3; % kJ/d, food intake females
