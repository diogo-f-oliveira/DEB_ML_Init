function [prdData, info] = predict_Regulus_regulus(par, data, auxData)
  ...
  % time-weight
  tf = [0 7 8 15 20 30; f_00 f_07 f_08 f_15 f_20 f_30]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 5 10 15 20; f1_00 f1_05 f1_10 f1_15 f1_20]'; tf(:,1) = tf(:,1) * kT_M;
