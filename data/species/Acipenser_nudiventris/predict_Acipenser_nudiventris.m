function [prdData, info] = predict_Acipenser_nudiventris(par, data, auxData)  
  ...
  %% uni-variate data
  L_b  = get_lb([g; k; v_Hb], f) * L_m; % cm, structural length at birth
  ir_B = 3/ k_M + 3 * f * L_m/ v;   r_B = 1/ ir_B; % d, 1/von Bert growth rate at f and T
  L_i  = f * L_m - L_T; % ultimate physical length at f
  L    = L_i - (L_i - L_b) * exp( - TC_tW * r_B * tW(:,1)); % cm, expected physical length at time
