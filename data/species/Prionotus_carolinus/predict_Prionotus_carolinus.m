function [prdData, info] = predict_Prionotus_carolinus(par, data, auxData) 
...
%% uni-variate data
  ...
  % time-length 
  tvel = get_tj(pars_tj,f_C,[],tL_CN(:,1)*kT_M);
  ELw_C = L_m * tvel(:,4)/ del_M;
  %
  tvel = get_tj(pars_tj,f_N,[],tL_CN(:,1)*kT_M);
  ELw_N = L_m * tvel(:,4)/ del_M;


%% pack to output
  prdData.tL_CN = [ELw_C ELw_N];
