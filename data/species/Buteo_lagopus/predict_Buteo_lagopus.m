function [prdData, info] = predict_Buteo_lagopus(par, data, auxData)
  ...
  % time-weight 
  tf = [10 f1_10; 20 f1_20; 30 f1_30; 40 f1_40; 50 f1_50; 60 f1_60]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW1(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [10 f2_10; 20 f2_20; 30 f2_30; 40 f2_40; 50 f2_50; 60 f2_60]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW2(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [10 f3_10; 20 f3_20; 30 f3_30; 40 f3_40; 50 f3_50; 60 f3_60]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW3(:,1)*kT_M);
  EWw_3 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [10 f4_10; 20 f4_20; 30 f4_30; 40 f4_40; 50 f4_50; 60 f4_60]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW4(:,1)*kT_M);
