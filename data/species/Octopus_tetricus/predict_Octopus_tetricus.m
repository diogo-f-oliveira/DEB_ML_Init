function [prdData, info] = predict_Octopus_tetricus(par, data, auxData)
  ...
  % time-weight
  % females
  tvel = get_tj(pars_tj, f_tW, [], tW_f(:,1) * kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 * (1 + f_tW * ome);   % g, wet weight
  % males
  tvel = get_tj(pars_tjm, f_tW, [], tW_m(:,1) * kT_M);
  EWw_m = (L_mm * tvel(:,4)).^3 * (1 + f_tW * ome);   % g, wet weight
