function [prdData, info] = predict_Pteropus_alecto(par, data, auxData)
  ...
  % time-length
  tf = [0 f_00; 60 f_10; 90 f_20; 120 f_30; 150 f_50]; % scaled func response in time
  [t, Le] = ode45(@get_Le, [0;tL_f(:,1)], [L_b; 1], [], tf, v * TC, g, L_m);
  Le(1,:) = []; ELw_f = Le(:,1)/ del_M;  % cm, length of forewarm
  %
  [t, Le] = ode45(@get_Le, [0;tL_m(:,1)], [L_b; 1], [], tf, v * TC, g_m, L_mm);
  Le(1,:) = []; ELw_m = Le(:,1)/ del_M;  % cm, legnth of forearm
