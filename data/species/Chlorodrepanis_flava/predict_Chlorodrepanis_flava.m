function [prdData, info] = predict_Chlorodrepanis_flava(par, data, auxData)
  ...
  tf = [2 f2_02; 5 f2_05; 10 f2_10; 15 f2_15]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [2 f3_02; 5 f3_05; 10 f3_10; 15 f3_15]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_3 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight


  % pack to output
  prdData.tW = [EWw_2 EWw_3];
end
