function [prdData, info] = predict_Pelophylax_nigromaculatus(par, data, auxData)
  ...
  TC_800 = tempcorr(temp.tL_f8, T_ref, T_A);
  ...
  % univariate data
  % time-length
  L_i = L_m * f_3; rT_B = TC * k_M/ 3/ (1 + f_3/ g);
  ELw_f3 = (L_i - (L_i - L_j) * exp(- rT_B * tL_f3(:,1)))/ del_M; % cm, SVL
  %
  L_im = L_mm * f_3; rT_Bm = TC * k_M/ 3/ (1 + f_3/ g_m);
  ELw_m3 = (L_im - (L_im - L_j) * exp(- rT_Bm * tL_m3(:,1)))/ del_M; % cm, SVL
  %
  L_i = L_m * f_8; rT_B = TC_800 * k_M/ 3/ (1 + f_8/ g);
  ELw_f8 = (L_i - (L_i - L_j) * exp(- rT_B * tL_f8(:,1)))/ del_M; % cm, SVL
  %
  L_im = L_mm * f_8; rT_Bm = TC_800 * k_M/ 3/ (1 + f_8/ g_m);
  ELw_m8 = (L_im - (L_im - L_j) * exp(- rT_Bm * tL_m8(:,1)))/ del_M; % cm, SVL

  
  % pack to output
  prdData.tL_f3 = ELw_f3;
  prdData.tL_m3 = ELw_m3;
  prdData.tL_f8 = ELw_f8;
  prdData.tL_m8 = ELw_m8;
