function [prdData, info] = predict_Diomedea_epomophora(par, data, auxData)
  ...
  tf = [0 150 300; f_000 f_150 f_300]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 150 300; f1_000 f1_150 f1_300]'; tf(:,1) = tf(:,1) * kT_M;
