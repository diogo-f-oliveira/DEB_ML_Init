function [prdData, info] = predict_Labeo_rohita(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  if f_00 > 1 || f_90 > 1; prdData = []; info = 0; return; end
  ...
  % time-length
  tf = [0 f_00; 90 f_90]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tj(pars_tj, tf, [], tW(:,1)*kT_M);
