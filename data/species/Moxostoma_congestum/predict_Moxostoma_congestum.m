function [prdData, info] = predict_Moxostoma_congestum(par, data, auxData)
...
%% compute temperature correction factors
  ...
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tLB);
  rT_B = rho_B * kT_M; tT_j = (t_j - t_b)/ kT_M; L_j = L_m * l_j; L_i = L_m * l_i;
  ELw_B = (L_i - (L_i - L_j) * exp( - rT_B * (tL_B(:,1) - tT_j)))/ del_M; % cm, total length
  %
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tLS);
  rT_B = rho_B * kT_M; tT_j = (t_j - t_b)/ kT_M; L_j = L_m * l_j; L_i = L_m * l_i;
  ELw_S = (L_i - (L_i - L_j) * exp( - rT_B * (tL_S(:,1) - tT_j)))/ del_M; % cm, total length
  %
  [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tLP);
  rT_B = rho_B * kT_M; tT_j = (t_j - t_b)/ kT_M; L_j = L_m * l_j; L_i = L_m * l_i;
  ELw_P = (L_i - (L_i - L_j) * exp( - rT_B * (tL_P(:,1) - tT_j)))/ del_M; % cm, total length
