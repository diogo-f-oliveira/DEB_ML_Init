function [prdData, info] = predict_Hydropotes_inermis(par, data, auxData)
  ...
  tT_pm = (t_pm - t_bm)/ k_M/ TC;      % d, time since birth to puberty
  ...
  % male
  tvel = get_tp(pars_tpm, f, [], tW_m(:,1)*kT_M);
  EWw_m = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w); % g, weight
