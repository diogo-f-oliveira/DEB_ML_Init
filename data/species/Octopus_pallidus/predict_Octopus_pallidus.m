function [prdData, info] = predict_Octopus_pallidus(par, data, auxData)
  ...
  TC_14 = tempcorr(temp.tWw_14, T_ref, T_A);
  TC_16 = tempcorr(temp.tWw_16, T_ref, T_A);
  ...
  % time-weight
  tvel = get_tj(pars_tj, f_tW, [], tWw_14(:,1) * k_M * TC_14);
  EWw_14 = (L_m * tvel(:,4)).^3 * (1 + f_tW * ome);   % g, wet weight
  %
  tvel = get_tj(pars_tj, f_tW, [], tWw_16(:,1) * k_M * TC_16);
  EWw_16 = (L_m * tvel(:,4)).^3 * (1 + f_tW * ome);   % g, wet weight
  
  % pack to output
  prdData.tWw_14 = EWw_14;
  prdData.tWw_16 = EWw_16;
