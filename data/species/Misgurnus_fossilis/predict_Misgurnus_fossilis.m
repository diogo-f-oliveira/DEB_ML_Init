function [prdData, info] = predict_Misgurnus_fossilis(par, data, auxData)
...
%% compute temperature correction factors
  ...
  % time-length
  % female NR
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, f_NR, [], tL_fmNR(:,1)*kT_M);
  ELw_fNR = L_m * tvel(:,4)/ del_M;   % cm, length 
  % male NR
  tvel = get_tj(pars_tj, f_NR, [t_b, f_NR*z/z_m, l_b*z/z_m], tL_fmNR(:,1)*kT_M);
  ELw_mNR = L_mm * tvel(:,4)/ del_M;   % cm, length 
  %
  % female RN
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, f_RN, [], tL_fmRN(:,1)*kT_M);
  ELw_fRN = L_m * tvel(:,4)/ del_M;   % cm, length 
  % male RN
  tvel = get_tj(pars_tjm, f_RN, [t_b, f_RN*z/z_m, l_b*z/z_m], tL_fmRN(:,1)*kT_M);
  ELw_mRN = L_mm * tvel(:,4)/ del_M;   % cm, length 
  
  % temperature-age at birth
  Eab = a_b./TC_ab; % d, age at birth
  
%% pack to output
  prdData.tL_fmNR = [ELw_fNR ELw_mNR];
  prdData.tL_fmRN = [ELw_fRN ELw_mRN];
  prdData.Tab = Eab;
