function [prdData, info] = predict_Bison_bonasus(par, data, auxData)
  ...
  % time-weight
  % female captive
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_C, [], tW_fC(:,1)*kT_M);
  EWw_fC = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male captive
  tvel = get_tp(pars_tpm, f_C, [t_b, f_C*z/z_m, l_b*z/z_m], tW_mC(:,1)*kT_M);
  EWw_mC = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * w_m); % g, weight
  % female free
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_F, [], tW_fF(:,1)*kT_M);
  EWw_fF = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male free
  tvel = get_tp(pars_tpm, f_F, [t_b, f_F*z/z_m, l_b*z/z_m], tW_mF(:,1)*kT_M);
  EWw_mF = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * w_m); % g, weight

  % pack to output
  prdData.tW_fC = EWw_fC;
  prdData.tW_fF = EWw_fF;
  prdData.tW_mC = EWw_mC;
  prdData.tW_mF = EWw_mF;
