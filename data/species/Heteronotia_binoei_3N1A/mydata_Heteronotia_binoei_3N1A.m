function [data, auxData, metaData, txtData, weights] = mydata_Heteronotia_binoei_3N1A
%% set metadata
metaData.phylum     = 'Chordata';
metaData.class      = 'Reptilia';
metaData.order      = 'Squamata';
metaData.family     = 'Gekkonidae';
metaData.species    = 'Heteronotia_binoei_3N1A';
metaData.species_en = 'Bynoe''s gecko 3N1A';
...
metaData.date_acc    = [2017 06 13];

%% set data
% zero-variate data;
data.ab = 87.7;     units.ab = 'd';    label.ab = 'age at birth';                    bibkey.ab = 'Kear2004a';
...
data.ap = data.ab+123; units.ap = 'd'; label.ap = 'age at puberty';              bibkey.ap = 'Kear2004b';
  ...
  comment.ap = 'Fig. 7 from Kearney''s PhD thesis - rough';
...
data.Lb  = 2.08;     units.Lb  = 'cm';   label.Lb  = 'snout to vent length at birth';       bibkey.Lb  = 'Kear2004c';
  comment.Lb = 'mean size of newborns';
data.Lp  = 3.8;      units.Lp  = 'cm';    label.Lp  = 'snout to vent length at puberty';    bibkey.Lp  = 'Kear2005';
  comment.Lp = 'smallest female that reproduced - likely an underesimate';
data.Li  = 5.3;     units.Li  = 'cm';   label.Li  = 'ultimate snout to vent length';        bibkey.Li  = 'Kear2005';
...
data.Ri  = 3.2/365;    units.Ri  = '#/d';  label.Ri  = 'maximum reprod rate';  bibkey.Ri  = 'Kear2005';
  temp.Ri = C2K(29);  units.temp.Ri = 'K'; label.temp.Ri = 'temperature';

% uni-variate data
data.Tab = [24  27  30; 143.4 87.7    59.9]'; units.Tab  = {'C','d'};  label.Tab  = {'temperature','development time'};  bibkey.Tab  = 'Kear2004a';

LW = csvread('LW.csv');
data.LW = cat(2,LW(:,1),LW(:,2));
units.LW = {'cm', 'g'};     label.LW = {'snout vent length', 'wet weight'};  bibkey.LW = {'Kear2004a','Robe2012'};

tW = csvread('tW.csv');
data.tW = cat(2,tW(:,1),tW(:,2));
units.tW = {'d', 'g'};     label.tW = {'time', 'wet weight'};  bibkey.tW = {'Kear2004c'};
temp.tW = C2K(29);  units.temp.tW = 'K'; label.temp.tW = 'temperature';
 
WO = csvread('WO.csv');
data.WO = cat(2,WO(:,1),WO(:,2));
...
metaData.biblist.(bibkey) = ['''@', type, '{', bibkey, ', ' bib, '}'';'];
%
bibkey = 'Kear2004b'; type = 'PhDthesis'; bib = [ ... % meant as example; replace this and further bib entries
   'author = {Kearney, M. R.}, ' ...
