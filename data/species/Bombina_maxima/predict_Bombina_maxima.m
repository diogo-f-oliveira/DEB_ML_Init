function [prdData, info] = predict_Bombina_maxima(par, data, auxData)
  ...
  L = L_i - (L_i - L_b) * exp(- rT_B * tL_fm(:,1)); ELw_f = L/ del_M; 
  %
  rT_Bm = TC * k_M/ 3/ (1 + f_tL/ g_m); L_im = f_tL * L_mm; L_bm = L_mm * get_lb([g_m; k; v_Hb], f_tL); 
  L = L_im - (L_im - L_bm) * exp(- rT_Bm * tL_fm(:,1)); ELw_m = L/ del_M; 
