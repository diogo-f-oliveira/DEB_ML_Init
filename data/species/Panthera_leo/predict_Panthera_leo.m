function [prdData, info] = predict_Panthera_leo(par, data, auxData)
  ...
  [t_pm, l_pm, l_bm] = get_tp(pars_tpm, f_tW, [t_b, z/z_m, l_b]);
  ...
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f_tW1, [], tW_1f(:,1)*kT_M);
  EWw_1f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * w);   % g, weight
  % male
  tvel = get_tp(pars_tpm, f_tW1, [t_b, f_tW1*z/z_m, l_b*z/z_m], tW_1m(:,1)*kT_M);
  ...
  prdData.tW_f = EWw_f;
  prdData.tW_m = EWw_m;
  prdData.tW_1f = EWw_1f;
  prdData.tW_1m = EWw_1m;
