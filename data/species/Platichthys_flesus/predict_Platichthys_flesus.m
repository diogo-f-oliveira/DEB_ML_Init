function [prdData, info] = predict_Platichthys_flesus(par, data, auxData)
  ...
  % birth
  aT_b = tau_b/ kT_M;                 % d, time since hatch at birth
  ...
  % time-length
  % female
  tvel = get_tj(pars_tj, f_tLW, [], tL_fm(:,1)*kT_M);
  L = L_m * tvel(:,4); ELw_f = L/ del_M;   % cm, length
  EWw_f = L.^3 .* (1 + ome_m * tvel(:,3)); % g, weight
  % male
  tvel = get_tj(pars_tjm, f_tLW, [0, z/z_m, l_b*z/z_m], tL_fm(:,1)*kT_M);
