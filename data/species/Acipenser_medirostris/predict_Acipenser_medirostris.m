function [prdData, info] = predict_Acipenser_medirostris(par, data, auxData)  
  ...
  RT_i = TC * reprod_rate(L_i, f, pars_R);          % #/d, reproduction rate
  ...
  % time-weight
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tpm(pars_tp, f, [], tWw_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 * (1 + f * w);   % g, weight
  % male
  tvel = get_tpm(pars_tpm, f, [t_b, f*z/z_m, l_b*z/z_m], tWw_m(:,1)*kT_M);
