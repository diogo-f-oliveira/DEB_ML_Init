function [prdData, info] = predict_Oenanthe_familiaris(par, data, auxData)
  ...
  %
  tf = [0 f2_00; 3 f2_03; 6 f2_06; 9 f2_09]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 f3_00; 3 f3_03; 6 f3_06; 9 f3_09]; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
