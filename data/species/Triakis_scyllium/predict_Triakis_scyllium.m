function [prdData, info] = predict_Triakis_scyllium(par, data, auxData)
  ...
  % time-length
  tvel = get_tp(pars_tp, f_tL, [], tL_f(:,1)*kT_M); % cm, total length
  ELw_f = L_m * tvel(:,4)/ del_M;
  %
  tvel = get_tp(pars_tpm, f_tL, [0, z/z_m, l_b*z/z_m], tL_m(:,1)*kT_M); % cm, total length
