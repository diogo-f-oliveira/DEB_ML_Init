function [prdData, info] = predict_Diagramma_pictum(par, data, auxData)
  ...
  RT_i = TC * reprod_rate_j(55*del_M, f, pars_R); % #/d, max reprod rate at TL 55 cm
  ...
  % time-length 
  tvel = get_tj(pars_tj, f_tL, [], tL_f(:,1)*kT_M/ del_M); 
  ELw_f = L_m * tvel(:,4)/ del_M; % cm, total length
  %
  tvel = get_tj(pars_tjm, f_tL, [0 f_tL*z/z_m l_b*z/z_m], tL_m(:,1)*kT_M/ del_M); 
