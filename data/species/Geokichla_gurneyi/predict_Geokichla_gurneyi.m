function [prdData, info] = predict_Geokichla_gurneyi(par, data, auxData)
  ...
  tf = [0 4 8 12; f1_00 f1_04 f1_08 f1_12]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 4 8 12; f2_00 f2_04 f2_08 f2_12]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
