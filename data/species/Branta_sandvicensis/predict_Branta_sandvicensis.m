function [prdData, info] = predict_Branta_sandvicensis(par, data, auxData)
...
% uni-variate data
  ...
  % time-weight
  tf = [5 f_05; 20 f_20; 40 f_40; 60 f_60; 80 f_80]; tf(:,1) = tf(:,1) * kT_M;
  % female
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, tf, [], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  % male
  tvel = get_tp(pars_tpm, tf, [t_b, f*z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
