function [prdData, info] = predict_Chionis_albus(par, data, auxData)
  
  % unpack par, data, auxData
  cPar = parscomp_st(par); vars_pull(par); 
  vars_pull(cPar);  vars_pull(data);  vars_pull(auxData);

  if E_Hx < 0 || p_M > 9500 || f_30 < 0
  ...
  Ww_bm = L_bm^3 * (1 + f * ome_m);      % g, ultimate wet weight
  ...
  % time-weight 
  tf = [0 20 30 35 40 50 60; f_00 f_20 f_30 f_35 f_40 f_50 f_60]'; tf(:,1) = tf(:,1) * kT_M;
