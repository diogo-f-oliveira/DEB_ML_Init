function [prdData, info] = predict_Procellaria_aequinoctialis(par, data, auxData)
  ...
  tf = [0 40 65 99; f_00 f_40 f_65 f_99]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 40 65 99; f1_00 f1_40 f1_65 f1_99]'; tf(:,1) = tf(:,1) * kT_M;
