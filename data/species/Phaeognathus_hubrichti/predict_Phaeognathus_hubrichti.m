function [prdData, info] = predict_Phaeognathus_hubrichti(par, data, auxData)
  ...
  [l_p l_b info] = get_lp(pars_tpm, f); % cm, SVL at puberty
  if ~info; prdData = []; return; end
  Lw_pm = L_mm * l_p/ del_M; % cm, total length at puberty
  Ww_im = L_im^3 * (1 + f * w_m);      % g, ultimate wet weigtht
