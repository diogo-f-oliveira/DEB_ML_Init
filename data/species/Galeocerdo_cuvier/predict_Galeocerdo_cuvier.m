function [prdData, info] = predict_Galeocerdo_cuvier(par, data, auxData)
  ...
  % time-length
  rT_B = kT_M/ 3/ (1 + f_tL/ g); Lw_i = L_m * f_tL/ del_M;
  EdLw_f = rT_B * (Lw_i - LdL_f(:,1)); % cm/d, change in fork length
  %
  rT_Bm = kT_M/ 3/ (1 + f_tL/ g_m); Lw_im = L_mm * f_tL/ del_M;
  EdLw_m = rT_Bm * (Lw_im - LdL_m(:,1)); % cm/d, change in fork length
