function [prdData, info] = predict_Watasenia_scintillans(par, data, auxData)
  ...
  TC = tempcorr(temp.am, T_ref, T_A); kT_M = k_M * TC; kT_M = k_M * TC;
  ...
  rT_Bm = rho_Bm * kT_M;
  L_jm = L_mm * l_jm;  L_pm = L_mm * l_pm; L_im = L_mm * l_im; % cm, structural lengths at f
  ...
  Lw_im = L_amm/ del_M; Lw_pm = L_pm/ del_M; % cm, mantle length at death
  Ww_im = L_amm^3 * (1 + f * w_m);     % g, wet weight at death
  ...
  % time-length
  % female
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, f_tL, [], (t_0+tL_f(:,1))*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, dorsal mantle length at time
  % male
  tvel = get_tj(pars_tjm, f_tL, [t_b, f_tL*z/z_m, l_b*z/z_m], (t_0+tL_m(:,1))*kT_M);
  ELw_m = L_mm * tvel(:,4)/ del_M;   % cm, dorsal mantle length at time

  % time-weight
  % female
  [tvel, t_j, t_p, t_b, l_j, l_p, l_b] = get_tj(pars_tj, f_tL, [], (t_0+tWw_f(:,1))*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 * (1 + f_tL * w);   % g, weight
  % male
  tvel = get_tj(pars_tjm, f_tL, [t_b, f_tL*z/z_m, l_b*z/z_m], (t_0+tWw_m(:,1))*kT_M);
