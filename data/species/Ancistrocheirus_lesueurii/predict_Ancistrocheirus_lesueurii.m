function [prdData, info] = predict_Ancistrocheirus_lesueurii(par, data, auxData)
  ...
  Ni = cum_reprod_j(650, f, pars_R); % #, fecundity
  ...
  [t_jm, t_pm, t_bm, l_jm, l_pm, l_bm, l_im, r_jm, r_Bm] = get_tj(pars_tjm, f, [t_b, f*z/z_m, l_b*z/z_m]);
  L_bm = L_mm * l_bm; L_jm = L_mm * l_jm; Lw_pm = L_mm * l_pm/ del_M; L_im = L_mm * l_im; 
  tT_jm = (t_jm - t_bm)/ kT_M; rT_jm = r_jm * kT_M; rT_Bm = r_Bm * kT_M; 
  pars_tmm = [pars_tjm; h_a/ k_M^2; s_G]; % compose parameter vector at T_ref
  aT_mm = get_tm_j(pars_tmm, f)/ kT_M; % d, mean life span at T
  L_amm = L_im - (L_im - L_jm) * exp(- rT_Bm * (aT_mm - tT_jm));  
  ...
  EWw_f = (L_m * tvel(:,4)).^3 * (1 + f * w);   % g, wet weight
  % male
  tvel = get_tj(pars_tj, f, [t_b, f*z/z_m, l_b*z/z_m], tWw_m(:,1)*kT_M);
  EWw_m = (L_m * tvel(:,4)).^3 * (1 + f * w_m);   % g, wet weight
