function [prdData, info] = predict_Menura_novaehollandiae(par, data, auxData)
  ...
  % uni-variate data
  % time-weight
  tf = [15 f1_15; 30 f1_30]; tf(:,1) =  tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW1(:,1)*kT_M);
  EWw_1 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [15 f2_15; 30 f2_30]; tf(:,1) =  tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW2(:,1)*kT_M);
  EWw_2 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [15 f3_15; 30 f3_30]; tf(:,1) =  tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW3(:,1)*kT_M);
  EWw_3 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [15 f4_15; 30 f4_30]; tf(:,1) =  tf(:,1) * kT_M;
  [~, ia, ic] = unique(tW4(:,1));
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW4(ia,1)*kT_M);
  EWw_4 = (L_m * tvel(ic,4)).^3 .* (1 + tvel(ic,3) * ome);   % g, weight
  %
  tf = [15 f5_15; 30 f5_30]; tf(:,1) =  tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [tau_b 1 l_b], tW5(:,1)*kT_M);
