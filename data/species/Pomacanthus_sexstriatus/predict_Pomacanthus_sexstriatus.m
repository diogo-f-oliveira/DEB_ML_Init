function [prdData, info] = predict_Pomacanthus_sexstriatus(par, data, auxData)
...
% zero-variate data
  ...
  tT_p = (t_p - t_b)/ k_M/ TC;      % d, timse since birt at puberty
  ...
  [l_jm, l_pm, l_bm] = get_lj(pars_tj, f);
  L_pm = L_mm * l_pm; Lw_pm = L_pm/ del_M; % cm, length at puberty
  L_im = L_mm * f * l_j/l_b; Lw_im = L_im/ del_M; % cm, ultimate length  
  Ww_im = L_im^3 * (1 + f * ome_m); % g, ultimate weight
