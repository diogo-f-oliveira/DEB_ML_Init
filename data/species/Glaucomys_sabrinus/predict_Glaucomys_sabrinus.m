function [prdData, info] = predict_Glaucomys_sabrinus(par, data, auxData)
  ...
  % time-weight
  rT_B = TC * k_M/ 3/ (1 + f_tW/ g);  % 1/d, von Bert growth 
  L_b = L_m * get_lb([g k v_Hb], f_tW); L_i = L_m * f_tW; 
  L = L_i - (L_i - L_b) * exp(- rT_B * tWw_f(:,1));  % cm, structural length at time
  EWw_f = L.^3 * (1 + f * w);                        % g, wet weight females
  %
  L = L_i - (L_i - L_b) * exp(- rT_B * tWw_m(:,1));  % cm, structural length at time
  EWw_m = L.^3 * (1 + f * w);                        % g, wet weight females
