function [prdData, info] = predict_Squalus_mitsukurii(par, data, auxData)  
  ...
  % puberty
  L_p = l_p * L_m; Lw_p = L_p/ del_M;   % cm, total length at puberty
  
  % ultimate
  L_i = f * L_m; Lw_i = L_i/ del_M;     % cm, max total length
  ...
  Ww_im = L_im^3 * (1 + w_m * f);      % g, ultimate wet weight
  ...
  EN = 2 * 365 * TC * reprod_rate(LN(:,1)*del_M, f, pars_R); % #, 2-yr fecundity
