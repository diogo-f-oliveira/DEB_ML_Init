function [prdData, info] = predict_Glossanodon_semifasciatus(par, data, auxData) 
...
%% uni-variate data
  ...
  % time-length 
  t = linspace(0,2000,300)'; 
  tvel = get_tj(pars_tj, f_tL, [], t*kT_M);
  tLw = [t, L_m*tvel(:,4)/del_M]; 
  t = spline1(LdL(:,1),tLw(:,[2 1])); 
  ELw = spline(t+time.LdL,tLw);
