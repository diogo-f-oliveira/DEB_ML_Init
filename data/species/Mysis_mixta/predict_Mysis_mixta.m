function [prdData, info] = predict_Mysis_mixta(par, data, auxData)
  ...
  rT_B = TC * rho_B * k_M; L_i = L_m * l_i;
  ELw = (L_i - (L_i - Lw_0 * del_M) * exp( - rT_B * tL(:,1)))/ del_M; % cm, length
  
  % time-weight 
  EWd = (L_i - (L_i - Lw_0 * del_M) * exp( - rT_B * tW(:,1))).^3 * d_V * (1 + f_tW * w); % g, dry weight
  
  % length weight
  EWd_L = (LW(:,1) * del_M).^3 * d_V * (1 + f_tW * w); % g, dry weight
