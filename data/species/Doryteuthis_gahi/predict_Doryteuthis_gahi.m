function [prdData, info] = predict_Doryteuthis_gahi(par, data, auxData)
  ...
  % time-weight
  tvel = get_tj(pars_tj, f, [], tW_f(:,1)*kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 * (1 + f * w);   % g, wet weight
  % male
  tvel = get_tj(pars_tjm, f, [t_b, f*z/z_m, l_b*z/z_m], tW_m(:,1)*kT_M);
  EWw_m = (L_mm * tvel(:,4)).^3 * (1 + f * w_m);   % g, wet weight
