function [prdData, info] = predict_Chordeiles_minor(par, data, auxData)
  ...
  % time-weight 
  L_0 = (W_0/ (1 + f_tW * ome))^(1/3); % cm, struc length
  tvel = get_tp(pars_tp, f_tW, L_0/L_m, tW(:,1)*kT_M);
  EWw = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

   % time-weight: varying food
  tf = [0 3 8 35; f_00 f_03 f_08 f_35]'; tf(:,1) = tf(:,1) * kT_M;
