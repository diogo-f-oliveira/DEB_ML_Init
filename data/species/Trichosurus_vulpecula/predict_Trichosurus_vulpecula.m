function [prdData, info] = predict_Trichosurus_vulpecula(par, data, auxData)
  ...
  ELw = L/ del_M;                                     % cm, head+body length
  %
  L = L_i - (L_i - L_b) * exp( - rT_B * tWw(:,1));      % cm, structural length at time
  EWw = L.^3 * (1 + f_tW * w);                         % g, weight
  %
  L = L_i - (L_i - L_b) * exp( - rT_B * tWw_j(:,1));      % cm, structural length at time
  EWw_j = L.^3 * (1 + f_tW * w);                         % g, weight
