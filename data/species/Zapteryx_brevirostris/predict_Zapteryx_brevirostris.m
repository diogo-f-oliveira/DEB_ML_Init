function [prdData, info] = predict_Zapteryx_brevirostris(par, data, auxData)  
  ...
  pars_lpm = [g_m; k; l_T; v_Hb; v_Hpm];  % compose parameter vector
  Lw_pm = L_mm *  get_lp(pars_lpm, f)/ del_M; % cm, total length at puberty
  ...
  % uni-variate data
  % time-length
  rT_B = kT_M/ 3/ (1 + f_tL/ g); Lw_b = L_m * get_lb([g k v_Hb])/ del_M; Lw_i = L_m * f_tL/ del_M;
