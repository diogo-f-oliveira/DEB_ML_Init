function [prdData, info] = predict_Xiphophorus_helleri(par, data, auxData)
  ...
  % time - weight
  pars_tp = [g k l_T v_Hb v_Hp];
  [tau_p, tau_b, l_p, l_b,  info] = get_tp(pars_tp, f_tWw);
  if info == 0
    prdData = []; return
  end
  L_b = L_m * l_b; L_i = L_m * f_tWw; rT_B = TC_tWw * k_M/ 3/ (1 + f_tWw/ g);
  L = L_i - (L_i - L_b) * exp(-rT_B * tWw(:,1));
  EWw = L.^3 * (1 + f_tWw * w);
