function [prdData, info] = predict_Crystallogobius_linearis(par, data, auxData)
...
% zero-variate data
  ...
  [tau_pm, tau_bm, l_pm, l_bm, l_im] = get_tp(pars_tpm, f);
  Lw_pm = L_mm * l_pm/ del_M;         % cm, total length at puberty
  Lw_im = L_mm * f/ del_M;            % cm, total length
  Ww_im = (L_mm * f)^3*(1+f*ome_m);   % g, weight
  ...
  % time-length 
  [tvel,~,~,~,l_b] = get_tp(pars_tp, f_tL, [], tL_fm(:,1)*kT_M);
  ELw_f = L_m * tvel(:,4)/ del_M;   % cm, length
  %
  tvel = get_tp(pars_tpm, f_tL, [0 1 l_b*z/z_m], tL_fm(:,1)*kT_M);
