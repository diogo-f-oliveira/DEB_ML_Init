function [prdData, info] = predict_Ovibos_moschatus(par, data, auxData)
  ...
  Ww_xm = L_xm^3 * (1 + f * w_m);      % g, wet weight at weaning
  ...
  % male
  [tvel, t_p, t_b, l_p, l_b] = get_tp(pars_tp, f, [t_b, f*z/z_m, l_b*z/z_m], tW(:,1)*kT_M);
  EWw = (L_mm * tvel(:,4)).^3 .* (1 + tvel(:,3) * w_m);   % g, weight
