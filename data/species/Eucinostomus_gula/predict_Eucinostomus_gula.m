function [prdData, info] = predict_Eucinostomus_gula(par, data, auxData)
  ...
  % time-length
  tf = [0 f_0000; 1000 f_1000; 4000 f_4000]; tf(:,1) = tf(:,1) * kT_M;
  [tvel, ~, ~, ~, ~, ~, ~, ~, ~, ~, info] = get_tj(pars_tj, tf, [], tL(:,1)*kT_M); 
  if ~info; prdData = []; return; end
  ELw = L_m * tvel(:,4)/ del_M; % cm 

  % length-weight
  EWw = (LW(:,1) * del_M).^3 * (1 + f_4000 * ome);
