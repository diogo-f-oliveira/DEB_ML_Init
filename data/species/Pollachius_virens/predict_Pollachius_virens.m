function [prdData, info] = predict_Pollachius_virens(par, data, auxData) 
...
%% uni-variate data
  ...
  rT_B = TC * rho_B * k_M; rT_j = TC * rho_j * k_M; tT_j = (t_j - t_b)/ k_M/ TC;
  L_b = L_m * l_b;  L_j = L_m * l_j; L_i = L_m * l_i;
  L = L_b * exp(tL_1200(:,1) * rT_j/ 3) .* (tL_1200(:,1) < tT_j);
  L = L + (tL_1200(:,1) >= tT_j) .* (L_i - (L_i - L_j) * exp( - rT_B * (tL_1200(:,1) - tT_j))); % cm, expected length at time
  tLw_1200 = L/ del_M;
  
  
%% pack to output
  prdData.tL_1995 = tLw_1995;
  prdData.tL_1200 = tLw_1200;
