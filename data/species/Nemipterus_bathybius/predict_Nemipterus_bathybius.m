function [prdData, info] = predict_Nemipterus_bathybius(par, data, auxData)
...
%% compute temperature correction factors
  ...
  % time-length
  tf = [500 f_0500; 3000 f_3000]; tf(:,1) = tf(:,1)*kT_M;
  ...
  % time-weight
  tvel = get_tj(pars_tj, tf, [], tW_f(:,1) * kT_M);
  EWw_f = (L_m * tvel(:,4)).^3 .* (1+tvel(:,3)*ome);  
  tvel = get_tj(pars_tjm, tf, [t_b z/z_m l_b*z/z_m], tW_m(:,1) * kT_M);
  EWw_m = (L_mm * tvel(:,4)).^3 .* (1+tvel(:,3)*ome);  

  % length-fecundity
  EN = TC * 365 * reprod_rate_j((WN(:,1)/(1+ome*f)).^(1/3), f, pars_R); % #, fecundity
