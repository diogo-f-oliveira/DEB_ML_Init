function [prdData, info] = predict_Sapajus_apella(par, data, auxData)
  ...
  % univariate data
  % time-weight
  tf = [0 200 350; f_bx f_bx f_xi]';
  [t, EL] = ode45(@get_EL, tW_f(:,1), [E_m, L_b], [], tf, v * TC, g, E_m, L_m, p_Am * TC); % EL: {J/cm^3, cm}, with {[E], L}
  EWw_f = EL(:,2).^3 .* (1 + EL(:,1) * w_E/ mu_E/ d_E); % g, wet weight
  %
  [t, EL] = ode45(@get_EL, tW_m(:,1), [E_m_m, L_b], [], tf, v * TC, g, E_m_m, L_mm, p_Am_m * TC); % EL: {J/cm^3, cm}, with {[E], L}
