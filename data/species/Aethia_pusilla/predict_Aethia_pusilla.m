function [prdData, info] = predict_Aethia_pusilla(par, data, auxData)
...
% life cycle
  ...
  % time-weight  
  tf = [0 10 20 30; f_66_00 f_66_10 f_66_20 f_66_30]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW_66(:,1)*kT_M);
  EWw_66 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight
  %
  tf = [0 10 20 30; f_67_00 f_67_10 f_67_20 f_67_30]'; tf(:,1) = tf(:,1) * kT_M;
  tvel = get_tp(pars_tp, tf, [], tW_67(:,1)*kT_M);
  EWw_67 = (L_m * tvel(:,4)).^3 .* (1 + tvel(:,3) * ome);   % g, weight

  % pack to output
  prdData.tW_66 = EWw_66;
  prdData.tW_67 = EWw_67;
